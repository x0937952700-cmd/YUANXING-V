...
# 不在 gunicorn import app 時直接跑完整資料庫 migration。
# 舊版 502 的主因是啟動階段就連 PostgreSQL + 回填舊資料，Render 在服務尚未啟動前就被卡住。
# 這版改成：頁面先秒開；API 第一次需要資料時才初始化 DB。
if os.environ.get('YX_INIT_DB_ON_STARTUP', '0') == '1':
    try:
        init_db()
        DB_INITIALIZED = True
...
def ensure_db_ready(force=False):
...
def _ensure_db_before_api():
    if request.path.startswith('/api/') and request.path not in ('/api/health', '/api/db-status'):
        ensure_db_ready()
...
def _ensure_auto_user():
    if not AUTO_FIX101_UI:
        return
    # 頁面顯示不應該等待資料庫；否則 PostgreSQL 慢或 migration 卡住時，首頁/功能頁會 502。
    # API 寫入時仍會由 ensure_db_ready() 確保資料庫與 users 表存在。
...
@app.get('/api/region-customers')
@require_login
def api_region_customers():
    """Fast customer list for orders/master pages.

    Returns only grouped customer buttons first; product rows are loaded only
    after clicking a customer. This avoids opening pages by downloading every
    item row from PostgreSQL.
    """
    source = normalize_table(request.args.get('source') or 'master_orders')
    try:
        profiles = fetch_all('SELECT uid,name,region,trade_type FROM customer_profiles WHERE is_archived=0')
        by_uid = {r.get('uid'): r for r in profiles if r.get('uid')}
        by_name = {r.get('name'): r for r in profiles if r.get('name')}
        groups = fetch_all(f"""SELECT customer_uid, customer_name, COUNT(*) AS rows_count, COALESCE(SUM(qty),0) AS qty_sum
                            FROM {source}
                            WHERE COALESCE(customer_name,'')!='' OR COALESCE(customer_uid,'')!=''
                            GROUP BY customer_uid, customer_name
                            ORDER BY customer_name""")
        items = []
        for g in groups:
            p = by_uid.get(g.get('customer_uid') or '') or by_name.get(g.get('customer_name') or '') or {}
            name = p.get('name') or g.get('customer_name') or '未指定客戶'
            items.append({
                'uid': p.get('uid') or g.get('customer_uid') or '',
                'name': name,
                'region': p.get('region') or '北區',
                'trade_type': p.get('trade_type') or '',
                'total_qty': int(g.get('qty_sum') or 0),
                'total_rows': int(g.get('rows_count') or 0),
                'source': source,
            })
...
def register_item_routes(prefix, table_name):
    ...
    def list_route(table=table_name):
        return ok(items=list_items(table, request.args.get('customer_uid',''), request.args.get('customer_name',''), request.args.get('zone',''), request.args.get('search',''), request.args.get('limit'), request.args.get('offset', 0)))
...
def api_order_list_compat():
    return ok(items=list_items('orders', request.args.get('customer_uid',''), request.args.get('customer_name',''), request.args.get('zone',''), request.args.get('search',''), request.args.get('limit'), request.args.get('offset',0)))
...
def api_master_order_list_compat():
    return ok(items=list_items('master_orders', request.args.get('customer_uid',''), request.args.get('customer_name',''), request.args.get('zone',''), request.args.get('search',''), request.args.get('limit'), request.args.get('offset',0)))
...
@app.get('/health')
def health_page():
    # Render health check 要快，不能在這裡跑完整 migration 或大量 count。
    return ok(
        status='ok' if not STARTUP_DB_ERROR else 'degraded',
        db_initialized=DB_INITIALIZED,
        db_error=STARTUP_DB_ERROR or '',
...
def api_health():
    ...
    try:
        ...
        samples = {
            'inventory': fetch_all('SELECT id,customer_name,material,product_text,qty,zone FROM inventory ORDER BY id DESC LIMIT 3'),
            'orders': fetch_all('SELECT id,customer_name,material,product_text,qty,zone FROM orders ORDER BY id DESC LIMIT 3'),
            'master_orders': fetch_all('SELECT id,customer_name,material,product_text,qty,zone FROM master_orders ORDER BY id DESC LIMIT 3'),
        }
    except Exception as exc:
