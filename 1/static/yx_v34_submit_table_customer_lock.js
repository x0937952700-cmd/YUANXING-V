/* V34 final emergency lock: restore submit event from stable flow, keep UI locks. */
(function(){
  'use strict';
  if (window.__YX_V34_SUBMIT_TABLE_CUSTOMER_LOCK__) return;
  window.__YX_V34_SUBMIT_TABLE_CUSTOMER_LOCK__ = true;
  ...
  async function api(url, opt={}){
    const r = await fetch(url, {
      ...
      headers:{'Accept':'application/json','Content-Type':'application/json','Cache-Control':'no-cache','X-YX-V34':'true',...(opt.headers||{})}
    });
  ...
  function applyServerRows(m, customer, posted, items, activeZone){
    ...
    try {
      if (act?.rowsStore) {
        ...
        if (verify && typeof verify.catch === 'function') verify.catch(()=>{});
      }
  ...
  async function submitNow(ev){
    ...
    try {
      ...
      const posted = await api(apiPath(m), {method:'POST', body:JSON.stringify({customer_name:customer, ocr_text:text, items, duplicate_mode:mode, location:activeZone, zone:activeZone, region:(m==='orders'||m==='master_order')?'北區':'', request_key:`v34-submit-${m}-${Date.now()}-${Math.random().toString(36).slice(2)}`})});
      if (ta) ta.value = '';
