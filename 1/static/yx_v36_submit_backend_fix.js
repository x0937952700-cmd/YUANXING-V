/* V36 final lock: restore submit and protect backend/display flow. */
(function(){
  'use strict';
  if (window.__YX_V36_SUBMIT_BACKEND_FIX__) return;
  window.__YX_V36_SUBMIT_BACKEND_FIX__ = true;
  ...
  async function api(url, opt={}){
    const r = await fetch(url, {
      ...
      headers:{'Accept':'application/json','Content-Type':'application/json','Cache-Control':'no-cache','X-YX-V36':'true',...(opt.headers||{})}
    });
  ...
  function submittedRows(customer, items, location){
    const now = Date.now();
    return items.map((it, idx) => ({id:`tmp-v36-${now}-${idx}`, product_text:it.product_text, product_code:it.material||'', material:it.material||'', qty:it.qty, customer_name:customer||'', location:location||'', zone:location||'', warehouse_zone:location||'', __optimistic:true}));
  }
  ...
  async function submitNow(ev){
    ...
    try {
      ...
      const posted = await api(apiPath(m), {method:'POST', body:JSON.stringify({customer_name:customer, ocr_text:text, items, duplicate_mode:mode, location:activeZone, zone:activeZone, region:(m==='orders'||m==='master_order')?'北區':'', request_key:`v37-submit-${m}-${Date.now()}-${Math.random().toString(36).slice(2)}`})});
      if (ta) ta.value = '';
      ...
      try { window.dispatchEvent(new CustomEvent('yx:v37-submit-success', {detail:{module:m, customer_name:customer, items, posted}})); } catch(_e) {}
