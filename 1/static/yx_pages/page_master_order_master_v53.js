/* 沅興木業 FULL MASTER V53 PRODUCTS / SHIP / WAREHOUSE FIXED20 WRITEBACK - page_master_order_master_v53 */
(function(){ window.__YX_FULL_MASTER_V53_PAGE__='page_master_order_master_v53'; })();
...
(function(){
  ...
  function qtyOf(r){ const q=Number(r?.qty ?? r?.effective_qty ?? 0)||0; return q>0 ? q : qtyFromText(r?.product_text || r?.support || '', 0); }
  ...
  function rowActionsHTML(source, id){
    // V52：庫存 / 訂單 / 總單單列操作按鈕永久移除，不再產生操作欄。
    return '';
  }
  ...
  function renderSummary(source){
    ...
    // V51：庫存頁不再顯示「加到訂單 / 加到總單 / 移到A區 / 移到B區」這一排操作，避免之後又被加回。
    ...
    const zoneMoveButtons = source === 'inventory' ? '' : `<button class="ghost-btn small-btn" type="button" data-yx132-batch-zone="A" data-source="${source}">移到A區</button><button class="ghost-btn small-btn" type="button" data-yx132-batch-zone="B" data-source="${source}">移到B區</button>`;
    const controls = (moveButtons || zoneMoveButtons) ? `<div class="yx128-summary-controls">${moveButtons}${zoneMoveButtons}</div>` : '';
    const scope = editingIds(source);
    const displayRows = editing && scope ? rows.filter(r => scope.has(String(idOf(r) || ''))) : rows;
    const actionHeader = ''; // V52：所有產品頁移除操作欄
    const emptyColspan = 5;
    const body = displayRows.length ? displayRows.map(r => {
      const p = splitProduct(r.product_text || '');
      const id = idOf(r);
      if (!editing) {
        const actionCell = ''; // V53：庫存 / 訂單 / 總單全部移除單列操作欄
        return `<tr class="yx113-summary-row" data-source="${source}" data-id="${id}"><td class="mat"><input class="yx113-row-check" type="checkbox" data-id="${id}" data-source="${source}">${YX.esc(materialOf(r))}</td><td class="size">${YX.esc(p.size || r.product_text || '')}</td><td class="support">${YX.esc(p.support || String(qtyOf(r)))}</td><td class="qty total-qty">${qtyOf(r)}</td><td class="zone">${YX.esc(zoneLabel(r))}</td>${actionCell}</tr>`;
      }
      const editActionCell = ''; // V52：所有產品頁移除操作欄
  ...
  function cardHTML(source, r){
    const p = splitProduct(r.product_text || '');
    const q = qtyOf(r);
    const actions = ''; // V52：單列商品操作按鈕全部移除
    return `<div class="deduct-card yx113-product-card yx112-product-card ${Number(r.unplaced_qty || 0) > 0 ? 'needs-red' : ''}" data-source="${source}" data-id="${idOf(r)}"><div class="yx128-card-top"><strong class="material-text">${YX.esc(materialOf(r))}</strong><strong>${q}件</strong></div><button class="yx113-product-main" type="button" data-yx113-action="filter"><span>${YX.esc(p.size || r.product_text || '')}</span><span>${YX.esc(p.support || String(q))}</span></button>${customerOf(r) ? `<div class="small-note">${YX.esc(customerOf(r))}</div>` : ''}<div class="btn-row compact-row yx113-product-actions">${actions}</div></div>`;
  }
  ...
  function removeDeletedRowsFromDom(source, ids){
    ...
    if (tbody && !tbody.querySelector('tr')) tbody.innerHTML = '<tr><td colspan="'+(source === 'inventory' ? 5 : 6)+'">目前沒有資料</td></tr>';
  }
...
(function(){
  ...
  function forceSelectedCustomerProductPanel(customer, rows, source){
    customer = clean(customer || '');
    if (!customer || !['orders','master_order'].includes(source)) return;
    const panel = $('selected-customer-items');
    if (!panel) return;
    const list = (Array.isArray(rows) ? rows : []).filter(function(r){ return clean(r.customer_name || r.customer || '') === customer; });
    const total = list.reduce(function(sum, r){ return sum + (qtyFromProduct(r.product_text || '', r.qty || 1) || Number(r.qty || 0) || 0); }, 0);
    panel.classList.remove('hidden');
    panel.style.display = '';
    panel.dataset.yxV51ImmediateCustomerPanel = 'real-loaded';
    const cards = list.length ? list.map(function(it){
      const raw = String(it.product_text || '');
      const ps = raw.split('=');
      const material = it.material || it.product_code || '未填材質';
      const qty = qtyFromProduct(raw, it.qty || 1) || Number(it.qty || 0) || 1;
      const src = it.source || (source === 'orders' ? '訂單' : '總單');
      return `<div class="deduct-card yx112-product-card yx-v51-immediate-customer-item"><div class="yx113-product-head"><strong class="material-text">${esc(material)}</strong><strong>${qty}件</strong></div><div class="yx113-product-main"><span>${esc(ps[0] || raw)}</span><span>${esc(ps.slice(1).join('=') || it.qty || '')}</span></div><div class="small-note">${esc(src)}</div></div>`;
    }).join('') : '<div class="empty-state-card compact-empty">此客戶目前沒有商品</div>';
    panel.innerHTML = `<div class="customer-detail-card yx121-selected-customer-products yx-v51-selected-customer-products"><div class="customer-detail-header"><div><div class="section-title">${esc(customer)}</div><div class="muted">${total}件 / ${list.length}筆商品｜已立即同步下方清單</div></div></div><div class="card-list">${cards}</div></div>`;
  }
  ...
  async function refreshAfterSubmit(m, customer, posted, submittedItems, location){
    ...
    if (m === 'orders' || m === 'master_order') {
      try {
        const directRows = (exactRows && exactRows.length) ? exactRows : (Array.isArray(serverRows) ? serverRows : []);
        forceSelectedCustomerProductPanel(customer, directRows, m);
        window.YX113CustomerRegions?.clearSelectedCache?.(customer);
      } catch(_e) {}
      try { window.YX113CustomerRegions?.unhideCustomer?.(customer); } catch(_e) {}
      // 後端已在 /api/orders 或 /api/master_orders 寫入 customer_profiles，這裡先用回傳 customers 重畫，不等待二次 ensure。
      try { if (Array.isArray(posted?.hidden_customers) && window.YX113CustomerRegions?.setHiddenCustomers) window.YX113CustomerRegions.setHiddenCustomers(posted.hidden_customers); if (Array.isArray(posted?.customers) && window.YX113CustomerRegions?.renderBoards) window.YX113CustomerRegions.renderBoards(posted.customers); } catch(_e) {}
      try { forceCustomerCardVisible(customer, m); } catch(_e) {}
      try { window.YX113CustomerRegions?.renderFromCurrentRows?.(); } catch(_e) {}
      try { forceCustomerCardVisible(customer, m); } catch(_e) {}
      try { if (window.YX113CustomerRegions?.selectCustomer) await window.YX113CustomerRegions.selectCustomer(customer); } catch(_e) {}
  ...
  async function finalConfirmSubmit(ev){
    ...
    try{
      ...
      const requestKey = `v51-submit-${m}-${Date.now()}-${Math.random().toString(36).slice(2)}`;
...
/* ===== MERGED INTO V24 FROM static/yx_modules/customer_regions_hardlock.js ===== */
...
(function(){
  ...
  if (!YX) return;
  const $ = id => document.getElementById(id);
  const state = {items:[], bound:false, oldSelect:null, rendering:false, observer:null, repairTimer:null, lastRenderAt:0, itemCache:new Map(), hiddenNames:new Set()};
  ...
  function setHiddenCustomers(names){
    state.hiddenNames = new Set((Array.isArray(names) ? names : []).map(n => YX.clean(n || '')).filter(Boolean));
    try { window.__YX_HIDDEN_CUSTOMERS__ = Array.from(state.hiddenNames); } catch(_e) {}
  }
  function isHiddenCustomer(name){
    name = YX.clean(name || '');
    if (!name) return false;
    if (state.hiddenNames && state.hiddenNames.has(name)) return true;
    try { return Array.isArray(window.__YX_HIDDEN_CUSTOMERS__) && window.__YX_HIDDEN_CUSTOMERS__.map(x => YX.clean(x || '')).includes(name); } catch(_e) { return false; }
  }
  function unhideCustomer(name){
    name = YX.clean(name || ''); if (!name) return;
    try { state.hiddenNames.delete(name); window.__YX_HIDDEN_CUSTOMERS__ = Array.from(state.hiddenNames); } catch(_e) {}
  }
  ...
  function mergeCustomerRows(items, mode){
    ...
    const map = new Map();
    (items || []).forEach(c => {
      if (isHiddenCustomer(c.name || c.customer_name || '')) return;
  ...
  function relationCustomersFromRows(existingItems){
    ...
    (existingItems || []).forEach(c => { const n = YX.clean(c.name || c.customer_name || ''); if (n && !isHiddenCustomer(n)) byName.set(n, c); });
    const add = (name, source, row) => {
      name = YX.clean(name || ''); if (!name) return;
      if (isHiddenCustomer(name)) return;
  ...
  async function loadCustomerBlocks(force=true){
    ...
    try {
      ...
      setHiddenCustomers(d.hidden_customers || d.hiddenCustomers || []);
  ...
  function clearSelectedCache(name){
    try {
      name = YX.clean(name || '');
      if (!name) { state.itemCache.clear(); return; }
      state.itemCache.delete(mergeKey(name) || name);
    } catch(_e) {}
  }
  ...
  async function deleteCustomer(name){
    ...
    state.hiddenNames.add(YX.clean(name)); try { window.__YX_HIDDEN_CUSTOMERS__ = Array.from(state.hiddenNames); } catch(_e) {}
    ...
    try {
      ...
      const addRows = (key, rowsKey, qtyKey) => {
        (Array.isArray(stores[key]) ? stores[key] : []).forEach(r => {
          if (YX.clean(r.customer_name || r.customer || '') === name) { rc[rowsKey] += 1; { const q=Number(r.qty||0)||0; rc[qtyKey] += q>0 ? q : (qtyFromProduct(r.product_text || '', 1) || 1); } }
        });
      };
  ...
  async function ensureCustomerVisible(name, fallbackRegion='北區', preserveExisting=true){
    name = YX.clean(name || '');
    unhideCustomer(name); if (!name) return null;
  ...
  function lockGlobals(){
    ...
    window.YX113CustomerRegions = {loadCustomerBlocks, renderBoards, selectCustomer, ensureCustomerVisible, renderFromCurrentRows, moveCustomer, setHiddenCustomers, unhideCustomer, clearSelectedCache};
...
/* ===== MERGED INTO V24 FROM static/yx_pages/page_customers_master.js ===== */
...
/* ===== V50 product submit guard: real-loaded HTML/JS/CSS writeback ===== */
(function(){
  'use strict';
  if (window.__YX_V51_PRODUCTS_GUARD__) return;
  window.__YX_V51_PRODUCTS_GUARD__ = true;
  try {
  ...
  if (typeof originalConfirm === 'function' && !originalConfirm.__yxV51Wrapped) {
    var wrapped = async function(ev){
      ...
      try {
        if (btn) { btn.dataset.yxV51Submitting = '1'; btn.classList.add('yx-v51-submitting'); }
        ...
        if (r) {
          ...
          r.innerHTML = '<strong style="color:#b91c1c">送出失敗 / 未寫入清單</strong><div class="small-note">'+esc(e && e.message ? e.message : e)+'</div><div class="small-note yx-v51-submit-debug">V51 已寫入實際載入母版；若還看到舊錯誤，請重新整理清快取。</div>';
        }
        toast((e && e.message) || '送出失敗', 'error');
      } finally {
        if (btn) { delete btn.dataset.yxV51Submitting; btn.classList.remove('yx-v51-submitting'); }
      }
    };
    wrapped.__yxV51Wrapped = true;
...
/* ===== V50 products real-loaded guard: no inline double-submit, material select, product-only payload ===== */
(function(){
  'use strict';
  if (window.__YX_V51_PRODUCTS_GUARD__) return;
  window.__YX_V51_PRODUCTS_GUARD__ = true;
  ...
  document.addEventListener('DOMContentLoaded', function(){
    var btn=document.getElementById('submit-btn');
    if(btn){ btn.classList.add('yx-v51-submit-btn'); btn.removeAttribute('onclick'); btn.dataset.yxV51Submit='real-loaded'; }
    var screen=document.querySelector('.module-screen[data-module]');
    if(screen) screen.dataset.yxV50Products='real-loaded';
    var dp=document.getElementById('duplicate-action-panel');
    if(dp){ dp.classList.add('yx-v51-duplicate-panel'); dp.dataset.yxV51DuplicatePanel='real-loaded'; }
  }, {once:true});
})();


/* ===== V50 products real-loaded final submit capture + DOM/CSS lock ===== */
(function(){
  'use strict';
  if (window.__YX_V51_PRODUCTS_FINAL_CAPTURE__) return;
  window.__YX_V51_PRODUCTS_FINAL_CAPTURE__ = true;
  try {
    document.documentElement.dataset.yxV51ProductsWriteback = 'locked';
    window.YX = window.YX || window.YXHardLock || {toast:function(msg){try{alert(msg);}catch(_e){}}, clean:function(v){return String(v==null?'':v).trim();}};
  } catch(_e) {}
  ...
  function setup(){
    ...
    var btn=document.getElementById('submit-btn');
    if(btn){
      btn.classList.add('yx-v51-submit-btn');
      btn.removeAttribute('onclick');
      btn.dataset.yxV51Submit='real-loaded';
      btn.dataset.yxRealLoadedSubmit='v50';
    }
    var dp=document.getElementById('duplicate-action-panel');
    if(dp){ dp.classList.add('yx-v51-duplicate-panel'); dp.dataset.yxV51DuplicatePanel='real-loaded'; }
    var result=document.getElementById('module-result');
    if(result) result.dataset.yxV51Result='real-loaded';
  }
  ...
  window.addEventListener('click', function(ev){
    ...
    if(btn.dataset.yxV51Submitting === '1') return;
    btn.dataset.yxV51Submitting = '1';
    var done = function(){ try { delete btn.dataset.yxV51Submitting; } catch(_e){} };
    try {
...
/* ===== V51 PRODUCTS REAL LOADED WRITEBACK LOCK =====
   實際寫回母版：庫存移除單列操作按鈕；訂單/總單送出後立即刷新客戶與商品清單。 */
(function(){
  'use strict';
  if (window.__YX_V51_PRODUCTS_CUSTOMER_GUARD__) return;
  window.__YX_V51_PRODUCTS_CUSTOMER_GUARD__ = true;
  try {
    document.documentElement.dataset.yxV51ProductsWriteback = 'locked';
    window.YX = window.YX || window.YXHardLock || {toast:function(msg){try{alert(msg);}catch(_e){}}, clean:function(v){return String(v==null?'':v).trim();}};
  } catch(_e) {}
  ...
  function clean(v){ return String(v == null ? '' : v).trim(); }
  function esc(v){ return String(v == null ? '' : v).replace(/[&<>'"]/g, function(ch){ return {'&':'&amp;','<':'&lt;','>':'&gt;',"'":'&#39;','"':'&quot;'}[ch]; }); }
  function cssEscape(v){ try { return CSS.escape(String(v)); } catch(_e){ return String(v).replace(/[^a-zA-Z0-9_-]/g, '\\$&'); } }
  function toast(msg, type){ try { (window.YXHardLock?.toast || window.YX?.toast || window.alert)(msg, type); } catch(_e){ try{ alert(msg); }catch(__){} } }

  function removeInventoryRowActions(){
    try {
      var screen = document.querySelector('.module-screen[data-module="inventory"]');
      if (!screen) return;
      screen.dataset.yxV51InventoryNoRowActions = 'locked';
      var summary = document.getElementById('yx113-inventory-summary');
      if (!summary) return;
      summary.querySelectorAll('thead th').forEach(function(th){ if ((th.textContent||'').trim() === '操作') th.remove(); });
      summary.querySelectorAll('td.yx131-action-cell').forEach(function(td){ td.remove(); });
      summary.querySelectorAll('[data-yx131-row-action],.yx-v45-row-actions,[data-yx113-action="to-orders"],[data-yx113-action="to-master"]').forEach(function(el){ el.remove(); });
      summary.querySelectorAll('tbody tr').forEach(function(tr){
        var cells = tr.children ? tr.children.length : 0;
        if (cells > 5) {
          var last = tr.lastElementChild;
          if (last && ((last.classList && last.classList.contains('yx131-action-cell')) || /編輯|刪除|加到訂單|加到總單/.test(last.textContent||''))) last.remove();
        }
      });
    } catch(_e) {}
  }
  window.YX_V51_REMOVE_INVENTORY_ROW_ACTIONS = removeInventoryRowActions;

  function forceNorthCustomerCard(customer){
    customer = clean(customer);
    if (!customer) return;
    try { window.__YX_SELECTED_CUSTOMER__ = customer; } catch(_e) {}
    var input = document.getElementById('customer-name');
    if (input) input.value = customer;
    var html = '<button type="button" class="customer-region-card yx113-customer-card yx114-customer-card yx116-customer-card yx117-customer-card yx-v51-force-customer-card" data-customer-name="'+esc(customer)+'" data-customer="'+esc(customer)+'" data-region="北區"><span class="yx113-customer-left yx116-customer-name">'+esc(customer)+'</span><span class="yx113-customer-tag yx116-customer-tag">北區</span><span class="yx113-customer-count yx116-customer-count">已建立</span></button>';
    ['region-north','customers-north'].forEach(function(id){
      var box = document.getElementById(id);
      if (!box) return;
      box.querySelector('.empty-state-card')?.remove();
      var existed = Array.from(box.querySelectorAll('[data-customer-name],[data-customer]')).some(function(el){ return clean(el.dataset.customerName || el.dataset.customer) === customer; });
      if (!existed) box.insertAdjacentHTML('beforeend', html);
    });
  }

  async function ensureCustomerAndRefresh(m, customer){
    customer = clean(customer);
    var act = window.YX113ProductActions || window.YX132ProductActions || window.YX128ProductActions;
    try { if (m === 'inventory') removeInventoryRowActions(); } catch(_e) {}
    if (!customer || !['orders','master_order'].includes(m)) return;
    forceNorthCustomerCard(customer);
    try {
      var res = await fetch('/api/customers/ensure', {method:'POST', credentials:'same-origin', headers:{'Content-Type':'application/json'}, body:JSON.stringify({name:customer, region:'北區', preserve_existing:true, request_key:'v51-ensure-'+Date.now()+'-'+Math.random().toString(36).slice(2)})});
      var data = {}; try { data = await res.json(); } catch(_e) {}
      if (Array.isArray(data.items) && window.YX113CustomerRegions?.renderBoards) window.YX113CustomerRegions.renderBoards(data.items);
      forceNorthCustomerCard(customer);
    } catch(e) { forceNorthCustomerCard(customer); }
    try { await act?.loadSource?.(m, {force:true, v50AfterSubmit:true}); } catch(_e) {}
    try { act?.renderSummary?.(m); act?.renderCards?.(m); } catch(_e) {}
    try { await window.YX113CustomerRegions?.selectCustomer?.(customer); } catch(_e) {}
    try { window.YX113CustomerRegions?.renderFromCurrentRows?.(); } catch(_e) {}
    forceNorthCustomerCard(customer);
  }

  function setup(){
    removeInventoryRowActions();
    var act = window.YX113ProductActions || window.YX132ProductActions || window.YX128ProductActions;
    if (act && !act.__yxV51InventoryNoRowActions) {
      ['renderSummary','loadSource','refreshCurrent'].forEach(function(name){
        var orig = act[name];
        if (typeof orig !== 'function') return;
        act[name] = function(){
          var args = arguments;
          var ret = orig.apply(this, args);
          Promise.resolve(ret).finally(function(){ removeInventoryRowActions(); });
          return ret;
        };
      });
      act.__yxV51InventoryNoRowActions = true;
    }
    var btn = document.getElementById('submit-btn');
    if (btn) { btn.dataset.yxRealLoadedSubmit = 'v50'; btn.classList.add('yx-v51-submit-btn'); }
  }
  if (document.readyState === 'loading') document.addEventListener('DOMContentLoaded', setup, {once:true}); else setup();

  if (typeof window.confirmSubmit === 'function' && !window.confirmSubmit.__yxV51Wrapped) {
    var previous = window.confirmSubmit;
    var wrapped = async function(ev){
      var m = page();
      var customer = clean(document.getElementById('customer-name')?.value || window.__YX_SELECTED_CUSTOMER__ || '');
      try {
        var ret = await previous.call(this, ev);
        await ensureCustomerAndRefresh(m, customer);
        return ret;
      } catch(e) {
        await ensureCustomerAndRefresh(m, customer);
        throw e;
      } finally {
        setTimeout(removeInventoryRowActions, 0);
        setTimeout(removeInventoryRowActions, 150);
      }
    };
    wrapped.__yxV51Wrapped = true;
    window.confirmSubmit = wrapped;
  ...
  // Last-line protection: if any old renderer later injects inventory row buttons, remove them immediately.
  document.addEventListener('click', function(ev){
    if (document.querySelector('.module-screen[data-module="inventory"]') && ev.target?.closest?.('[data-yx131-row-action],.yx-v45-row-actions,[data-yx113-action="to-orders"],[data-yx113-action="to-master"]')) {
      ev.preventDefault(); ev.stopPropagation(); ev.stopImmediatePropagation && ev.stopImmediatePropagation();
      removeInventoryRowActions();
      toast('庫存單列操作按鈕已永久移除，請使用上方批量功能。','warn');
    }
  }, true);
  // V51：庫存操作欄已從實際 renderer 移除，不再用全頁 MutationObserver 反覆掃 DOM。
})();


/* ===== V52 PRODUCTS REAL LOADED HTML / JS / CSS / APP WRITEBACK =====
   1) 庫存 / 訂單 / 總單單列操作按鈕徹底不再產生。
   2) 送出後強制重讀後端真實資料，並直接刷新北區客戶與下方客戶商品清單。
   3) 這段寫在實際載入的 page_*_master_v53.js，不是外掛補丁。 */
(function(){
  'use strict';
  if(window.__YX_V52_PRODUCTS_WRITEBACK__) return;
  window.__YX_V52_PRODUCTS_WRITEBACK__ = true;
  try { document.documentElement.dataset.yxV52ProductsWriteback = 'locked'; } catch(_e) {}
  const clean = v => String(v == null ? '' : v).trim();
  const esc = v => String(v == null ? '' : v).replace(/[&<>'"]/g, ch => ({'&':'&amp;','<':'&lt;','>':'&gt;',"'":'&#39;','"':'&quot;'}[ch]));
  const page = () => document.querySelector('.module-screen[data-module]')?.dataset.module || '';
  const apiSource = m => m === 'master_order' ? 'master_orders' : m;
  const apiPath = m => m === 'inventory' ? '/api/inventory' : m === 'orders' ? '/api/orders' : '/api/master_orders';
  const toast = (msg,type) => { try { (window.YXHardLock?.toast || window.YX?.toast || window.toast || window.alert)(msg,type); } catch(_e){ try{ alert(msg); }catch(__){} } };
  async function api(url,opt={}){
    const res=await fetch(url,{credentials:'same-origin',cache:'no-store',...opt,headers:{'Accept':'application/json','Content-Type':'application/json','Cache-Control':'no-cache',...(opt.headers||{})}});
  ...
  function stripRowActions(scope){
    const root = scope || document;
    root.querySelectorAll('.yx-v45-row-actions,.yx113-product-actions,[data-yx131-row-action],[data-yx113-action="ship"],[data-yx113-action="delete"]').forEach(el=>el.remove());
    root.querySelectorAll('th,td').forEach(cell=>{ if((cell.textContent||'').trim()==='操作' || cell.classList.contains('yx131-action-cell')) cell.remove(); });
    root.querySelectorAll('tbody tr').forEach(tr=>{
      const last=tr.lastElementChild;
      if(last && /編輯|刪除|直接出貨|加到訂單|加到總單/.test(last.textContent||'')) last.remove();
    });
  }
  function lockActionlessTables(){
    const m=page();
    if(!['inventory','orders','master_order'].includes(m)) return;
    const screen=document.querySelector('.module-screen[data-module]');
    if(screen) screen.dataset.yxV52NoRowActions='locked';
    ['inventory','orders','master_order'].forEach(src=>{
      const box=document.getElementById('yx113-'+src+'-summary');
      if(box) stripRowActions(box);
    });
    ['inventory-inline-table','orders-inline-table','master-inline-table'].forEach(id=>{ const el=document.getElementById(id); if(el) stripRowActions(el); });
  }
  window.YX_V52_STRIP_PRODUCT_ROW_ACTIONS = lockActionlessTables;
  function qtyFromProduct(text, fallback){
    try { return window.yxEffectiveQty ? Number(window.yxEffectiveQty(text, fallback||1)) : Number(fallback||1); } catch(_e){ return Number(fallback||1); }
  }
  function renderCustomerPanel(customer, rows, source){
    customer=clean(customer); if(!customer || !['orders','master_order'].includes(source)) return;
    const panel=document.getElementById('selected-customer-items'); if(!panel) return;
    const list=(Array.isArray(rows)?rows:[]).filter(r=>clean(r.customer_name||r.customer||'')===customer);
    const total=list.reduce((s,r)=>s+(qtyFromProduct(r.product_text||'', r.qty||1)||Number(r.qty||0)||0),0);
    panel.classList.remove('hidden'); panel.style.display='';
    panel.dataset.yxV52Immediate='real-loaded';
    panel.innerHTML='<div class="customer-detail-card yx-v52-selected-customer-products"><div class="customer-detail-header"><div><div class="section-title">'+esc(customer)+'</div><div class="muted">'+total+'件 / '+list.length+'筆商品｜V52 後端真實清單</div></div></div><div class="card-list">'+(list.length?list.map(it=>{const raw=String(it.product_text||'');const ps=raw.split('=');return '<div class="deduct-card yx112-product-card"><div class="yx113-product-head"><strong class="material-text">'+esc(it.material||it.product_code||'未填材質')+'</strong><strong>'+qtyFromProduct(raw,it.qty||1)+'件</strong></div><div class="yx113-product-main"><span>'+esc(ps[0]||raw)+'</span><span>'+esc(ps.slice(1).join('=')||it.qty||'')+'</span></div><div class="small-note">'+esc(it.source|| (source==='orders'?'訂單':'總單'))+'</div></div>';}).join(''):'<div class="empty-state-card compact-empty">此客戶目前沒有商品</div>')+'</div></div>';
  }
  function forceNorth(customer, rows, source){
    customer=clean(customer); if(!customer) return;
    try{ window.__YX_SELECTED_CUSTOMER__=customer; }catch(_e){}
    const input=document.getElementById('customer-name'); if(input) input.value=customer;
    const list=Array.isArray(rows)?rows.filter(r=>clean(r.customer_name||r.customer||'')===customer):[];
    const total=list.reduce((s,r)=>s+(qtyFromProduct(r.product_text||'',r.qty||1)||0),0) || '';
    const count=list.length || '';
    const html='<button type="button" class="customer-region-card yx113-customer-card yx114-customer-card yx116-customer-card yx117-customer-card yx-v52-force-customer-card" data-customer-name="'+esc(customer)+'" data-customer="'+esc(customer)+'" data-region="北區"><span class="yx113-customer-left yx116-customer-name">'+esc(customer)+'</span><span class="yx113-customer-tag yx116-customer-tag">北區</span><span class="yx113-customer-count yx116-customer-count">'+(total? total+'件 / '+count+'筆':'已建立')+'</span></button>';
    ['region-north','customers-north'].forEach(id=>{const box=document.getElementById(id); if(!box) return; box.querySelector('.empty-state-card')?.remove(); const old=Array.from(box.querySelectorAll('[data-customer-name],[data-customer]')).find(el=>clean(el.dataset.customerName||el.dataset.customer||'')===customer); if(old) old.outerHTML=html; else box.insertAdjacentHTML('beforeend',html);});
    renderCustomerPanel(customer, list, source);
  }
  async function v52RefreshAfterWrite(m, customer, posted){
    const act=window.YX113ProductActions||window.YX132ProductActions||window.YX128ProductActions;
    let rows=[];
    try{
      const d=await api(apiPath(m)+'?ts='+Date.now());
      rows=Array.isArray(d.items)?d.items:[];
      if(act?.rowsStore) act.rowsStore(m, rows);
      act?.renderSummary?.(m); act?.renderCards?.(m);
    }catch(e){
      const snaps=posted?.snapshots||{}; rows=Array.isArray(snaps[m])?snaps[m]:(m==='master_order'&&Array.isArray(snaps.master_orders)?snaps.master_orders:(Array.isArray(posted?.items)?posted.items:[]));
      if(act?.rowsStore && Array.isArray(rows)) act.rowsStore(m, rows);
      act?.renderSummary?.(m); act?.renderCards?.(m);
    }
    lockActionlessTables();
    if(['orders','master_order'].includes(m) && customer){
      try{ await api('/api/customers/ensure',{method:'POST',body:JSON.stringify({name:customer,region:'北區',preserve_existing:true,request_key:'v52-ensure-'+Date.now()+'-'+Math.random().toString(36).slice(2)})}); }catch(_e){}
      try{ await window.YX113CustomerRegions?.loadCustomerBlocks?.(true); }catch(_e){}
      try{ window.YX113CustomerRegions?.renderFromCurrentRows?.(); }catch(_e){}
      forceNorth(customer, rows, m);
      try{ await window.YX113CustomerRegions?.selectCustomer?.(customer); }catch(_e){}
      forceNorth(customer, rows, m);
    }
  }
  function wrapConfirm(){
    if(typeof window.confirmSubmit!=='function' || window.confirmSubmit.__yxV52Wrapped) return;
    const prev=window.confirmSubmit;
    const wrapped=async function(ev){
      const m=page();
      const beforeCustomer=clean(document.getElementById('customer-name')?.value || window.__YX_SELECTED_CUSTOMER__ || '');
      let ret;
      try{ ret=await prev.call(this,ev); }
      finally{
        const customer=clean(document.getElementById('customer-name')?.value || window.__YX_SELECTED_CUSTOMER__ || beforeCustomer);
        await v52RefreshAfterWrite(m, customer, ret||{}).catch(()=>{});
        setTimeout(lockActionlessTables,0); setTimeout(lockActionlessTables,160); setTimeout(lockActionlessTables,500);
      }
      return ret;
    };
    wrapped.__yxV52Wrapped=true;
    window.confirmSubmit=wrapped;
  }
  function setup(){
    wrapConfirm(); lockActionlessTables();
    const btn=document.getElementById('submit-btn');
    if(btn){ btn.classList.add('yx-v52-submit-btn'); btn.dataset.yxV52Submit='real-loaded'; btn.removeAttribute('onclick'); }
  }
  if(document.readyState==='loading') document.addEventListener('DOMContentLoaded',setup,{once:true}); else setup();
  window.addEventListener('load',()=>{setup(); setTimeout(lockActionlessTables,300);});
  document.addEventListener('click', function(ev){
    const rowAct=ev.target?.closest?.('[data-yx131-row-action],.yx-v45-row-actions,[data-yx113-action="ship"],[data-yx113-action="delete"]');
    if(rowAct && ['inventory','orders','master_order'].includes(page())){ ev.preventDefault(); ev.stopPropagation(); ev.stopImmediatePropagation?.(); lockActionlessTables(); toast('此頁單列操作按鈕已永久移除，請使用上方批量功能。','warn'); }
  }, true);
})();


/* ===== V53 PRODUCTS REAL LOADED PERMANENT WRITE / NO ROW ACTIONS =====
   實際載入母版內部補強：送出後一定重新讀 DB、直寫表格、北區客戶卡與下方清單同步；庫存/訂單/總單單列按鈕永久移除。 */
(function(){
  'use strict';
  if(window.__YX_V53_PRODUCTS_PERMANENT_WRITE__) return;
  window.__YX_V53_PRODUCTS_PERMANENT_WRITE__ = true;
  try{ document.documentElement.dataset.yxV53ProductsWriteback='locked'; }catch(_e){}
  const clean=v=>String(v==null?'':v).trim();
  const esc=v=>String(v==null?'':v).replace(/[&<>'"]/g,ch=>({'&':'&amp;','<':'&lt;','>':'&gt;',"'":'&#39;','"':'&quot;'}[ch]));
  const page=()=>document.querySelector('.module-screen[data-module]')?.dataset.module||'';
  const apiPath=m=>m==='inventory'?'/api/inventory':m==='orders'?'/api/orders':'/api/master_orders';
  const title=m=>m==='inventory'?'庫存清單':m==='orders'?'訂單清單':'總單清單';
  const summaryId=m=>'yx113-'+m+'-summary';
  const toast=(msg,type)=>{try{(window.YXHardLock?.toast||window.YX?.toast||window.toast||window.alert)(msg,type);}catch(_e){try{alert(msg);}catch(__){}}};
  async function api(url,opt={}){const r=await fetch(url,{credentials:'same-origin',cache:'no-store',...opt,headers:{'Accept':'application/json','Content-Type':'application/json','Cache-Control':'no-cache',...(opt.headers||{})}});const t=await r.text();let d={};try{d=t?JSON.parse(t):{};}catch(_e){d={success:false,error:t||'伺服器回應格式錯誤'};}if(!r.ok||d.success===false)throw new Error(d.error||d.message||('HTTP '+r.status));return d;}
  function qty(text,fallback){const q=Number(fallback||0)||0; if(q>0) return q; try{return Number(window.yxEffectiveQty?window.yxEffectiveQty(text,0):0)||1;}catch(_e){return 1;}}
  function splitProduct(t){const raw=clean(t); const a=raw.split('='); return {size:a[0]||raw, support:a.slice(1).join('=')||''};}
  function material(r){return clean(r.material||r.product_code||'')||'未填材質';}
  function zone(r){const z=clean(r.location||r.zone||r.warehouse_zone||'').toUpperCase(); return z==='A'?'A區':z==='B'?'B區':(z||'');}
  function rowCustomer(r){return clean(r.customer_name||r.customer||'');}
  function rowsFor(m, rows){let out=Array.isArray(rows)?rows.slice():[]; if((m==='orders'||m==='master_order') && clean(window.__YX_SELECTED_CUSTOMER__||document.getElementById('customer-name')?.value||'')){const c=clean(window.__YX_SELECTED_CUSTOMER__||document.getElementById('customer-name')?.value||''); out=out.filter(r=>rowCustomer(r)===c);} return out;}
  function stripRowActions(scope){const root=scope||document; root.querySelectorAll('.yx-v45-row-actions,.yx113-product-actions,[data-yx131-row-action],[data-yx113-action="ship"],[data-yx113-action="delete"],[data-yx113-action="edit"],td.yx131-action-cell').forEach(el=>el.remove()); root.querySelectorAll('th,td').forEach(cell=>{if((cell.textContent||'').trim()==='操作'||cell.classList.contains('yx131-action-cell')) cell.remove();}); root.querySelectorAll('tbody tr').forEach(tr=>{const last=tr.lastElementChild;if(last && /編輯|刪除|直接出貨|加到訂單|加到總單/.test(last.textContent||'')) last.remove();});}
  function directRenderTable(m, rows){
    const box=document.getElementById(summaryId(m)); if(!box) return;
    const list=rowsFor(m,rows);
    const total=list.reduce((s,r)=>s+qty(r.product_text||'',r.qty||1),0);
    const customer=clean(window.__YX_SELECTED_CUSTOMER__||document.getElementById('customer-name')?.value||'');
    const body=list.length?list.map(r=>{const p=splitProduct(r.product_text||''); const id=Number(r.id||r.item_id||0)||''; return '<tr class="yx113-summary-row yx-v53-row" data-source="'+esc(m)+'" data-id="'+esc(id)+'"><td class="mat"><input class="yx113-row-check" type="checkbox" data-id="'+esc(id)+'" data-source="'+esc(m)+'"> '+esc(material(r))+'</td><td class="size">'+esc(p.size||r.product_text||'')+'</td><td class="support">'+esc(p.support||String(qty(r.product_text||'',r.qty||1)))+'</td><td class="qty total-qty">'+qty(r.product_text||'',r.qty||1)+'</td><td class="zone">'+esc(zone(r))+'</td></tr>';}).join(''):'<tr><td colspan="5">目前沒有資料</td></tr>';
    box.innerHTML='<div class="yx113-summary-head yx128-summary-head yx-v53-summary-head"><div class="yx132-summary-title">'+(customer&&m!=='inventory'?'<span class="yx132-customer-tag">'+esc(customer)+'</span>':'<span class="yx132-customer-tag">'+(m==='inventory'?'庫存':'')+'</span>')+'<strong>'+total+'件 / '+list.length+'筆</strong><span>'+esc(title(m))+'｜已從資料庫重新讀取</span></div></div><div class="yx113-table-wrap"><table class="yx113-table yx128-inline-table yx-v53-no-row-actions"><thead><tr><th>材質</th><th>尺寸</th><th>支數 x 件數</th><th>總數量</th><th>A/B區</th></tr></thead><tbody>'+body+'</tbody></table></div>';
    stripRowActions(box);
  }
  function forceCustomerPanel(customer, rows, m){
    customer=clean(customer); if(!customer||!['orders','master_order'].includes(m)) return;
    try{window.__YX_SELECTED_CUSTOMER__=customer;}catch(_e){}
    const inp=document.getElementById('customer-name'); if(inp) inp.value=customer;
    const list=(Array.isArray(rows)?rows:[]).filter(r=>rowCustomer(r)===customer);
    const total=list.reduce((s,r)=>s+qty(r.product_text||'',r.qty||1),0);
    const panel=document.getElementById('selected-customer-items');
    if(panel){panel.classList.remove('hidden');panel.style.display='';panel.dataset.yxV53Permanent='real-db';panel.innerHTML='<div class="customer-detail-card yx-v53-selected-customer-products"><div class="customer-detail-header"><div><div class="section-title">'+esc(customer)+'</div><div class="muted">'+total+'件 / '+list.length+'筆商品｜資料庫固定保存</div></div></div><div class="card-list">'+(list.length?list.map(r=>{const p=splitProduct(r.product_text||'');return '<div class="deduct-card yx112-product-card"><div class="yx113-product-head"><strong class="material-text">'+esc(material(r))+'</strong><strong>'+qty(r.product_text||'',r.qty||1)+'件</strong></div><div class="yx113-product-main"><span>'+esc(p.size||r.product_text||'')+'</span><span>'+esc(p.support||r.qty||'')+'</span></div><div class="small-note">'+(m==='orders'?'訂單':'總單')+'</div></div>';}).join(''):'<div class="empty-state-card compact-empty">此客戶目前沒有商品</div>')+'</div></div>';}
    const card='<button type="button" class="customer-region-card yx113-customer-card yx114-customer-card yx116-customer-card yx117-customer-card yx-v53-force-customer-card" data-customer-name="'+esc(customer)+'" data-customer="'+esc(customer)+'" data-region="北區"><span class="yx113-customer-left yx116-customer-name">'+esc(customer)+'</span><span class="yx113-customer-tag yx116-customer-tag">北區</span><span class="yx113-customer-count yx116-customer-count">'+total+'件 / '+list.length+'筆</span></button>';
    ['region-north','customers-north'].forEach(id=>{const box=document.getElementById(id); if(!box) return; box.querySelector('.empty-state-card')?.remove(); const old=Array.from(box.querySelectorAll('[data-customer-name],[data-customer]')).find(el=>clean(el.dataset.customerName||el.dataset.customer||'')===customer); if(old) old.outerHTML=card; else box.insertAdjacentHTML('beforeend',card);});
  }
  async function verifyAndRender(m, customer){
    if(!['inventory','orders','master_order'].includes(m)) return [];
    const d=await api(apiPath(m)+'?v53_verify=1&ts='+Date.now());
    let rows=Array.isArray(d.items)?d.items:[];
    const act=window.YX113ProductActions||window.YX132ProductActions||window.YX128ProductActions;
    try{act?.rowsStore?.(m,rows); }catch(_e){}
    try{directRenderTable(m,rows);}catch(_e){}
    try{act?.renderCards?.(m);}catch(_e){}
    stripRowActions(document);
    if(['orders','master_order'].includes(m) && clean(customer)){
      try{await api('/api/customers/ensure',{method:'POST',body:JSON.stringify({name:customer,region:'北區',preserve_existing:true,request_key:'v53-ensure-'+Date.now()+'-'+Math.random().toString(36).slice(2)})});}catch(_e){}
      try{await window.YX113CustomerRegions?.loadCustomerBlocks?.(true);}catch(_e){}
      try{window.YX113CustomerRegions?.renderFromCurrentRows?.();}catch(_e){}
      forceCustomerPanel(customer,rows,m);
      try{await window.YX113CustomerRegions?.selectCustomer?.(customer);}catch(_e){}
      forceCustomerPanel(customer,rows,m);
    }
    return rows;
  ...
  function setup(){
    const m=page(); if(!['inventory','orders','master_order'].includes(m)) return;
    const btn=document.getElementById('submit-btn'); if(btn){btn.dataset.yxV53Submit='real-loaded';btn.classList.add('yx-v53-submit-btn');btn.removeAttribute('onclick');}
    stripRowActions(document);
    verifyAndRender(m, clean(document.getElementById('customer-name')?.value||window.__YX_SELECTED_CUSTOMER__||'')).catch(()=>{});
    if(typeof window.confirmSubmit==='function' && !window.confirmSubmit.__yxV53PermanentWrite){
      const prev=window.confirmSubmit;
      const wrapped=async function(ev){
        const pm=page(); const before=clean(document.getElementById('customer-name')?.value||window.__YX_SELECTED_CUSTOMER__||'');
        let ret;
        try{ ret=await prev.call(this,ev); return ret; }
        finally{
          const after=clean(document.getElementById('customer-name')?.value||window.__YX_SELECTED_CUSTOMER__||before);
          await verifyAndRender(pm,after).catch(e=>toast('已送出，但重讀資料庫失敗：'+(e.message||e),'warn'));
          setTimeout(()=>{stripRowActions(document); verifyAndRender(pm,after).catch(()=>{});},250);
        }
      };
      wrapped.__yxV53PermanentWrite=true;
      window.confirmSubmit=wrapped;
    }
  }
  if(document.readyState==='loading') document.addEventListener('DOMContentLoaded',setup,{once:true}); else setup();
  window.addEventListener('load',()=>{setup(); setTimeout(()=>stripRowActions(document),300);});
  document.addEventListener('click',ev=>{const rowAct=ev.target?.closest?.('[data-yx131-row-action],.yx-v45-row-actions,[data-yx113-action="ship"],[data-yx113-action="delete"],[data-yx113-action="edit"]'); if(rowAct && ['inventory','orders','master_order'].includes(page())){ev.preventDefault();ev.stopPropagation();ev.stopImmediatePropagation?.();stripRowActions(document);toast('單列按鈕已永久移除，請使用上方批量功能。','warn');}},true);
  window.YX_V53_VERIFY_PRODUCTS=verifyAndRender;
})();
