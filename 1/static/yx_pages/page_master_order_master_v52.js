/* 沅興木業 FULL MASTER V52 PRODUCTS / SHIP / WAREHOUSE WRITEBACK - page_master_order_master_v52 */
(function(){ window.__YX_FULL_MASTER_V51_PAGE__='page_master_order_master_v52'; })();
...
(function(){
  ...
  function renderSummary(source){
    ...
    const emptyColspan = source === 'inventory' ? 5 : 6;
    const body = displayRows.length ? displayRows.map(r => {
      const p = splitProduct(r.product_text || '');
      const id = idOf(r);
      if (!editing) {
        const actionCell = source === 'inventory' ? '' : `<td class="yx131-action-cell">${r.__pending_server_id ? '<span class="small-note warn">寫入中，請稍候</span>' : rowActionsHTML(source, id)}</td>`;
    ...
    const tableClass = source === 'inventory' ? 'yx113-table yx128-inline-table yx-v51-inventory-no-actions' : 'yx113-table yx128-inline-table';
...
/* ===== V52 PRODUCTS REAL LOADED HTML / JS / CSS / APP WRITEBACK =====
   ...
   3) 這段寫在實際載入的 page_*_master_v52.js，不是外掛補丁。 */
(function(){
  'use strict';
