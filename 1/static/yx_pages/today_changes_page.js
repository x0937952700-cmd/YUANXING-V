...
(function(){
  ...
  async function loadTodayChanges112(opts={}){
    ...
    const cacheKey = 'today_changes_light';
    ...
    const fetchFresh = async () => {
      try {
        ...
        let data = await YX.api('/api/today-changes?yx143_final=1&force=' + (forceHeavy ? '1' : '0'), {method:'GET'}); state.forceNext=false;
  ...
  function install(){
    ...
    document.documentElement.dataset.yx112Today = 'main';
    document.documentElement.dataset.yx114Today = 'main';
    bindEvents();
    publishGlobals();
    cleanLegacyTodayDom();
