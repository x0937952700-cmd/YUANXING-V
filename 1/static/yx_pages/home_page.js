...
/* 沅興木業 FULL MASTER main REAL LOADED COMPLETE - page_home_master_main */
(function(){ window.__YX_FULL_MASTER_main_PAGE__='page_home_master_main'; })();
...
// Batch3: lightweight 今日異動 badge. Home page no longer needs the heavy today payload.
(function(){
  let __yxTodayBadgeTimer = null;
  async function refreshTodayBadge(){
    try{
      if(!window.YX || !YX.api) return;
      const d = await YX.api('/api/today-changes/count?include_unplaced=1&ts=' + Date.now(), {method:'GET', timeoutMs:9000, softCache:false});
      const targets = document.querySelectorAll('[data-today-badge], .today-badge, #today-badge, #todayChangesBadge');
      targets.forEach(el=>{
        const n = Number(d.unread || 0);
        el.textContent = n > 99 ? '99+' : String(n);
        el.style.display = n > 0 ? '' : 'none';
      });
      document.body.dataset.todayUnread = String(d.unread || 0);
      document.body.dataset.todayUnplaced = String(d.unplaced_count || 0);
      document.body.dataset.todayInbound = String(d.total || 0);
      try { window.__YX_TODAY_BADGE_SUMMARY__ = d; } catch(_e) {}
    }catch(_e){}
  }
  function refreshTodayBadgeSoon(){
    try { if(__yxTodayBadgeTimer) clearTimeout(__yxTodayBadgeTimer); } catch(_e) {}
    __yxTodayBadgeTimer = setTimeout(()=>{ __yxTodayBadgeTimer=null; refreshTodayBadge(); }, 180);
  }
  ...
  window.YXRefreshTodayBadge = refreshTodayBadge;
})();
