/* 沅興木業 FULL MASTER V50 PRODUCTS CUSTOMER LOGIN WRITEBACK - page_master_order_master_v50 */
(function(){ window.__YX_FULL_MASTER_V50_PAGE__='page_master_order_master_v50'; })();
...
(function(){
  'use strict';
  if (window.YXHardLock && window.YXHardLock.version === 'full-master-v50-products-login-customer-real-loaded-html-js-css-writeback') return;
  ...
  ensureVisualToast();
  window.YXHardLock = {
    version: 'full-master-v50-products-login-customer-real-loaded-html-js-css-writeback',
...
(function(){
  ...
  function rowActionsHTML(source, id){
    // V50：庫存清單單列「編輯 / 刪除 / 加到訂單 / 加到總單」按鈕永久移除；之後任何修復不得再加回。
  ...
  function renderSummary(source){
    ...
    // V50：庫存頁不再顯示「加到訂單 / 加到總單 / 移到A區 / 移到B區」這一排操作，避免之後又被加回。
...
(function(){
  ...
  async function finalConfirmSubmit(ev){
    ...
    try{
      ...
      const requestKey = `v50-submit-${m}-${Date.now()}-${Math.random().toString(36).slice(2)}`;
...
(function(){
  ...
  function lockGlobals(){
    ...
    window.YX113CustomerRegions = {loadCustomerBlocks, renderBoards, selectCustomer, ensureCustomerVisible, renderFromCurrentRows, moveCustomer, setHiddenCustomers, unhideCustomer};
...
(function(){
  'use strict';
  if (window.__YX_V50_PRODUCTS_GUARD__) return;
  window.__YX_V50_PRODUCTS_GUARD__ = true;
  try {
  ...
  if (typeof originalConfirm === 'function' && !originalConfirm.__yxV50Wrapped) {
    var wrapped = async function(ev){
      ...
      try {
        if (btn) { btn.dataset.yxV50Submitting = '1'; btn.classList.add('yx-v50-submitting'); }
        ...
        if (r) {
          ...
          r.innerHTML = '<strong style="color:#b91c1c">送出失敗 / 未寫入清單</strong><div class="small-note">'+esc(e && e.message ? e.message : e)+'</div><div class="small-note yx-v50-submit-debug">V50 已寫入實際載入母版；若還看到舊錯誤，請重新整理清快取。</div>';
        }
        toast((e && e.message) || '送出失敗', 'error');
      } finally {
        if (btn) { delete btn.dataset.yxV50Submitting; btn.classList.remove('yx-v50-submitting'); }
      }
    };
    wrapped.__yxV50Wrapped = true;
...
(function(){
  ...
  document.addEventListener('DOMContentLoaded', function(){
    var btn=document.getElementById('submit-btn');
    if(btn){ btn.classList.add('yx-v50-submit-btn'); btn.removeAttribute('onclick'); btn.dataset.yxV50Submit='real-loaded'; }
    ...
    if(dp){ dp.classList.add('yx-v50-duplicate-panel'); dp.dataset.yxV50DuplicatePanel='real-loaded'; }
  }, {once:true});
})();
...
(function(){
  'use strict';
  if (window.__YX_V50_PRODUCTS_FINAL_CAPTURE__) return;
  window.__YX_V50_PRODUCTS_FINAL_CAPTURE__ = true;
  try {
    document.documentElement.dataset.yxV50ProductsWriteback = 'locked';
    window.YX = window.YX || window.YXHardLock || {toast:function(msg){try{alert(msg);}catch(_e){}}, clean:function(v){return String(v==null?'':v).trim();}};
  } catch(_e) {}
  ...
  function setup(){
    ...
    var btn=document.getElementById('submit-btn');
    if(btn){
      btn.classList.add('yx-v50-submit-btn');
      btn.removeAttribute('onclick');
      btn.dataset.yxV50Submit='real-loaded';
      btn.dataset.yxRealLoadedSubmit='v50';
    }
    var dp=document.getElementById('duplicate-action-panel');
    if(dp){ dp.classList.add('yx-v50-duplicate-panel'); dp.dataset.yxV50DuplicatePanel='real-loaded'; }
    var result=document.getElementById('module-result');
    if(result) result.dataset.yxV50Result='real-loaded';
  }
  ...
  window.addEventListener('click', function(ev){
    ...
    if(btn.dataset.yxV50Submitting === '1') return;
    btn.dataset.yxV50Submitting = '1';
    var done = function(){ try { delete btn.dataset.yxV50Submitting; } catch(_e){} };
    try {
...
/* ===== V50 PRODUCTS + CUSTOMER + LOGIN REAL LOADED WRITEBACK LOCK =====
   實際寫回母版：庫存移除單列操作按鈕；訂單/總單送出後立即刷新客戶與商品清單。 */
(function(){
  'use strict';
  if (window.__YX_V50_PRODUCTS_CUSTOMER_GUARD__) return;
  window.__YX_V50_PRODUCTS_CUSTOMER_GUARD__ = true;
  try {
    document.documentElement.dataset.yxV50ProductsWriteback = 'locked';
    window.YX = window.YX || window.YXHardLock || {toast:function(msg){try{alert(msg);}catch(_e){}}, clean:function(v){return String(v==null?'':v).trim();}};
  } catch(_e) {}
  ...
  function removeInventoryRowActions(){
    try {
      ...
      if (!screen) return;
      screen.dataset.yxV50InventoryNoRowActions = 'locked';
  ...
  window.YX_V50_REMOVE_INVENTORY_ROW_ACTIONS = removeInventoryRowActions;
  ...
  function forceNorthCustomerCard(customer){
    ...
    var html = '<button type="button" class="customer-region-card yx113-customer-card yx114-customer-card yx116-customer-card yx117-customer-card yx-v50-force-customer-card" data-customer-name="'+esc(customer)+'" data-customer="'+esc(customer)+'" data-region="北區"><span class="yx113-customer-left yx116-customer-name">'+esc(customer)+'</span><span class="yx113-customer-tag yx116-customer-tag">北區</span><span class="yx113-customer-count yx116-customer-count">已建立</span></button>';
  ...
  async function ensureCustomerAndRefresh(m, customer){
    ...
    try {
      var res = await fetch('/api/customers/ensure', {method:'POST', credentials:'same-origin', headers:{'Content-Type':'application/json'}, body:JSON.stringify({name:customer, region:'北區', preserve_existing:true, request_key:'v50-ensure-'+Date.now()+'-'+Math.random().toString(36).slice(2)})});
  ...
  function setup(){
    ...
    var act = window.YX113ProductActions || window.YX132ProductActions || window.YX128ProductActions;
    if (act && !act.__yxV50InventoryNoRowActions) {
      ...
      act.__yxV50InventoryNoRowActions = true;
    }
    var btn = document.getElementById('submit-btn');
    if (btn) { btn.dataset.yxRealLoadedSubmit = 'v50'; btn.classList.add('yx-v50-submit-btn'); }
  }
  if (document.readyState === 'loading') document.addEventListener('DOMContentLoaded', setup, {once:true}); else setup();

  if (typeof window.confirmSubmit === 'function' && !window.confirmSubmit.__yxV50Wrapped) {
  ...
  try {
    var mo = new MutationObserver(function(){ removeInventoryRowActions(); });
