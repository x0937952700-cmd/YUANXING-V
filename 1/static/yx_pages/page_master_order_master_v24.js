/* 沅興木業 FULL MASTER V24 REAL LOADED FINAL AUDIT - page_master_order_master_v24 */
(function(){ window.__YX_FULL_MASTER_V24_PAGE__='page_master_order_master_v24'; })();
...
/* ===== v24 final product submit master: 實際寫入後才更新畫面，禁止 tmp 暫存列 ===== */
(function(){
  'use strict';
  if (window.__YX_V24_FINAL_PRODUCT_SUBMIT__) return;
  window.__YX_V24_FINAL_PRODUCT_SUBMIT__ = true;
  const $ = id => document.getElementById(id);
  ...
  async function refreshAfterSubmit(m, customer, posted, submittedItems, location){
    ...
    if (m === 'orders' || m === 'master_order') {
      ...
      try {
        const ensured = await api('/api/customers/ensure', {method:'POST', body:JSON.stringify({name:customer, region:'北區', preserve_existing:true, request_key:`v24-ensure-${Date.now()}-${Math.random().toString(36).slice(2)}`})});
        ...
        console.warn('[YX v24 ensure customer]', e);
        try { YX.toast('客戶已先顯示，但後端保存確認失敗：' + (e.message || e), 'warn'); } catch(_e) {}
      }
    }
  }
  ...
  async function finalConfirmSubmit(ev){
    ...
    try{
      ...
      const requestKey = `v12-submit-${m}-${Date.now()}-${Math.random().toString(36).slice(2)}`;
