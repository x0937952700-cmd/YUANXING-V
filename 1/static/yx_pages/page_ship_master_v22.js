...
(function(){
  ...
  function previewRowHtml(item,idx,preview){
    ...
    const whPlan=Array.isArray(preview.warehouse_deduct_plan)?preview.warehouse_deduct_plan:[];
    const whBad=preview.warehouse_strict_ok===false;
    const whHtml=whPlan.length
      ? `<div class="yx97-warehouse-deduct-list">${whPlan.map(w=>`<span class="yx97-warehouse-deduct-chip">${esc((w.zone||'').toString().toUpperCase())}區${Number(w.column_index||0)}欄${Number(w.slot_number||0)}格：${Number(w.before_qty||0)}→${Number(w.after_qty||0)}，扣${Number(w.deduct_qty||0)}件</span>`).join('')}<em class="yx97-warehouse-deduct-note">確認後才會同步扣倉庫圖</em></div>`
      : `<button type="button" class="yx22-location-btn" data-prod="${esc(item.product_text||preview.product_text||preview.product||'')}">${whBad?'倉庫圖不足':'查商品位置'}</button>`;
    return`<tr class="${whBad?'is-over-qty':''}"><td>${idx+1}</td><td>${esc(state.customer||item.customer||preview.customer||'')}</td><td><span class="mat-tag">${esc(item.material||preview.material||'未填材質')}</span></td><td>${esc(p.size)}${p.support?'='+esc(p.support):''}</td><td>${q}件</td><td>出貨源：${esc(shipSourceLabel(item,preview))}</td><td>${whHtml}</td><td>${before>0?`${before} → ${after}`:'待確認'}</td><td>${shortage||whBad?'<span class="danger-text">不足</span>':'可出貨'}</td></tr>`;
  }
...
/* ===== V99 MAINFILE ship warehouse shortage detail panel ===== */
(function(){'use strict';
  if(window.__YX_V99_SHIP_SHORTAGE_DETAIL__) return; window.__YX_V99_SHIP_SHORTAGE_DETAIL__=true;
  ...
  function productLines(){
    ...
    return raw.split(/\n+/).map(x=>x.trim()).filter(Boolean).map(x=>({product_text:x,qty:0}));
  }
  function currentPayload(){return {customer_name:document.getElementById('customer-name')?.value||document.getElementById('ship-customer-search')?.value||'',items:productLines(),allow_inventory_fallback:true,skip_snapshot:true};}
  function cellChip(c){const z=String(c.zone||'').toUpperCase();const col=Number(c.column_index||0);const slot=Number(c.slot_number||0);const qty=Number(c.available_qty||c.qty||0);const prod=c.product_text||'';const mat=c.material||'';const cust=c.customer_name||c.customer||'庫存';const href=`/warehouse?loc=${encodeURIComponent(`${z}-${col}-${slot}`)}&open=1&customer=${encodeURIComponent(cust)}&highlight_item=${encodeURIComponent(prod)}`;return `<a class="yx99-shortage-cell" href="${href}"><b>${esc(z)}區${col}欄${slot}格</b><span>${esc(cust)}｜${esc(mat)} ${esc(prod)}</span><strong>${qty}件</strong></a>`;}
  function rowHtml(s){const cells=Array.isArray(s.available_cells)?s.available_cells:[];const plan=Array.isArray(s.warehouse_deduct_plan)?s.warehouse_deduct_plan:[];const raw=encodeURIComponent(JSON.stringify(s||{}));return `<div class="yx99-shortage-row" data-yx101-shortage='${raw}'><div class="yx99-shortage-head"><b>${esc(s.material||'未填材質')}｜${esc(s.product_text||'')}</b><span>需要 ${Number(s.qty||0)} 件 / 目前 ${Number(s.available_qty||0)} 件 / 不足 ${Number(s.shortage_qty||0)} 件</span></div><p>${esc(s.message||'倉庫圖數量不足')}</p><div class="btn-row compact-row"><button type="button" class="ghost-btn small-btn" data-yx101-reallocate>用目前可扣數量重算</button><button type="button" class="ghost-btn small-btn" data-yx101-fill-available>把數量改成可出貨件數</button></div><div class="yx99-shortage-cells">${cells.length?cells.map(cellChip).join(''):(plan.length?plan.map(cellChip).join(''):'<em>目前倉庫圖找不到符合客戶＋材質＋尺寸的格子</em>')}</div><div class="yx101-reallocate-result"></div></div>`;}
  async function renderShortageDetail(){
    const panel=document.getElementById('ship-preview-panel')||document.getElementById('module-result');
    if(!panel||!panel.textContent.includes('倉庫')) return;
    const payload=currentPayload();
    if(!payload.customer_name||!payload.items.length) return;
    let box=document.getElementById('yx99-ship-shortage-detail');
    if(!box){box=document.createElement('div');box.id='yx99-ship-shortage-detail';box.className='yx99-shortage-panel';panel.appendChild(box);}
    box.innerHTML='<div class="yx99-shortage-loading">檢查倉庫圖目前可扣格位…</div>';
    try{
      const d=await api('/api/ship/warehouse-shortage-detail',{method:'POST',body:JSON.stringify(payload)});
      const shortages=Array.isArray(d.shortages)?d.shortages:[];
      if(!shortages.length){box.innerHTML='<div class="yx99-shortage-ok">倉庫圖扣除檢查完成，沒有額外不足明細。</div>';return;}
      box.innerHTML=`<div class="yx99-shortage-title">倉庫圖不足明細</div>${shortages.map(rowHtml).join('')}`;
    }catch(e){box.innerHTML=`<div class="error-card">倉庫不足明細查詢失敗：${esc(e.message||e)}</div>`;}
  }
  function wrapConfirm(){
    const old=window.confirmSubmit||window.YXShipConfirmSubmit;
    if(typeof old!=='function'||old.__yx99Wrapped) return false;
    const fn=async function(){const r=await old.apply(this,arguments);setTimeout(renderShortageDetail,500);setTimeout(renderShortageDetail,1200);return r;};
    fn.__yx99Wrapped=true; window.confirmSubmit=fn; window.YXShipConfirmSubmit=fn; return true;
  }
  function install(){wrapConfirm();document.addEventListener('click',e=>{const b=e.target.closest?.('.yx22-location-btn');if(b&&String(b.textContent||'').includes('不足'))setTimeout(renderShortageDetail,300);},true);}
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',install,{once:true});else install();
  window.YX99RenderShipWarehouseShortageDetail=renderShortageDetail;
})();
/* ===== END V99 ship warehouse shortage detail panel ===== */


/* ===== V101 MAINFILE ship shortage reallocation helper ===== */
(function(){'use strict';
  if(window.__YX_V101_SHIP_REALLOCATE__) return; window.__YX_V101_SHIP_REALLOCATE__=true;
  const esc=v=>String(v??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
  ...
  function customer(){return document.getElementById('customer-name')?.value||document.getElementById('ship-customer-search')?.value||'';}
  function parseRow(btn){const row=btn.closest('[data-yx101-shortage]'); if(!row) return {}; try{return JSON.parse(decodeURIComponent(row.getAttribute('data-yx101-shortage')||'%7B%7D'));}catch(_){return {};}}
  function renderAlloc(row,d){const box=row.querySelector('.yx101-reallocate-result'); if(!box)return; const cells=Array.isArray(d.allocation)?d.allocation:[]; box.innerHTML='<div class="yx101-realloc-card"><b>可出貨 '+Number(d.can_ship_qty||0)+' / 原需求 '+Number(d.requested_qty||0)+' 件</b><div class="small-note">依倉庫扣除規則：先扣數量最少格，數量一樣時由後端出貨流程隨機扣同量格。</div>'+(cells.length?cells.map(c=>`<a class="yx101-realloc-cell" href="${esc(c.url||'#')}">${esc(c.location||'格位')}｜${esc(c.material||'')} ${esc(c.product_text||'')}｜預計扣 ${Number(c.suggested_deduct_qty||0)} / 現有 ${Number(c.available_qty||0)} 件</a>`).join(''):'<em>目前沒有可扣格位</em>')+'</div>';}
  async function recalc(btn){const row=btn.closest('[data-yx101-shortage]'); const s=parseRow(btn); const payload={customer_name:customer(), item:{product_text:s.product_text, material:s.material, qty:s.qty, source:s.source_preference||s.source_table||''}}; const d=await api('/api/ship/warehouse-reallocate-preview',{method:'POST',body:JSON.stringify(payload)}); renderAlloc(row,d); return d;}
  async function fillAvailable(btn){const d=await recalc(btn); const qty=Number(d.can_ship_qty||0); const ta=document.getElementById('ocr-text'); if(!ta||!d.product_text||qty<=0){(window.toast||alert)('目前沒有可自動回填的可出貨數量','warn');return;} const lines=ta.value.split(/\n/); const key=String(d.product_text).split('=')[0].trim(); let changed=false; for(let i=0;i<lines.length;i++){if(lines[i].includes(key)){lines[i]=String(d.product_text).replace(/=(.*)$/,'')+'='+qty; changed=true; break;}} if(!changed) lines.push(String(d.product_text).replace(/=(.*)$/,'')+'='+qty); ta.value=lines.join('\n'); ta.dispatchEvent(new Event('input',{bubbles:true})); (window.toast||console.log)('已把出貨數量改成目前倉庫可扣數量','ok');}
  document.addEventListener('click',async e=>{const r=e.target.closest?.('[data-yx101-reallocate]'); const f=e.target.closest?.('[data-yx101-fill-available]'); if(!r&&!f)return; e.preventDefault(); try{if(r) await recalc(r); else await fillAvailable(f);}catch(err){(window.toast||alert)(err.message||'重算可出貨數量失敗','error');}},true);
})();
/* ===== END V101 ship shortage reallocation helper ===== */

/* ===== V102 MAINFILE ship shortage source reopen + selectable available plan ===== */
(function(){'use strict';
  if(window.__YX_V102_SHIP_STOCK_CHOICE__) return; window.__YX_V102_SHIP_STOCK_CHOICE__=true;
  const esc=v=>String(v??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
  const norm=v=>String(v||'').replace(/\s+/g,'').toLowerCase();
  function toast(m,k){try{(window.toast||console.log)(m,k||'ok')}catch(_){}}
  function qtyInputs(){return Array.from(document.querySelectorAll('input[name*="qty"],input[id*="qty"],input[placeholder*="件"],input[type="number"]')).filter(x=>!x.disabled&&!x.readOnly);}
  function setLineQty(productText,qty){const ta=document.getElementById('ocr-text'); const key=String(productText||'').split('=')[0].trim(); let changed=false; if(ta&&key){const lines=ta.value.split(/\n/); for(let i=0;i<lines.length;i++){if(norm(lines[i]).includes(norm(key))){lines[i]=key+'='+Number(qty||0);changed=true;break;}} if(!changed) lines.push(key+'='+Number(qty||0)); ta.value=lines.join('\n');ta.dispatchEvent(new Event('input',{bubbles:true}));return true;} const inp=qtyInputs()[0]; if(inp){inp.value=Number(qty||0); inp.dispatchEvent(new Event('input',{bubbles:true})); inp.dispatchEvent(new Event('change',{bubbles:true})); return true;} return false;}
  function sourceUrl(source,customer,product){const map={inventory:'/inventory',orders:'/orders',master_order:'/master_order',master_orders:'/master_order'}; const path=map[source]||'/ship'; const u=new URL(path,location.origin); if(customer)u.searchParams.set('customer',customer); if(product)u.searchParams.set('highlight_item',product); u.searchParams.set('target_row','1'); return u.pathname+'?'+u.searchParams.toString();}
  function choiceHtml(data,source,customer,product){const cells=Array.isArray(data.allocation)?data.allocation:[]; const qty=Number(data.can_ship_qty||0); return `<div class="yx102-stock-choice"><div class="yx102-stock-choice-title">目前倉庫可扣 ${qty} 件</div>${cells.map(c=>`<div class="yx102-stock-choice-row"><a href="${esc(c.url||'#')}">${esc(c.location||'格位')}｜現有 ${Number(c.available_qty||0)} 件</a><strong>預計扣 ${Number(c.suggested_deduct_qty||0)} 件</strong></div>`).join('')||'<div class="small-note">目前沒有可扣格位</div>'}<div class="btn-row compact-row"><button type="button" class="primary-btn small-btn" data-yx102-use-qty="${qty}" data-yx102-product="${esc(product)}">套用可扣數量</button><a class="ghost-btn small-btn" href="${esc(sourceUrl(source,customer,product))}">回來源頁確認</a></div><div class="yx102-stock-choice-total">規則：先扣同客戶同材質同尺寸中數量最少格；同量格由後端隨機扣。</div></div>`;}
  document.addEventListener('click',async e=>{const b=e.target.closest?.('[data-yx102-use-qty]'); if(!b)return; e.preventDefault(); const ok=setLineQty(b.dataset.yx102Product,Number(b.dataset.yx102UseQty||0)); toast(ok?'已套用目前倉庫可扣數量':'找不到可回填欄位','ok');},true);
  document.addEventListener('click',async e=>{const btn=e.target.closest?.('[data-yx101-reallocate]'); if(!btn)return; setTimeout(()=>{try{const row=btn.closest('[data-yx101-shortage]'); const result=row?.querySelector('.yx101-reallocate-result'); if(!row||!result||result.querySelector('.yx102-stock-choice'))return; const raw=JSON.parse(decodeURIComponent(row.getAttribute('data-yx101-shortage')||'%7B%7D')); const product=raw.product_text||''; const source=raw.source_preference||raw.source_table||''; const customer=document.getElementById('customer-name')?.value||document.getElementById('ship-customer-search')?.value||raw.customer_name||''; const qtyText=(result.textContent.match(/可出貨\s*(\d+)/)||[])[1]||0; const data={can_ship_qty:Number(qtyText||0),allocation:[]}; result.insertAdjacentHTML('beforeend',choiceHtml(data,source,customer,product));}catch(_){}},650);},true);
})();
/* ===== END V102 ship shortage source reopen + selectable available plan ===== */


/* ===== V103 MAINFILE ship shortage apply available qty + source jump ===== */
(function(){'use strict';
  if(window.__YX_V103_SHIP_SHORTAGE_APPLY__) return; window.__YX_V103_SHIP_SHORTAGE_APPLY__=true;
  const esc=v=>String(v??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
  ...
  function customer(){return document.getElementById('customer-name')?.value||document.getElementById('ship-customer-search')?.value||document.querySelector('[name="customer_name"]')?.value||'';}
  function rowPayload(row){let raw={};try{raw=JSON.parse(decodeURIComponent(row?.getAttribute('data-yx101-shortage')||row?.getAttribute('data-yx-shortage')||'%7B%7D'));}catch{} const txt=row?.textContent||'';return {customer_name:customer()||raw.customer_name||'', product_text:raw.product_text||((txt.match(/([A-Z]{0,4}\s*\d{2,3}\s*x\s*\d{1,3}\s*x\s*\d{1,3}[^\n\s]*)/i)||[])[1]||''), material:raw.material||'', qty:Number(raw.qty||raw.requested_qty||((txt.match(/不足\s*(\d+)/)||[])[1])||0), source_table:raw.source_preference||raw.source_table||raw.source||''};}
  function setQtyNear(row,qty){let hit=null; const inputs=Array.from(document.querySelectorAll('input[type="number"],input[name*="qty"],input[id*="qty"],input[placeholder*="件"]')); const prod=(rowPayload(row).product_text||'').replace(/\s+/g,'').toLowerCase(); if(prod){hit=inputs.find(i=>{const host=i.closest('.card,.deduct-card,tr,.ship-item,.form-row')||i.parentElement;return (host?.textContent||'').replace(/\s+/g,'').toLowerCase().includes(prod);});} hit=hit||inputs[0]; if(hit){hit.value=String(qty||0); hit.dispatchEvent(new Event('input',{bubbles:true})); hit.dispatchEvent(new Event('change',{bubbles:true})); return true;} return false;}
  function renderPlan(row,d){let box=row.querySelector('.yx103-apply-plan'); if(!box){box=document.createElement('div');box.className='yx103-apply-plan';row.appendChild(box);} const alloc=d.allocation||[]; box.innerHTML=`<div class="yx103-apply-head">目前倉庫可扣 <b>${esc(d.apply_qty||0)}</b> 件 / 原需求 ${esc(d.requested_qty||0)} 件</div><div class="yx103-alloc-list">${alloc.map(a=>`<a class="yx103-alloc-chip" href="${esc(a.url||'#')}">${esc(a.location||'倉庫格')} 扣 ${esc(a.deduct_qty||0)} 件</a>`).join('')||'<span class="muted">沒有可扣格位</span>'}</div><div class="btn-row"><button type="button" class="primary-btn small-btn" data-yx103-apply-qty="${esc(d.apply_qty||0)}">套用可扣數量</button>${d.source_url?`<a class="ghost-btn small-btn" href="${esc(d.source_url)}">開來源確認</a>`:''}</div>`;}
  document.addEventListener('click',async e=>{const b=e.target.closest?.('[data-yx103-check-available],.yx102-stock-choice [data-yx102-apply-available]'); if(!b)return; const row=b.closest('[data-yx101-shortage],.yx101-shortage-card,.deduct-card,.ship-shortage-card')||b.closest('.card'); if(!row)return; e.preventDefault(); try{const d=await api('/api/ship/shortage-apply-plan',rowPayload(row)); renderPlan(row,d);}catch(err){(window.toast||alert)(err.message||'重算可扣數量失敗','error');}},true);
  document.addEventListener('click',e=>{const b=e.target.closest?.('[data-yx103-apply-qty]'); if(!b)return; const row=b.closest('.yx103-apply-plan')?.parentElement||b.closest('.card'); const ok=setQtyNear(row,Number(b.dataset.yx103ApplyQty||0)); (window.toast||console.log)(ok?'已套用目前倉庫可扣數量':'找不到可回填數量欄位', ok?'ok':'warn');},true);
  function decorate(){document.querySelectorAll('[data-yx101-shortage],.ship-shortage-card,.yx101-shortage-card').forEach(row=>{if(row.dataset.yx103Decorated)return; row.dataset.yx103Decorated='1'; const btn=document.createElement('button');btn.type='button';btn.className='ghost-btn small-btn';btn.dataset.yx103CheckAvailable='1';btn.textContent='重算可扣數量'; (row.querySelector('.btn-row,.actions')||row).appendChild(btn);});}
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',()=>{decorate();setTimeout(decorate,700);},{once:true});else{decorate();setTimeout(decorate,700);} document.addEventListener('yx:ship-shortage-rendered',decorate);
})();
/* ===== END V103 ship shortage apply ===== */
