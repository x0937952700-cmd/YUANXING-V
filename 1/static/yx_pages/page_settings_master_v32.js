/* 沅興木業 FULL MASTER V32 REAL LOADED WRITEBACK - page_settings_master_v32 */
(function(){ window.__YX_FULL_MASTER_V32_PAGE__='page_settings_master_v32'; })();
...
(function(){
  ...
  window.YXHardLock = {
    version: 'full-master-v32-real-loaded-html-js-css-writeback',
...
(function(){
  ...
  window.changePassword=async function(){const old_password=document.getElementById('old-password')?.value||'';const new_password=document.getElementById('new-password')?.value||'';const confirm_password=document.getElementById('confirm-password')?.value||'';if(!new_password||new_password!==confirm_password)return msg('settings-msg','新密碼與確認密碼不一致','error');try{await api('/api/change_password',{method:'POST',body:JSON.stringify({old_password,new_password,confirm_password})});msg('settings-msg','密碼已更新','ok');}catch(e){msg('settings-msg',e.message,'error');}};
  window.logout=async function(){try{await api('/api/logout',{method:'POST',body:'{}'});}catch(_){} location.href='/login';};
  window.undoLastAction=async function(){const el=document.getElementById('undo-msg');try{const d=await api('/api/undo-last',{method:'POST',body:'{}'});if(el)el.textContent=d.message||'已還原上一筆';}catch(e){if(el)el.textContent=e.message||'目前沒有可還原資料';}};
  window.downloadReport=function(type){const start_date=document.getElementById('report-start')?.value||'';const end_date=document.getElementById('report-end')?.value||'';const q=new URLSearchParams({type,start_date,end_date});location.href='/api/reports/export?'+q.toString();};
  window.loadAuditTrails=async function(){const box=document.getElementById('audit-trails-list');if(!box)return;box.innerHTML='<div class="empty-state-card compact-empty">載入中…</div>';try{const q=new URLSearchParams({q:document.getElementById('audit-q')?.value||'',username:document.getElementById('audit-user')?.value||'',entity_type:document.getElementById('audit-entity')?.value||'',start_date:document.getElementById('audit-start')?.value||'',end_date:document.getElementById('audit-end')?.value||''});const d=await api('/api/audit-trails?'+q.toString());const rows=d.items||d.logs||[];box.innerHTML=rows.length?rows.map(r=>`<div class="deduct-card yx28-audit-row"><strong>${esc(r.username||r.user||'')}</strong><div>${esc(r.action_label||r.action||r.message||r.summary_text||'')}</div><div class="small-note">${esc(r.entity_label||r.entity_type||r.entity||r.table_name||'')}｜${esc(r.created_at||r.time||'')}</div>${r.summary_text?`<div class="small-note">${esc(r.summary_text)}</div>`:''}</div>`).join(''):'<div class="empty-state-card compact-empty">沒有差異紀錄</div>'; }catch(e){box.innerHTML='<div class="empty-state-card compact-empty">'+esc(e.message)+'</div>';}};
  window.loadAdminUsers=async function(){const box=document.getElementById('admin-users');if(!box)return;box.innerHTML='載入中…';try{const d=await api('/api/admin/users');const rows=d.items||d.users||[];box.innerHTML=`<table><thead><tr><th>帳號</th><th>狀態</th><th>操作</th></tr></thead><tbody>${rows.map(u=>{const name=esc(u.username||u.name||'');const blocked=Number(u.is_blocked??u.blocked??0)===1||u.blocked===true;return `<tr><td>${name}</td><td>${blocked?'黑名單':'正常'}</td><td><button class="ghost-btn small-btn ${blocked?'':'danger-btn'}" data-block-user="${name}" data-block-next="${blocked?'0':'1'}">${blocked?'解除黑名單':'加入黑名單'}</button></td></tr>`;}).join('')}</tbody></table>`;}catch(e){box.textContent=e.message;}};
  ...
  async function loadBackupsManual(){const box=document.getElementById('backup-panel');if(!box)return;box.textContent='載入中…';try{const d=await api('/api/backups');const rows=d.files||d.items||d.backups||[];box.innerHTML=rows.length?rows.map(b=>{const filename=String(b.filename||b.name||'');const canRestore=/\.json$/i.test(filename);return `<div class="deduct-card yx26-backup-row yx28-backup-row"><div class="yx28-backup-main"><strong>${esc(filename)}</strong><div class="small-note">${esc(b.created_at||b.time||'')}｜${b.size?Math.ceil(Number(b.size||0)/1024)+' KB':''}${canRestore?'':'｜SQLite .db 僅支援下載'}</div></div><div class="btn-row compact yx28-backup-actions"><button class="ghost-btn small-btn" type="button" data-backup-download="${esc(filename)}">下載</button>${canRestore?`<button class="ghost-btn small-btn danger-btn" type="button" data-backup-restore="${esc(filename)}">還原</button>`:''}</div></div>`;}).join(''):'尚無備份，按「立即備份」才會建立。';}catch(e){box.textContent=e.message;}}window.loadBackupsManual=loadBackupsManual;
  window.restoreBackup=async function(filename){filename=String(filename||'').trim();if(!filename)return;const ok=confirm('確定要還原這份備份？還原前建議先下載目前備份。\n\n'+filename);if(!ok)return;const box=document.getElementById('backup-panel');if(box)box.insertAdjacentHTML('afterbegin','<div class="small-note">還原中…</div>');try{await api('/api/backups/restore',{method:'POST',body:JSON.stringify({filename})});alert('備份已還原，請重新整理頁面確認資料。');}catch(e){alert(e.message||'還原失敗');}};
  document.addEventListener('click',async e=>{const dl=e.target.closest('[data-backup-download]');if(dl){e.preventDefault();const filename=dl.getAttribute('data-backup-download')||'';if(filename) location.href='/api/backups/download/'+encodeURIComponent(filename);return;}const rs=e.target.closest('[data-backup-restore]');if(rs){e.preventDefault();await window.restoreBackup(rs.getAttribute('data-backup-restore')||'');return;}},true);
  document.addEventListener('click',async e=>{const b=e.target.closest('[data-block-user]');if(!b)return;try{await api('/api/admin/block',{method:'POST',body:JSON.stringify({username:b.dataset.blockUser,blocked:b.dataset.blockNext==='1'})});loadAdminUsers();}catch(err){alert(err.message);}},true);
...
/* V26 SETTINGS ENDPOINT LOCK: password confirm_password, reports start_date/end_date, backup files list, admin block toggle. */
