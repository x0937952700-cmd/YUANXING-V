  ...
  async function loadSource(source, opts={}){
    ...
    const fetchFresh = async () => {
      ...
      try {
        ...
        const d = await YX.api(endpoint(source) + '?yx143_final=1&fast=1&limit=' + encodeURIComponent(limit), {method:'GET'});
...
(function(){
  ...
  async function refreshAfterSubmit(m, customer, posted, submittedItems, location){
    ...
    if (posted && posted.fast_response) {
      try {
...
(function(){
  ...
  async function loadCustomerBlocks(force=true){
    ...
    try {
      const cached = !force && window.YX?.cache?.read('customer_blocks_' + moduleKey(), 1000*60*60*12);
      if (cached && Array.isArray(cached.items)) { state.items = cached.items; renderBoards(state.items); }
    } catch(_e) {}
    try {
      const d = await YX.api('/api/customers?yx114=1&fast=1', {method:'GET'});
      ...
      try { window.YX?.cache?.write('customer_blocks_' + moduleKey(), {items:state.items}); } catch(_e) {}
