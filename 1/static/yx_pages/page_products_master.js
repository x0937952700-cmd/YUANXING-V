...
(function(){
  ...
  function showDataStatus(source, message, kind){
    const box = ensureSummary(source) || listEl(source);
    if(!box) return;
    let node = document.getElementById('yx-data-status-' + source);
    if(!node){
      node = document.createElement('div');
      node.id = 'yx-data-status-' + source;
      node.className = 'yx-data-status-card';
      box.insertAdjacentElement('beforebegin', node);
    }
    if(!message){ node.remove(); return; }
    node.className = 'yx-data-status-card ' + (kind || 'warn');
    node.innerHTML = `<strong>${YX.esc(title(source))}讀取提醒</strong><span>${YX.esc(message)}</span><button class="ghost-btn tiny-btn" type="button" data-yx-data-retry="${YX.esc(source)}">重新讀取</button>`;
  }
  ...
  async function recoverDbOnce(){
    try { await YX.api('/api/health/db-init?ts=' + Date.now(), {method:'GET'}); return true; }
  ...
  async function loadSource(source, opts={}){
    ...
    try {
      ...
      if(!opts.__dbRecovered){
        const ok = await recoverDbOnce();
        if(ok) return loadSource(source, {...opts, __dbRecovered:true, force:true});
      }
      ensureBatchToolbar(source);
      ensureSummary(source);
      const cachedRows = rowsStore(source);
      renderSummary(source);
      renderCards(source);
      const msg = e && e.message ? e.message : `${title(source)}讀取失敗`;
      showDataStatus(source, msg + '。已保留畫面，不會把現有資料假裝成空白；已嘗試自動復原 DB，可按重新讀取再試。', 'bad');
      try { window.dispatchEvent(new CustomEvent('yx:product-source-load-failed', {detail:{source, error:msg}})); } catch(_e) {}
      throw e;
    } finally {
  ...
  document.addEventListener('click', function(ev){
    const btn = ev.target && ev.target.closest && ev.target.closest('[data-yx-data-retry]');
    if(!btn) return;
    ev.preventDefault();
    loadSource(btn.dataset.yxDataRetry || sourceFromModule(), {force:true}).catch(e => YX.toast(e.message || '重新讀取失敗', 'error'));
  }, true);
