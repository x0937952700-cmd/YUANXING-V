/* 沅興木業 FULL MASTER V41 SHIP WAREHOUSE REAL LOADED WRITEBACK - page_ship_master_v41 */
(function(){ window.__YX_FULL_MASTER_V41_PAGE__='page_ship_master_v41'; })();
...
(function(){
  'use strict';
  if (window.YXHardLock && window.YXHardLock.version === 'full-master-v32-real-loaded-html-js-css-writeback') return;
...
(function(){
  ...
  const $=(id)=>document.getElementById(id);
  const state={customer:'',items:[],selected:[],customers:[],loadingName:'',itemCache:new Map(),bound:false,requestSeq:0,previewSeq:0};
  ...
  function previewRowHtml(item,idx,preview){const p=splitProduct(item.product_text||preview.product_text||preview.product||'');const q=selectedQtyOf(item)||Number(preview.qty||preview.need_qty||1);const candidates=[preview.before_qty,preview.available_before,preview.master_available,preview.order_available,preview.inventory_available,item.available_qty,item.original_qty];let before=0;for(const v of candidates){const n=Number(v);if(Number.isFinite(n)&&n>0){before=n;break;}}const shortage=Number(preview.shortage||0)>0||(before>0&&q>before);const after=before>0?Math.max(0,before-q):'';const loc=preview.location||preview.warehouse_location||preview.slot||'商品位置';return`<tr><td>${idx+1}</td><td>${esc(state.customer||item.customer||preview.customer||'')}</td><td><span class="mat-tag">${esc(item.material||preview.material||'未填材質')}</span></td><td>${esc(p.size)}${p.support?'='+esc(p.support):''}</td><td>${q}件</td><td><button type="button" class="yx22-location-btn" data-prod="${esc(item.product_text||preview.product_text||preview.product||'')}">${esc(loc)}</button></td><td>${before>0?`${before} → ${after}`:'待確認'}</td><td>${shortage?'<span class="danger-text">不足</span>':'可出貨'}</td></tr>`;}
  ...
  async function locateSelectedProduct(productText){const q=clean([state.customer,productText].filter(Boolean).join(' '));if(!q)return toast('缺少倉庫搜尋關鍵字','warn');try{const d=await api('/api/warehouse/search?q='+encodeURIComponent(q)+'&ts='+Date.now());const hits=Array.isArray(d.items)?d.items:[];if(!hits.length)return toast('倉庫圖找不到這筆商品位置','warn');const msg=hits.slice(0,5).map(h=>{const c=h.cell||h;return `${c.zone}-${Number(c.column_index)}-${Number(c.slot_number)}`;}).join('、');toast('倉庫位置：'+msg,'ok');}catch(e){toast(e.message||'查詢倉庫位置失敗','error');}}
  ...
  function bind(){if(state.bound)return;state.bound=true;document.addEventListener('click',(e)=>{const c=e.target.closest('[data-ship-customer]');if(c){e.preventDefault();loadItems(c.dataset.shipCustomer).catch(err=>toast(err.message,'error'));return;}const add=e.target.closest('[data-ship-add-index]');if(add){e.preventDefault();addItem(add.dataset.shipAddIndex);return;}const rm=e.target.closest('[data-selected-remove]');if(rm){e.preventDefault();state.selected.splice(Number(rm.dataset.selectedRemove),1);state.previewSeq++;const panel=$('ship-preview-panel')||$('module-result');if(panel)panel.classList.add('hidden');renderSelected();return;}const loc=e.target.closest('.yx22-location-btn');if(loc){e.preventDefault();locateSelectedProduct(loc.dataset.prod||'');return;}},true);document.addEventListener('keydown',(e)=>{if((e.target.id==='customer-name'||e.target.id==='ship-customer-search')&&e.key==='Enter'){e.preventDefault();loadItems(state.customer,{force:true}).catch(err=>toast(err.message,'error'));}},true);document.addEventListener('change',(e)=>{if(e.target.id==='ship-customer-item-select'&&e.target.value!==''){addItem(e.target.value);e.target.value='';}},true);document.addEventListener('input',(e)=>{if(e.target.id==='customer-name'||e.target.id==='ship-customer-search'){setCustomer(e.target.value);renderCustomers();renderItems();}if(e.target.matches('[data-support-editor]'))updateSelectedProductFromSupport(Number(e.target.dataset.supportEditor),e.target.value);},true);}
  function install(){document.documentElement.dataset.yxShipSingle='locked-one-line-html-v41-race-location';bind();loadCustomers();window.addEventListener('yx:customers-loaded',e=>{state.customers=Array.isArray(e.detail?.items)?e.detail.items:state.customers;},false);window.addEventListener('yx:customer-selected',e=>{const name=clean(e.detail?.name||'');if(name)loadItems(name,{force:true}).catch(err=>toast(err.message,'error'));},false);renderSelected();const name=clean($('customer-name')?.value||'');if(name)loadItems(name).catch(()=>{});else renderItems();window.confirmSubmit=confirmSubmit;}
