...
(function(){
  ...
  async function loadCustomerBlocks(force=true){
    ...
    try {
      const d = await YX.api('/api/customers?yx114=1&fast=1&light=1', {method:'GET'});
  ...
  async function editCustomer(name){
    ...
    YX.toast('客戶已更新', 'ok'); await loadCustomerBlocks(false); await selectCustomer(cleanName);
  }
  ...
  async function deleteCustomer(name){
    ...
    await loadCustomerBlocks(false);
...
(function(){
  ...
  async function loadCustomers(){let hadCached=false;try{const cached=window.YX?.cache?.read('ship_customers',1000*60*60*24*3); if(Array.isArray(cached?.items)){hadCached=true;state.customers=cached.items; renderCustomers();}}catch(_e){} const fresh=async()=>{try{const d=await api('/api/customers?ship_single=1&light=1&fast=1');state.customers=Array.isArray(d.items)?d.items:(Array.isArray(d.customers)?d.customers:[]); try{window.YX?.cache?.write('ship_customers',{items:state.customers});}catch(_e){} try{ if(window.YX113CustomerRegions&&window.YX113CustomerRegions.loadCustomerBlocks) window.YX113CustomerRegions.loadCustomerBlocks(false); }catch(_e){} renderCustomers();}catch(_){}}; if(hadCached){try{(window.YX?.scheduler?.idle||setTimeout)(fresh,1500);}catch(_e){fresh();}return state.customers;} return fresh(); }
  ...
  async function loadItems(name,opts={}){setCustomer(name||state.customer);renderItems();if(!state.customer)return;const key=state.customer;const cacheKey='ship_items_'+key;const cached=state.itemCache.get(key)||window.YX?.cache?.read(cacheKey,1000*60*60*24*3);let hadCached=false;if(!opts.force&&cached&&Array.isArray(cached.items)){hadCached=true;state.items=cached.items;state.itemCache.set(key,{items:state.items,at:Date.now()});renderItems();}if(state.loadingName===key)return;const fresh=async()=>{state.loadingName=key;if(!hadCached)renderItems();try{const d=await api('/api/customer-items?name='+encodeURIComponent(state.customer)+'&fast=1&ship_single=1'+variantsQuery());state.items=Array.isArray(d.items)?d.items:[];const saved={items:state.items,at:Date.now()};state.itemCache.set(key,saved);try{window.YX?.cache?.write(cacheKey,saved);}catch(_e){}renderItems();}finally{state.loadingName='';renderItems();}}; if(hadCached&&!opts.force){try{(window.YX?.scheduler?.idle||setTimeout)(fresh,1200);}catch(_e){fresh();}return;} return fresh();}
