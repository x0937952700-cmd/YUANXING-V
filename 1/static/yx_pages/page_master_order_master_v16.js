/* 沅興木業 FULL MASTER V12 REAL LOADED - page_master_order_master_v12 */
(function(){ window.__YX_FULL_MASTER_V12_PAGE__='page_master_order_master_v12'; })();
...
(function(){
  ...
  async function refreshCustomerBoardsSafe(customer){
    ...
    } catch(e) { console.warn('[YX v16 load customer boards]', e); }
    try {
      if (customer && window.YX113CustomerRegions?.ensureCustomerVisible) {
        await window.YX113CustomerRegions.ensureCustomerVisible(customer, '北區', true);
      }
...
(function(){
  ...
  function relationCustomersFromRows(existingItems){
    ...
    const add = (name, source, row) => {
      ...
      const region = normRegion(old.region || row.region || row.customer_region || '北區');
  ...
  function lockGlobals(){
    ...
    window.YX113CustomerRegions = {loadCustomerBlocks, renderBoards, selectCustomer, ensureCustomerVisible};
