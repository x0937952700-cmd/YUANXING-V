/* 沅興木業 FULL MASTER V52 HOME WRITEBACK - page_home_master_v53 */
(function(){ window.__YX_FULL_MASTER_V32_PAGE__='page_home_master_v53'; })();
...
/* ===== V52 HOME NO LOGOUT: old logout binding removed from actual loaded JS ===== */



/* ===== V52 HOME WRITEBACK: remove logout from home top actions ===== */
(function(){
  'use strict';
  if(window.__YX_V52_HOME_NO_LOGOUT__) return;
  window.__YX_V52_HOME_NO_LOGOUT__=true;
  function setup(){
    try{ document.documentElement.dataset.yxV52HomeNoLogout='locked'; }catch(_e){}
    document.querySelectorAll('#home-logout-btn,.home-logout-btn').forEach(el=>el.remove());
    var row=document.querySelector('.home-top-actions'); if(row){ row.classList.add('yx-v52-centered-home-actions'); }
  }
