...
/* 沅興木業 FULL MASTER main REAL LOADED COMPLETE - page_shipping_query_master_main */
(function(){ window.__YX_FULL_MASTER_main_PAGE__='page_shipping_query_master_main'; })();
...
(function(){'use strict';
  ...
  function detailTable(g){return `<div class="yx-ship-query-detail hidden yx-v63-ship-detail-list">${g.items.map(r=>{const src=r.source_summary||r.source_label||'';const note=r.note||'';return `<div class="yx-v63-ship-detail-row" data-ship-record-id="${Number(r.id||0)}"><span>${esc(r.product_text||'')}</span><b>${qty(r)}件</b><small>${esc(timeOf(r))}${src?'｜來源：'+esc(src):''}${note?'｜'+esc(note):''}</small>${Number(r.id||0)?`<button type="button" class="ghost-btn tiny-btn danger-btn" data-delete-ship-record="${Number(r.id||0)}">刪除</button>`:''}</div>`}).join('')}</div>`}
...
/* V337: open shipping query from operation status card shortcut and mark viewed. No renderer/interval/observer. */
(function(){
  'use strict';
  if(window.__YX_V337_SHIPPING_QUERY_SHORTCUT__) return;
  window.__YX_V337_SHIPPING_QUERY_SHORTCUT__=true;
  function clean(v){ return String(v == null ? '' : v).replace(/\s+/g,' ').trim(); }
  function moduleKey(){ return clean(document.body?.dataset?.module || document.querySelector('.module-screen[data-module]')?.dataset?.module || ''); }
  function toast(msg,kind='ok'){ try{ (window.toast||window.showToast||window.notify||console.log)(msg,kind); }catch(_e){} }
  function readIntent(){ try{ let raw=localStorage.getItem('yx_status_open_shipping_query_v337'); let key='yx_status_open_shipping_query_v337'; if(!raw){ raw=localStorage.getItem('yx_status_open_shipping_query_v332'); key='yx_status_open_shipping_query_v332'; } if(!raw){ raw=localStorage.getItem('yx_status_open_shipping_query_v327'); key='yx_status_open_shipping_query_v327'; } if(!raw) return null; const d=JSON.parse(raw||'{}'); localStorage.removeItem(key); return d&&typeof d==='object'?d:null; }catch(_e){ return null; } }
  function highlight(q, recordId){
    q=clean(q).toLowerCase(); recordId=clean(recordId);
    const rows=Array.from(document.querySelectorAll('[data-ship-record-id]'));
    let hit=null;
    if(recordId) hit=rows.find(el=>clean(el.getAttribute('data-ship-record-id'))===recordId);
    if(!hit && q) hit=rows.find(el=>clean(el.textContent).toLowerCase().includes(q));
    if(!hit) return;
    const card=hit.closest('.yx-ship-query-card');
    const detail=card?.querySelector?.('.yx-ship-query-detail');
    if(detail) detail.classList.remove('hidden');
    hit.classList.add('yx332-status-shortcut-hit','yx332-status-target-highlight');
    if(card) card.classList.add('yx332-status-target-highlight');
    hit.scrollIntoView?.({behavior:'smooth',block:'center'});
    setTimeout(()=>{ try{ hit.classList.remove('yx332-status-shortcut-hit','yx332-status-target-highlight'); if(card) card.classList.remove('yx332-status-target-highlight'); }catch(_e){} }, 3200);
  }
  async function applyIntent(){
    if(moduleKey()!=='shipping_query') return;
    const intent=readIntent();
    if(!intent) return;
    const q=clean(intent.q || intent.customer || intent.product || '');
    try{ const input=document.getElementById('ship-keyword'); if(input && q){ input.value=q; input.dispatchEvent(new Event('input',{bubbles:true})); } }catch(_e){}
    try{ if(typeof window.loadShippingRecords==='function') await window.loadShippingRecords(); }catch(e){ toast(e.message||'打開出貨紀錄失敗','error'); }
    setTimeout(()=>highlight(q, intent.record_id), 250);
  }
  if(document.readyState==='loading') document.addEventListener('DOMContentLoaded', applyIntent, {once:true});
  else applyIntent();
})();
