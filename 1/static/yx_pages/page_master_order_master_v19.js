/* 沅興木業 FULL MASTER V19 CLEAN REAL FIX - page_master_order_master_v19 */
(function(){ window.__YX_FULL_MASTER_V19_PAGE__='page_master_order_master_v19'; })();
...
/* ===== V2 MERGED FROM static/yx_pages/page_products_master.js ===== */
...
(function(){
  ...
  async function refreshCustomerBoardsSafe(customer){
    ...
    try {
      if (customer && window.YX113CustomerRegions?.ensureCustomerVisible) {
        await window.YX113CustomerRegions.ensureCustomerVisible(customer, '北區', true);
      }
    } catch(e) { console.warn('[YX v16 ensure customer before reload]', e); }
    try {
      if (window.YX113CustomerRegions?.loadCustomerBlocks) {
        await window.YX113CustomerRegions.loadCustomerBlocks(true);
      } else if (typeof window.loadCustomerBlocks === 'function') {
        await window.loadCustomerBlocks(true);
      } else if (typeof window.renderCustomers === 'function') {
        await window.renderCustomers(true);
      }
    } catch(e) { console.warn('[YX v17 load customer boards]', e); }
    try { window.YX113CustomerRegions?.renderFromCurrentRows?.(); } catch(_e) {}
    try {
      if (customer && window.YX113CustomerRegions?.ensureCustomerVisible) {
        await window.YX113CustomerRegions.ensureCustomerVisible(customer, '北區', true);
      }
    } catch(e) { console.warn('[YX v16 ensure customer after reload]', e); }
    try { if (customer) window.__YX_FORCE_CUSTOMER_CARD_VISIBLE__?.(customer, page()); } catch(e) { console.warn('[YX v16 force customer card]', e); }
    try { if (customer && window.YX113CustomerRegions?.selectCustomer) await window.YX113CustomerRegions.selectCustomer(customer); } catch(e) { console.warn('[YX v16 select customer]', e); }
  }

  async function refreshAfterSubmit(m, customer, posted, submittedItems, location){
    try {
      if (customer && (m === 'orders' || m === 'master_order')) {
        await api('/api/customers/ensure', {method:'POST', body:JSON.stringify({name:customer, region:'北區', preserve_existing:true, request_key:`v18-ensure-${Date.now()}-${Math.random().toString(36).slice(2)}`})});
      }
    } catch(e) { console.warn('[YX v18 ensure customer]', e); }
    try { if (customer) window.__YX_SELECTED_CUSTOMER__ = customer; } catch(_e) {}
    try { if (customer && $('customer-name')) $('customer-name').value = customer; } catch(_e) {}
    const act = window.YX113ProductActions || window.YX132ProductActions || window.YX128ProductActions;
    ...
    if (m === 'orders' || m === 'master_order') { await refreshCustomerBoardsSafe(customer); forceCustomerCardVisible(customer, m); }
  }
...
/* ===== END static/yx_pages/page_products_master.js ===== */
