/* 沅興木業 FULL MASTER V53 PRODUCTS / SHIP / WAREHOUSE FIXED20 WRITEBACK - page_ship_master_v53 */
(function(){ window.__YX_FULL_MASTER_V53_PAGE__='page_ship_master_v53'; })();
...
(function(){
  ...
  const $=(id)=>document.getElementById(id);
  const state={customer:'',items:[],selected:[],customers:[],loadingName:'',itemCache:new Map(),bound:false,requestSeq:0,previewSeq:0,locSeq:0};
  ...
  function hidePreview(){const panel=$('ship-preview-panel')||$('module-result');if(panel){panel.classList.add('hidden');panel.style.display='none';}const loc=$('ship-warehouse-link-panel');if(loc){loc.classList.add('hidden');} }
  function resetSelectedForCustomerChange(prev,next){if(prev&&next&&prev!==next){state.selected=[];state.items=[];state.loadingName='';state.previewSeq++;hidePreview();renderSelected();}}
  function setCustomer(name){const next=clean(name);const prev=state.customer;state.customer=next;resetSelectedForCustomerChange(prev,next);const a=$('customer-name');if(a&&a.value!==state.customer)a.value=state.customer;const b=$('ship-customer-search');if(b&&b.value!==state.customer)b.value=state.customer;}
  ...
  function previewLocationLabel(preview){const locs=Array.isArray(preview?.locations)?preview.locations:[];if(locs.length){const first=locs[0]||{};const label=`${first.zone||''}-${Number(first.column_index||0)}-${Number(first.slot_number||first.visual_slot||0)}`;return locs.length>1?`${label} 等 ${locs.length} 格`:label;}return preview.location||preview.warehouse_location||preview.slot||'查倉庫位置';}
  function previewRowHtml(item,idx,preview){const p=splitProduct(item.product_text||preview.product_text||preview.product||'');const q=selectedQtyOf(item)||Number(preview.qty||preview.need_qty||1);const candidates=[preview.before_qty,preview.available_before,preview.selected_available,preview.master_available,preview.order_available,preview.inventory_available,item.available_qty,item.original_qty];let before=0;for(const v of candidates){const n=Number(v);if(Number.isFinite(n)&&n>0){before=n;break;}}const shortage=Number(preview.shortage||preview.shortage_qty||0)>0||(before>0&&q>before);const after=before>0?Math.max(0,before-q):'';const loc=previewLocationLabel(preview);const prod=item.product_text||preview.product_text||preview.product||'';return`<tr><td>${idx+1}</td><td>${esc(state.customer||item.customer||preview.customer||'')}</td><td><span class="mat-tag">${esc(item.material||preview.material||'未填材質')}</span></td><td>${esc(p.size)}${p.support?'='+esc(p.support):''}</td><td>${q}件</td><td><button type="button" class="yx22-location-btn" data-prod="${esc(prod)}" data-ship-loc-query="${esc([state.customer,prod].filter(Boolean).join(' '))}">${esc(loc)}</button></td><td>${before>0?`${before} → ${after}`:'待確認'}</td><td>${shortage?'<span class="danger-text">不足</span>':'可出貨'}</td></tr>`;}
  async function confirmSubmit(){if(!state.customer)return toast('請先輸入客戶名稱','warn');if(!state.selected.length)return toast('請先加入出貨商品','warn');const overIdx=state.selected.findIndex(x=>warnOverQty(x));if(overIdx>=0){const o=warnOverQty(state.selected[overIdx]);renderSelected();toast(`第 ${overIdx+1} 筆出貨 ${o.q} 件大於可出貨 ${o.max} 件，不可扣除`,'error');document.querySelector(`[data-selected-card="${overIdx}"]`)?.scrollIntoView?.({behavior:'smooth',block:'center'});return;}const seq=++state.previewSeq;const btn=$('submit-btn');if(btn){btn.disabled=true;btn.textContent='預覽中…';}const payload={customer_name:state.customer,items:state.selected.map(x=>({...x})),allow_inventory_fallback:true,skip_snapshot:true,request_key:'ship_single_'+Date.now()+'_'+Math.random().toString(36).slice(2)};try{const preview=await api('/api/ship-preview',{method:'POST',body:JSON.stringify(payload)});if(seq!==state.previewSeq)return;showPreview(preview,payload);}catch(e){toast(e.message||'出貨預覽失敗','error');}finally{if(seq===state.previewSeq&&btn){btn.disabled=false;btn.textContent='確認送出';}}}
  ...
  function warehouseSearchQueries(productText){const prod=clean(productText||'');const customer=clean(state.customer||'');const p=splitProduct(prod);const size=clean(p.size||prod);return [...new Set([customer&&prod?`${customer} ${prod}`:'', prod, customer&&size?`${customer} ${size}`:'', size].filter(Boolean))];}
  function openWarehouseWithQuery(q){try{localStorage.setItem('yxWarehousePendingSearch',JSON.stringify({q,ts:Date.now(),from:'ship'}));}catch(_){} location.href='/warehouse?warehouse_q='+encodeURIComponent(q);}
  function showWarehouseHits(productText,hits,q){const panel=$('ship-warehouse-link-panel');const box=$('ship-warehouse-link-results');if(!panel||!box)return;panel.classList.remove('hidden');const rows=hits.slice(0,8).map((h,i)=>{const c=h.cell||h;const it=h.item||{};return `<button type="button" class="deduct-card yx42-ship-warehouse-hit" data-yx42-open-warehouse="${i}"><strong>${esc(c.zone)}-${Number(c.column_index)}-${Number(c.slot_number)}</strong><div>${esc(clean(it.customer_name||state.customer||''))}</div><div class="small-note">${esc(it.product_text||productText||'')}</div></button>`;}).join('');box.innerHTML=(rows||'<div class="empty-state-card compact-empty">倉庫圖找不到這筆商品位置</div>')+`<div class="btn-row compact-row"><button type="button" class="primary-btn small-btn" id="yx42-open-warehouse-search">開啟倉庫圖搜尋</button></div>`;box.querySelector('#yx42-open-warehouse-search')?.addEventListener('click',()=>openWarehouseWithQuery(q));box.querySelectorAll('[data-yx42-open-warehouse]').forEach(btn=>btn.addEventListener('click',()=>openWarehouseWithQuery(q)));panel.scrollIntoView?.({behavior:'smooth',block:'nearest'});}
  async function locateSelectedProduct(productText){const seq=++state.locSeq;const queries=warehouseSearchQueries(productText);if(!queries.length)return toast('缺少倉庫搜尋關鍵字','warn');try{let hits=[],used=queries[0];for(const q of queries){const d=await api('/api/warehouse/search?q='+encodeURIComponent(q)+'&ts='+Date.now());if(seq!==state.locSeq)return;hits=Array.isArray(d.items)?d.items:[];used=q;if(hits.length)break;}if(!hits.length){showWarehouseHits(productText,[],queries[0]);return toast('倉庫圖找不到這筆商品位置','warn');}const msg=hits.slice(0,5).map(h=>{const c=h.cell||h;return `${c.zone}-${Number(c.column_index)}-${Number(c.slot_number)}`;}).join('、');showWarehouseHits(productText,hits,used);toast('倉庫位置：'+msg,'ok');}catch(e){toast(e.message||'查詢倉庫位置失敗','error');}}
  ...
  function bindSubmitButtonV52(){
    const btn=$('submit-btn');
    if(!btn || btn.dataset.yxV52ShipBound==='1') return;
    btn.dataset.yxV52ShipBound='1';
    btn.classList.add('yx-v52-ship-submit-btn');
    btn.removeAttribute('onclick');
    btn.addEventListener('click', function(ev){
      ev.preventDefault(); ev.stopPropagation(); ev.stopImmediatePropagation?.();
      confirmSubmit().catch(err=>toast(err.message||'出貨送出失敗','error'));
    }, true);
  }

  function bind(){if(state.bound)return;state.bound=true;bindSubmitButtonV52();document.addEventListener('click',(e)=>{const c=e.target.closest('[data-ship-customer]');if(c){e.preventDefault();loadItems(c.dataset.shipCustomer).catch(err=>toast(err.message,'error'));return;}const add=e.target.closest('[data-ship-add-index]');if(add){e.preventDefault();addItem(add.dataset.shipAddIndex);return;}const rm=e.target.closest('[data-selected-remove]');if(rm){e.preventDefault();state.selected.splice(Number(rm.dataset.selectedRemove),1);state.previewSeq++;const panel=$('ship-preview-panel')||$('module-result');if(panel)panel.classList.add('hidden');renderSelected();return;}const loc=e.target.closest('.yx22-location-btn');if(loc){e.preventDefault();locateSelectedProduct(loc.dataset.prod||loc.dataset.shipLocQuery||'');return;}},true);document.addEventListener('keydown',(e)=>{if((e.target.id==='customer-name'||e.target.id==='ship-customer-search')&&e.key==='Enter'){e.preventDefault();loadItems(state.customer,{force:true}).catch(err=>toast(err.message,'error'));}},true);document.addEventListener('change',(e)=>{if(e.target.id==='ship-customer-item-select'&&e.target.value!==''){addItem(e.target.value);e.target.value='';}},true);document.addEventListener('input',(e)=>{if(e.target.id==='customer-name'||e.target.id==='ship-customer-search'){setCustomer(e.target.value);renderCustomers();renderItems();}if(e.target.matches('[data-support-editor]'))updateSelectedProductFromSupport(Number(e.target.dataset.supportEditor),e.target.value);},true);}
  function install(){document.documentElement.dataset.yxShipSingle='locked-one-line-html-v52-submit-bound-location-jump';bindSubmitButtonV52();bind();loadCustomers();window.addEventListener('yx:customers-loaded',e=>{state.customers=Array.isArray(e.detail?.items)?e.detail.items:state.customers;},false);window.addEventListener('yx:customer-selected',e=>{const name=clean(e.detail?.name||'');if(name)loadItems(name,{force:true}).catch(err=>toast(err.message,'error'));},false);renderSelected();const name=clean($('customer-name')?.value||'');if(name)loadItems(name).catch(()=>{});else renderItems();window.confirmSubmit=confirmSubmit;}
...
/* ===== V52 SHIP REAL LOADED WRITEBACK =====
   修正出貨頁主按鈕沒有綁定確認送出。 */
(function(){
  'use strict';
  if(window.__YX_V52_SHIP_SUBMIT_LOCK__) return;
  window.__YX_V52_SHIP_SUBMIT_LOCK__=true;
  try{ document.documentElement.dataset.yxV52ShipWriteback='locked'; }catch(_e){}
  function setup(){
    var btn=document.getElementById('submit-btn');
    if(!btn || btn.dataset.yxV52ShipSubmitFinal==='1') return;
    btn.dataset.yxV52ShipSubmitFinal='1';
    btn.classList.add('yx-v52-ship-submit-btn');
    btn.removeAttribute('onclick');
    btn.addEventListener('click', function(ev){
      var m=document.querySelector('.module-screen[data-module]')?.dataset.module || '';
      if(m!=='ship') return;
      ev.preventDefault(); ev.stopPropagation(); ev.stopImmediatePropagation && ev.stopImmediatePropagation();
      var fn=window.__YX_SHIP_NATIVE_CONFIRM__ || window.confirmSubmit;
      if(typeof fn==='function') Promise.resolve(fn()).catch(function(e){ try{(window.YXHardLock?.toast || window.toast || alert)(e.message||'出貨送出失敗','error');}catch(_e){} });
      else try{(window.YXHardLock?.toast || window.toast || alert)('出貨送出功能尚未載入，請重新整理','error');}catch(_e){}
    }, true);
  }
...
/* ===== V53 SHIP CONFIRM BUTTON REAL LOADED WRITEBACK =====
   主按鈕固定接到出貨預覽；預覽已存在時再次按主按鈕會直接觸發「確認扣除」。 */
(function(){
  'use strict';
  if(window.__YX_V53_SHIP_CONFIRM_FIX__) return;
  window.__YX_V53_SHIP_CONFIRM_FIX__=true;
  try{document.documentElement.dataset.yxV53ShipWriteback='locked';}catch(_e){}
  const toast=(msg,type)=>{try{(window.YXHardLock?.toast||window.toast||window.alert)(msg,type);}catch(_e){try{alert(msg);}catch(__){}}};
  function bind(){
    const btn=document.getElementById('submit-btn'); if(!btn) return;
    btn.dataset.yxV53ShipSubmit='real-loaded'; btn.classList.add('yx-v53-ship-submit-btn'); btn.removeAttribute('onclick');
    btn.addEventListener('click', function(ev){
      const m=document.querySelector('.module-screen[data-module]')?.dataset.module||''; if(m!=='ship') return;
      ev.preventDefault(); ev.stopPropagation(); ev.stopImmediatePropagation && ev.stopImmediatePropagation();
      const confirmBtn=document.getElementById('yx22-confirm-ship');
      if(confirmBtn && !confirmBtn.disabled && !confirmBtn.closest('.hidden')){ confirmBtn.click(); return; }
      const fn=window.__YX_SHIP_NATIVE_CONFIRM__ || window.confirmSubmit;
      if(typeof fn==='function') Promise.resolve(fn()).catch(e=>toast(e.message||'出貨送出失敗','error'));
      else toast('出貨送出功能尚未載入，請重新整理','error');
    }, true);
  }
