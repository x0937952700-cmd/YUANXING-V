/* 沅興木業 FULL MASTER V20 SPEED UI REAL FIX - page_master_order_master_v20 */
(function(){ window.__YX_FULL_MASTER_V19_PAGE__='page_master_order_master_v20'; })();
...
(function(){
  ...
  async function refreshAfterSubmit(m, customer, posted, submittedItems, location){
    ...
    if (m === 'orders' || m === 'master_order') { forceCustomerCardVisible(customer, m); refreshCustomerBoardsSafe(customer).then(()=>forceCustomerCardVisible(customer, m)).catch(()=>{}); }
  }
...
(function(){
  ...
  async function moveCustomer(name, region){
    ...
    try {
      ...
      savePromise.then(()=>YX.toast(`${name} 區域已保存`, 'ok')).catch(e=>YX.toast(e.message || '區域保存失敗','error'));
      return;
      await savePromise;
