/* 沅興木業 FULL MASTER V42 SHIP WAREHOUSE REAL LOADED WRITEBACK - page_ship_master_v42 */
(function(){ window.__YX_FULL_MASTER_V42_PAGE__='page_ship_master_v42'; })();
...
(function(){
  'use strict';
  if (window.YXHardLock && window.YXHardLock.version === 'full-master-v42-ship-warehouse-real-loaded-html-js-css-writeback') return;
...
(function(){
  ...
  function install(){document.documentElement.dataset.yxShipSingle='locked-one-line-html-v42-clear-customer-location-jump';bind();loadCustomers();window.addEventListener('yx:customers-loaded',e=>{state.customers=Array.isArray(e.detail?.items)?e.detail.items:state.customers;},false);window.addEventListener('yx:customer-selected',e=>{const name=clean(e.detail?.name||'');if(name)loadItems(name,{force:true}).catch(err=>toast(err.message,'error'));},false);renderSelected();const name=clean($('customer-name')?.value||'');if(name)loadItems(name).catch(()=>{});else renderItems();window.confirmSubmit=confirmSubmit;}
