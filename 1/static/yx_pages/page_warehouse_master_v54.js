/* 沅興木業 FULL MASTER V54 WAREHOUSE BATCH RENDER DB FIX WRITEBACK - page_warehouse_master_v54 */
(function(){ window.__YX_FULL_MASTER_V54_PAGE__='page_warehouse_master_v54'; })();
...
(function(){
  ...
  function maxSlot(z,c){
    ...
    if(nums.length) return Math.max(20, ...nums); // V53：每欄保底 20 格，插入可超過 20
    // V52：從根本檔案拉回動態格位外觀；HTML 不再預塞 20 格，初次載入用 20 作為資料庫尚未回來的臨時骨架。
    const domCount=getColumnList(z,c)?.querySelectorAll('[data-slot]')?.length || 0;
    return Math.max(20, domCount || 0); // V53：初次與空欄也固定 20 格
  }
  ...
  function updateNotes(){
    for(const z of zones){ const n=$(z==='A'?'zone-A-count-note':'zone-B-count-note'); if(n) n.textContent='6 欄｜每欄固定 20 格｜可長按插入/刪除/拖拉'; }
  }
  ...
  async function openWarehouseModal(z,c,s){
    ...
    // V42：先立刻開啟格位批量加入畫面，再背景抓 A/B 區未錄入商品；若使用者已關閉或改開別格，舊回應不可覆蓋目前格位。
    try { await loadAvailable(); if(state.modalSeq===modalSeq && !$('warehouse-modal')?.classList.contains('hidden')) renderCellItems(); } catch(e){ if(state.modalSeq===modalSeq) toast(e.message||'未錄入商品載入失敗','error'); }
  }
  ...
  async function saveCellRaw(z,c,s,items,note){
    const payload={zone:clean(z).toUpperCase(),column_index:Number(c),slot_type:'direct',slot_number:Number(s),items:items||[],note:note||''};
    const d=await api('/api/warehouse/cell',{method:'POST',body:JSON.stringify(payload)});
    if(!d || d.success===false) throw new Error(d?.error||d?.message||'格位儲存失敗');
    if(Array.isArray(d.cells)){ state.data.cells=d.cells; updateAllSlots(); }
    const saved=(Array.isArray(d.cells)?d.cells:state.data.cells||[]).find(x=>clean(x.zone).toUpperCase()===payload.zone && Number(x.column_index)===payload.column_index && Number(x.slot_number)===payload.slot_number);
    let savedItems=[]; try{ savedItems=Array.isArray(saved?.items)?saved.items:JSON.parse(saved?.items_json||'[]'); }catch(_){ savedItems=[]; }
    const wantQty=(payload.items||[]).reduce((n,it)=>n+itemQty(it),0);
    const gotQty=(savedItems||[]).reduce((n,it)=>n+itemQty(it),0);
    if(wantQty>0 && gotQty<=0){ throw new Error('格位已送出但資料庫回讀不到商品，已阻止假成功'); }
    return d;
  }
  async function saveWarehouseCell(){
    ...
    try{
      ...
      const savedResp = await saveCellRaw(target.zone,target.col,target.slot,items,$('warehouse-note')?.value||'');
      toast(added.length ? '商品已送出並永久寫入格位' : '格位已儲存','ok');
      if(Array.isArray(savedResp?.cells)){ state.data.cells=savedResp.cells; updateAllSlots(); }
      closeWarehouseModal();
  ...
  function install(){ if(!isWarehouse()) return; document.documentElement.dataset.yxWarehouseSingleHtmlDataJs='true'; document.documentElement.dataset.yxWarehouseV42Fix='pending-search-modal-safe'; document.documentElement.dataset.yxV54WarehouseBatchDbFixed='locked'; document.documentElement.dataset.yxV42ShipWarehouseWriteback='locked'; bindGlobal(); bindSlots(); setWarehouseZone(localStorage.getItem('warehouseActiveZone')||'A',false); renderWarehouse(true).then(applyPendingWarehouseSearch).catch(()=>{}); }
...
/* ===== V52 WAREHOUSE ROOT STYLE RESTORE =====
   從「根本.zip」拉回動態格位外觀：HTML 只保留每欄容器，格子由資料庫/JS 產生。 */
(function(){
  'use strict';
...
/* ===== V53 WAREHOUSE FIXED 20 REAL LOADED WRITEBACK =====
   每個 A/B 區 1~6 欄至少顯示 20 格；長按選單、插入、刪除、拖拉仍保留。 */
(function(){
  'use strict';
  if(window.__YX_V53_WAREHOUSE_FIXED20__) return;
  window.__YX_V53_WAREHOUSE_FIXED20__=true;
  try{document.documentElement.dataset.yxV54WarehouseBatchDbFixed='locked';}catch(_e){}
  function ensureMin20Dom(){
    ['A','B'].forEach(function(z){for(var c=1;c<=6;c++){var list=document.querySelector('.vertical-column-card[data-zone="'+z+'"][data-column="'+c+'"] .vertical-slot-list'); if(!list) continue; for(var s=1;s<=20;s++){if(!list.querySelector('[data-zone="'+z+'"][data-column="'+c+'"][data-slot="'+s+'"]')){try{ if(typeof window.renderWarehouse==='function'){ window.renderWarehouse(true); return; } }catch(_e){} }}}});
  }
  if(document.readyState==='loading') document.addEventListener('DOMContentLoaded',()=>setTimeout(ensureMin20Dom,300),{once:true}); else setTimeout(ensureMin20Dom,300);
  window.addEventListener('load',()=>setTimeout(ensureMin20Dom,600));
})();
