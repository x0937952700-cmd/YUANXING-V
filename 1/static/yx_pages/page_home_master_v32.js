/* 沅興木業 FULL MASTER V32 REAL LOADED WRITEBACK - page_home_master_v32 */
(function(){ window.__YX_FULL_MASTER_V32_PAGE__='page_home_master_v32'; })();
...
/* ===== V26 HOME REAL LOADED HTML/JS LOCK: home logout button wired in actual loaded page_home_master_v32.js ===== */
(function(){
  'use strict';
  async function api(url,opt={}){
    const headers={'Content-Type':'application/json',...(opt.headers||{})};
  ...
  async function logout(){
    const btn=document.getElementById('home-logout-btn');
    try{ if(btn) btn.disabled=true; await api('/api/logout',{method:'POST',body:'{}'}); }
    catch(_e){}
    location.href='/login';
  }
  function install(){
    document.documentElement.dataset.yxV26HomeLogout='locked';
    const btn=document.getElementById('home-logout-btn');
    if(btn && !btn.dataset.yxV26Bound){
      btn.dataset.yxV26Bound='1';
      btn.addEventListener('click', logout);
    }
    window.logoutHome=logout;
  }
  if(document.readyState==='loading') document.addEventListener('DOMContentLoaded',install,{once:true}); else install();
})();
/* ===== END V26 HOME REAL LOADED HTML/JS LOCK ===== */
