...
/* formal page module */
(function(){ window.__YX_FULL_MASTER_main_PAGE__='page_inventory_master_main'; })();
...
(function(){
  ...
  function hydrateRowsFromCache(source){
    try{
      const cached = window.YX?.cache?.read(cacheName(source), 1000*60*60*12);
  ...
  async function handleRowAction(source, id, action){
    ...
    if (action === 'to-master') {
      if (source === 'orders') {
        ...
        YX.toast('已加到總單', 'ok');
        if (!applySnapshotFromResponse(d, 'orders')) { try { window.YXDataStore?.setRows?.('orders', rowsStore('orders'), {reason:'inventory-order-confirmed'}); } catch(_e) {} }
        try { if (!applySnapshotFromResponse(d, 'master_order')) { try { window.YXDataStore?.setRows?.('master_order', rowsStore('master_order'), {reason:'inventory-master-confirmed'}); } catch(_e) {} } } catch(_e) {}
  ...
  function applySnapshotFromResponse(data, source){
    ...
    if (Array.isArray(rows)) {
      rowsStore(source, rows);
      try { window.YXDataStore?.setRows?.(source, rows, {reason:'inventory-apply-snapshot'}); } catch(_e) {}
      if (shouldAvoidRerender(source)) {
        updateSummaryHeaderOnly(source);
  ...
  async function batchTransfer(source, target){
    ...
    if (!applySnapshotFromResponse(d, source)) { try { window.YXDataStore?.setRows?.(source, rowsStore(source), {reason:'inventory-local-confirmed'}); } catch(_e) {}; if (shouldAvoidRerender(source)) updateSummaryHeaderOnly(source); else renderSummary(source); }
    try { if (target === 'orders' || target === 'master_order' || target === 'master_orders') { const t = target === 'master_orders' ? 'master_order' : target; if (customer) window.__YX_SELECTED_CUSTOMER__ = customer; if (!applySnapshotFromResponse(d, t)) { try { window.YXDataStore?.setRows?.(t, rowsStore(t), {reason:'inventory-target-local-confirmed'}); } catch(_e) {} } } } catch(_e) {}
  ...
  async function loadSource(source, opts={}){
    ...
    if (!source) return [];
    const canUseCache = !opts.force && !opts.afterSubmit && !opts.beforeEditSave;
    const hadCache = canUseCache ? hydrateRowsFromCache(source) : false;
    const fetchFresh = async () => {
      if (state.loading === source && !opts.force) return rowsStore(source);
      state.loading = source;
      try {
        const d = await YX.api(endpoint(source) + '?yx136_first=1&fast=1&limit=800', {method:'GET'});
        ...
        try { window.dispatchEvent(new CustomEvent('yx:product-source-loaded', {detail:{source, count:rows.length, rows, cached:false}})); } catch(_e) {}
        try { if ((source === 'orders' || source === 'master_order') && window.YX113CustomerRegions?.renderFromCurrentRows) window.YX113CustomerRegions.renderFromCurrentRows(); } catch(_e) {}
        return rowsStore(source);
      } finally { if (state.loading === source) state.loading = null; }
    };
    if (hadCache && !opts.force) {
      try { (window.requestIdleCallback || function(fn){ return setTimeout(fn, 40); })(()=>fetchFresh().catch(e=>{ try{ console.warn('fresh product load failed', e); }catch(_e){}; }), {timeout:900}); } catch(_e) { setTimeout(()=>fetchFresh().catch(()=>{}), 40); }
      return rowsStore(source);
    }
    return fetchFresh();
  }
  ...
  async function moveInventory(card, target){
    ...
    if (!applySnapshotFromResponse(d, 'inventory')) { try { window.YXDataStore?.setRows?.('inventory', rowsStore('inventory'), {reason:'inventory-confirmed'}); } catch(_e) {} }
    try { if (!applySnapshotFromResponse(d, targetSource)) { try { window.YXDataStore?.setRows?.(targetSource, rowsStore(targetSource), {reason:'inventory-target-confirmed'}); } catch(_e) {} } } catch(_e) {}
  ...
  async function shipItem(card){
    ...
    YX.toast('已直接出貨', 'ok');
    if (!applySnapshotFromResponse(d, source)) { try { window.YXDataStore?.setRows?.(source, rowsStore(source), {reason:'inventory-source-confirmed'}); } catch(_e) {} }
    try { await refreshCustomerBoards(customer); } catch(_e) {}
  }
  ...
  async function bulkDelete(source){
    ...
    try { window.YXDataStore?.setRows?.(source, rowsStore(source), {reason:'inventory-bulk-delete-optimistic'}); } catch(_e) {}
    clearSelected(source);
    renderSummary(source); renderCards(source);
...
(function(){
  ...
  async function api(url, opt={}){
    const r = await window.YXDataStore.requestResponse(url, {
  ...
  async function refreshAfterSubmit(m, customer, posted, submittedItems, location){
    ...
    if (act?.rowsStore) {
      if (Array.isArray(serverRows) && serverRows.length) {
        ...
        try { act.loadSource?.(m, {force:false, afterSubmit:true, customer_name:customer}); } catch(_e) {}
      }
      act.renderSummary?.(m);
      act.renderCards?.(m);
    }
  ...
  async function finalConfirmSubmit(ev){
    ...
    api(apiPath(m), {method:'POST', body:JSON.stringify({customer_name:customer, ocr_text:text, items, duplicate_mode:duplicateMode, location:activeZone, zone:activeZone, region:(m === 'orders' || m === 'master_order') ? '北區' : '', request_key:requestKey, verify_readback:true, v452_save_lock:true})})
      .then(async posted => {
        try {
          const act = window.YX113ProductActions || window.YX132ProductActions || window.YX128ProductActions;
