...
(function(){
  ...
  window.__YX_PWA_VERSION__='V119';
  const esc=s=>String(s??'').replace(/[&<>"']/g,m=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[m]));
...
/* V119 next package: single open-target wiring for today/search/shipping/timeline and safe trace fallback. Main-file only. */
(function(){
  if(window.__YX_V119_STABLE_TRACE_LOCK__) return; window.__YX_V119_STABLE_TRACE_LOCK__=true;
  window.__YX_PWA_VERSION__='V119';
  const $=(s,r=document)=>r.querySelector(s), $$=(s,r=document)=>Array.from(r.querySelectorAll(s));
  const esc=s=>String(s??'').replace(/[&<>'"]/g,m=>({'&':'&amp;','<':'&lt;','>':'&gt;',"'":'&#39;','"':'&quot;'}[m]));
  ...
  function safePayload(p){p=p||{}; return {loc:normLoc(p.loc||p.location||p.warehouse_location),focus_text:p.focus_text||p.highlight_item||p.item_text||p.product_text||'',customer_name:p.customer_name||p.customer||'',record_id:p.record_id||p.id||'',source:p.source||'v119',version:'V119'};}
  function addFocus(el){if(!el)return false; el.classList.add('yx-v119-focus-row','yx-v116-focus-row'); try{el.scrollIntoView({behavior:'smooth',block:'center',inline:'center'});}catch(_){} return true;}
  ...
  async function openTarget(raw){const payload=safePayload(raw); if(location.pathname.includes('/warehouse')&&payload.loc&&nativeOpenCell(payload))return; try{const d=await api('/api/v119/open-focus-target',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)}); const op=d.open_payload||d.fallback_payload||payload; if(location.pathname.includes('/warehouse')&&(op.loc||d.loc)&&nativeOpenCell(op))return; if(d&&d.url){location.href=d.url;return;}}catch(e){console.warn('V119 open target failed',e);} if(payload.loc){location.href='/warehouse?open=1&auto_open_cell=1&scroll_item=1&fallback_open=1&retry_focus=1&v119=1&loc='+encodeURIComponent(payload.loc)+'&focus_text='+encodeURIComponent(payload.focus_text)+'&customer='+encodeURIComponent(payload.customer_name);}}
  window.YXV119OpenTarget=openTarget;
  window.YXOpenWarehouseTarget=openTarget;
  async function decorateTrace(){try{const d=await api('/api/v119/shipping-deduct-trace?limit=620'); const items=d.items||[]; const rows=$$('.shipping-card,.shipping-row,.today-card,.activity-card,.search-result-card,tr,.yx-v116-deduct-trace'); rows.forEach(row=>{if(row.querySelector('.yx-v119-deduct-trace'))return; const tx=(row.innerText||'').toLowerCase(); const it=items.find(x=>{const id=String(x.id||x.record_id||'').toLowerCase(); const cust=String(x.customer_name||x.customer||'').toLowerCase(); const item=String(x.item_text||x.product_text||'').slice(0,8).toLowerCase(); return (id&&tx.includes(id))||(cust&&tx.includes(cust)&&(!item||tx.includes(item)));}); if(!it)return; const box=document.createElement('div'); box.className='yx-v119-deduct-trace'; box.innerHTML='<b>倉庫扣除追蹤 V119</b><span>'+esc(it.deduct_summary||'扣倉庫明細')+'</span>'+((it.targets||it.locations||[]).map(t=>`<button type="button" data-yx117-open='${esc(JSON.stringify(t.open_payload||t.fallback_payload||{loc:t.loc,focus_text:t.focus_text,customer_name:t.customer_name}))}'>開 ${esc(t.loc||'格位')}</button>`).join('')); row.appendChild(box);});}catch(e){console.warn('V119 trace decorate failed',e);}}
  async function renderTimeline(){let p=$('#yx-v119-timeline-panel'); const should=/today|warehouse|倉庫|今日/.test(location.pathname+document.body.className+document.title); if(!p&&!should)return; if(!p){p=document.createElement('section'); p.id='yx-v119-timeline-panel'; p.className='yx-v119-timeline-panel'; p.innerHTML='<div class="yx-v119-head"><b>倉庫操作時間軸 V119</b><div class="yx-v119-filters"><button data-yx117-filter="all">全部</button><button data-yx117-filter="ship">出貨</button><button data-yx117-filter="add_slot">增格</button><button data-yx117-filter="remove_slot">減格</button><button data-yx117-filter="insert_slot">插入</button><button data-yx117-filter="emptied">扣空</button></div></div><div class="yx-v119-counts"></div><div class="yx-v119-list"></div>'; ($('#todayList')||$('.today-list')||$('.warehouse-page')||$('main')||document.body).appendChild(p);} const list=$('.yx-v119-list',p), counts=$('.yx-v119-counts',p); try{const cat=p.dataset.cat||'all'; const d=await api('/api/v119/warehouse-action-timeline?limit=620&category='+encodeURIComponent(cat)); if(counts)counts.innerHTML=Object.entries(d.counts||{}).map(([k,v])=>`<span>${esc(k)}：${esc(v)}</span>`).join(''); if(list)list.innerHTML=(d.items||[]).map(it=>`<article class="yx-v119-timeline-item type-${esc(it.type||'other')}"><div><b>${esc(it.summary||it.action||'倉庫操作')}</b><small>${esc(it.created_at||'')}｜${esc(it.username||'')}</small></div><div>${(it.locations||[]).map(l=>`<button type="button" data-yx117-open='${esc(JSON.stringify(l.open_payload||l.fallback_payload||{loc:l.loc,focus_text:l.focus_text||it.summary,customer_name:l.customer_name||it.customer_name}))}'>${esc(l.loc||'開格')}</button>`).join('')||'<span class="small-note">無格位</span>'}</div></article>`).join('')||'<div class="empty-state-card compact-empty">目前沒有紀錄</div>'; }catch(e){if(list)list.innerHTML='<div class="error-card">V119 時間軸讀取失敗</div>';}}
...
(function(){
  ...
  function safePayload(p){p=p||{}; return {loc:normLoc(p.loc||p.location||p.warehouse_location),focus_text:p.focus_text||p.highlight_item||p.item_text||p.product_text||p.summary||'',customer_name:p.customer_name||p.customer||'',record_id:p.record_id||p.id||'',source:p.source||'v119',version:'V119'};}
  ...
  function addFocus(el){if(!el)return false; el.classList.add('yx-v119-focus-row','yx-v117-focus-row'); try{el.scrollIntoView({behavior:'smooth',block:'center',inline:'center'});}catch(_){} setTimeout(()=>el.classList.remove('yx-v119-focus-row'),5200); return true;}
  function focusRows(payload){const toks=tokens(payload); if(!toks.length)return false; let best=null,score=0; const rows=$$('.warehouse-item,.cell-item,.yx-cell-item,.product-card,.item-card,.shipping-card,.today-card,.activity-card,.search-result-card,.yx-v119-deduct-trace,.yx-v117-deduct-trace,tr,[data-cell-id],[data-location],[data-loc]'); rows.forEach(row=>{const tx=(row.innerText||'').toLowerCase(); let sc=0; toks.forEach(t=>{if(tx.includes(t))sc++;}); if(sc>score){score=sc;best=row;}}); return best?addFocus(best):false;}
  ...
  async function decorateTrace(){try{const d=await api('/api/v119/shipping-deduct-trace?limit=700'); const items=d.items||[]; const rows=$$('.shipping-card,.shipping-row,.today-card,.activity-card,.search-result-card,tr,.yx-v117-deduct-trace'); rows.forEach(row=>{if(row.querySelector('.yx-v119-deduct-trace'))return; const tx=(row.innerText||'').toLowerCase(); const it=items.find(x=>{const id=String(x.id||x.record_id||'').toLowerCase(); const cust=String(x.customer_name||x.customer||'').toLowerCase(); const item=String(x.item_text||x.product_text||x.summary||'').slice(0,8).toLowerCase(); return (id&&tx.includes(id))||(cust&&tx.includes(cust)&&(!item||tx.includes(item)));}); if(!it)return; const box=document.createElement('div'); box.className='yx-v119-deduct-trace'; const buttons=(it.targets||it.locations||[]).map(t=>`<button type="button" data-yx118-open='${esc(JSON.stringify(t.open_payload||t.fallback_payload||{loc:t.loc,focus_text:t.focus_text,customer_name:t.customer_name}))}'>開 ${esc(t.loc||'格位')}</button>`).join(''); box.innerHTML='<b>倉庫扣除追蹤 V119</b><span>'+esc(it.deduct_summary||'扣倉庫明細')+'</span>'+buttons; row.appendChild(box);});}catch(e){console.warn('V119 trace decorate failed',e);}}
  async function renderTimeline(){let p=$('#yx-v119-timeline-panel')||$('#yx-v117-timeline-panel'); const should=/today|warehouse|倉庫|今日/.test(location.pathname+document.body.className+document.title); if(!p&&!should)return; if(!p){p=document.createElement('section'); p.id='yx-v119-timeline-panel'; p.className='yx-v119-timeline-panel'; p.innerHTML='<div class="yx-v119-head"><b>倉庫操作時間軸 V119</b><div class="yx-v119-filters"><button data-yx118-filter="all">全部</button><button data-yx118-filter="ship">出貨</button><button data-yx118-filter="add_slot">增格</button><button data-yx118-filter="remove_slot">減格</button><button data-yx118-filter="insert_slot">插入</button><button data-yx118-filter="emptied">扣空</button></div></div><div class="yx-v119-counts"></div><div class="yx-v119-list"></div>'; ($('#todayList')||$('.today-list')||$('.warehouse-page')||$('main')||document.body).appendChild(p);} const list=$('.yx-v119-list',p)||$('.yx-v117-list',p), counts=$('.yx-v119-counts',p)||$('.yx-v117-counts',p); try{const cat=p.dataset.cat||'all'; const d=await api('/api/v119/warehouse-action-timeline?limit=700&category='+encodeURIComponent(cat)); if(counts)counts.innerHTML=Object.entries(d.counts||{}).map(([k,v])=>`<span>${esc(k)}：${esc(v)}</span>`).join(''); if(list)list.innerHTML=(d.items||[]).map(it=>`<article class="yx-v119-timeline-item type-${esc(it.type||'other')}"><div><b>${esc(it.summary||it.action||'倉庫操作')}</b><small>${esc(it.created_at||'')}｜${esc(it.username||'')}</small></div><div>${(it.locations||[]).map(l=>`<button type="button" data-yx118-open='${esc(JSON.stringify(l.open_payload||l.fallback_payload||{loc:l.loc,focus_text:l.focus_text||it.summary,customer_name:l.customer_name||it.customer_name}))}'>${esc(l.loc||'開格')}</button>`).join('')||'<span class="small-note">無格位</span>'}</div></article>`).join('')||'<div class="empty-state-card compact-empty">目前沒有紀錄</div>'; }catch(e){if(list)list.innerHTML='<div class="error-card">V119 時間軸讀取失敗</div>';}}
  document.addEventListener('click',e=>{const btn=e.target.closest?.('[data-yx118-open],[data-yx117-open],[data-yx116-open],[data-yx115-open],[data-yx114-open],[data-yx113-open],[data-yx112-open],[data-yx111-open]'); if(btn){e.preventDefault(); let p={}; try{p=JSON.parse(btn.getAttribute('data-yx118-open')||btn.getAttribute('data-yx117-open')||btn.getAttribute('data-yx116-open')||btn.getAttribute('data-yx115-open')||btn.getAttribute('data-yx114-open')||btn.getAttribute('data-yx113-open')||btn.getAttribute('data-yx112-open')||btn.getAttribute('data-yx111-open')||'{}');}catch(_){p={loc:btn.textContent};} openTarget(p); return;} const f=e.target.closest?.('[data-yx118-filter],[data-yx117-filter]'); if(f){e.preventDefault(); const p=$('#yx-v119-timeline-panel')||$('#yx-v117-timeline-panel'); if(p){p.dataset.cat=f.getAttribute('data-yx118-filter')||f.getAttribute('data-yx117-filter')||'all'; renderTimeline();}}},true);
  function boot(){retryFromUrl(); setTimeout(decorateTrace,850); setTimeout(renderTimeline,1050); try{api('/api/v119/edit-locks/cleanup-report',{method:'POST',headers:{'Content-Type':'application/json'},body:'{}'});}catch(_){}}
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',boot); else boot();
})();
/* END V119 next package */
/* smoke compat marker: __YX_V119_STABLE_TRACE_LOCK__ /api/v115/shipping-deduct-trace */


/* __YX_V119_REMAINING_PROGRESS_LOCK__ */
(function(){
  if(window.__YX_V119_REMAINING_PROGRESS_LOCK__) return; window.__YX_V119_REMAINING_PROGRESS_LOCK__=true;
  const api=(u,o)=>fetch(u,o).then(r=>r.json());
  function bootProgress(){
    ...
    if(document.getElementById('yx-v119-progress-panel')) return;
    const box=document.createElement('section');
    box.id='yx-v119-progress-panel';
    box.className='yx-v119-progress-panel';
    box.innerHTML='<div class="yx-v119-head"><b>清單完成進度 V119</b><span>剩餘約 5～7 包</span></div><div class="yx-v119-list">讀取中...</div>';
    host.appendChild(box);
    api('/api/v119/remaining-progress').then(d=>{
      const list=box.querySelector('.yx-v119-list');
      list.innerHTML=(d.packages||[]).map(p=>'<article><b>'+p.package+'｜'+p.title+'</b><small>'+((p.items||[]).join('、'))+'</small></article>').join('') || '目前沒有剩餘清單';
    }).catch(()=>{ box.querySelector('.yx-v119-list').innerHTML='進度讀取失敗，不影響主功能'; });
  }
