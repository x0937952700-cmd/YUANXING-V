...
(function(){
  ...
  const state = {
    ...
    current:{zone:'A',col:1,slot:1,items:[],note:''}, batchCount:3, drag:null, loading:null, bound:false, unplacedOpen:false, localMutationAt:0, saving:false
  };
  ...
  const zones = ['A','B'];
  const CACHE_KEY='yx_warehouse_visible_snapshot_v520stable';
  ...
  function loadCache(){try{const raw=localStorage.getItem(CACHE_KEY); if(!raw)return false; const o=JSON.parse(raw); if(!o||!o.data)return false; state.data=o.data||state.data; state.available=Array.isArray(o.available)?o.available:state.available; state.availableByZone=o.availableByZone||state.availableByZone; return true;}catch(_e){return false;}}
  function markLocal(){state.localMutationAt=Date.now(); cacheNow();}
  function canApplyServer(){return Date.now()-Number(state.localMutationAt||0)>30000 && !state.saving;}
  function itemQty(it){
    const text=clean(it?.product_text||it?.product||'');
    if(text && typeof window.YX126Qty==='function'){
      const q=Number(window.YX126Qty(text, it?.qty||it?.quantity||it?.pieces||0));
    ...
    const m=right.match(/(?:x|×|\*)\s*(\d+)\s*(?:件)?\s*$/i); if(m) return Math.max(1,Number(m[1]));
    return 1;
  }
  ...
  function sizeSummaryFor(items){
    ...
    // 20260516ba：格子第一排只顯示「每個尺寸加總」的件數拆解；單一尺寸不顯示，避免 71x12x10 15件 這種長文字塞爆格子。
  ...
  function updateSlotUI(z,c,s){
    ...
    if(!items.length){
      el.innerHTML=`<div class="yx108-slot-row yx108-slot-row1 yx116-slot-row1"><span class="yx108-slot-no">${s}</span><span class="yx108-slot-customers yx108-slot-empty yx520-wh-empty">空格</span></div><div class="yx108-slot-row yx108-slot-row2 yx116-slot-row2"><span class="yx108-slot-sum"></span><span class="yx108-slot-total">0件</span></div><div class="yx520-wh-actions"><button type="button" class="yx520-wh-act" data-wh-cell-act="open" aria-label="編輯" data-yx-label="編輯">編輯</button><button type="button" class="yx520-wh-act" data-wh-cell-act="insert" aria-label="加格" data-yx-label="加格">加格</button><button type="button" class="yx520-wh-act danger" data-wh-cell-act="delete" aria-label="刪格" data-yx-label="刪格">刪格</button></div>`;
      return;
    }
    ...
    el.innerHTML=`<div class="yx520-wh-head"><span class="yx520-wh-slotno">${s}</span>${sumHtml}<span class="yx520-wh-total">${totalQty}件</span></div><div class="yx520-wh-body">${itemRows}</div><div class="yx520-wh-actions"><button type="button" class="yx520-wh-act" data-wh-cell-act="open" aria-label="編輯" data-yx-label="編輯">編輯</button><button type="button" class="yx520-wh-act" data-wh-cell-act="insert" aria-label="加格" data-yx-label="加格">加格</button><button type="button" class="yx520-wh-act danger" data-wh-cell-act="delete" aria-label="刪格" data-yx-label="刪格">刪格</button></div>`;
  }
  ...
  async function renderWarehouse(force=false){
    if(!force && loadCache()){ updateAllSlots(); }
    if(state.loading && !force) return state.loading;
    state.loading=(async()=>{ try{ const [d]=await Promise.all([api('/api/warehouse?ts='+Date.now()), loadAvailable()]);
  ...
  function setCellLocal(z,c,s,items,note){
    ...
    cell.items=JSON.parse(JSON.stringify(items||[])); cell.items_json=JSON.stringify(cell.items); cell.note=note||'';
  ...
  async function saveWarehouseCell(){ const z=state.current.zone,c=state.current.col,s=state.current.slot; const items=[...(state.current.items||[]),...collectBatchItems()]; const note=$('warehouse-note')?.value||''; setCellLocal(z,c,s,items,note); closeWarehouseModal(); highlightWarehouseCell(z,c,s); toast('格位已先更新，背景保存中','ok'); state.saving=true; saveCellRaw(z,c,s,items,note).then(()=>{state.saving=false; markLocal(); toast('格位已保存到資料庫','ok'); loadAvailable().catch(()=>{});}).catch(e=>{state.saving=false; localStorage.setItem('yx_warehouse_pending_failed_save', JSON.stringify({z,c,s,items,note,at:Date.now()})); toast(e.message||'背景保存失敗，已保留畫面並待重送','error');}); }
  ...
  async function moveCellContents(from,to){
    ...
    state.saving=true; Promise.all([saveCellRaw(f.zone,f.col,f.slot,[],src.note), saveCellRaw(t.zone,t.col,t.slot,dstAfter,dst.note)]).then(()=>{state.saving=false; markLocal(); toast('移動已保存到資料庫','ok'); loadAvailable().catch(()=>{});}).catch(e=>{state.saving=false; toast(e.message||'背景保存失敗，畫面已保留','error');});
  }
