/* YX 20260518: settings page repaired. Safe API fallback + no blank/error page. */
...
(function(){
  ...
  async function api(url, opt={}){
    ...
    const runner = window.YXDataStore?.requestResponse || window.fetch.bind(window);
    const res = await runner(url, {credentials:'same-origin', cache:'no-store', ...opt, headers});
...
(function(){
  ...
  async function api(url,opt={}){const runner=window.YXDataStore?.requestResponse||window.fetch.bind(window);const res=await runner(url,{credentials:'same-origin',cache:'no-store',...opt,headers:{'Content-Type':'application/json',...(opt.headers||{})}});const txt=await res.text();let d={};try{d=txt?JSON.parse(txt):{}}catch{d={success:false,error:txt||'伺服器回應格式錯誤'}};if(!res.ok||d.success===false)throw new Error(d.error||d.message||`請求失敗 ${res.status}`);return d;}
