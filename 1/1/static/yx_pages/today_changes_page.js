...
(function(){
  ...
  async function loadTodayChanges112(opts={}){
    ...
    const cacheKey = 'today_changes_light_v342';
    ...
    const fetchFresh = async () => {
      try {
        ...
        let data = await YX.api('/api/today-changes?yx143_final=1&v287=1&force=' + (forceHeavy ? '1' : '0'), {method:'GET'}); state.forceNext=false;
        // V136: 不再每次開今日異動就另外打 warehouse/available-items；只有手動刷新(force)才補最新未入倉區域統計。
        if (forceHeavy) {
          try {
            const wz = await YX.api('/api/warehouse/available-items?fast=1&yx138_manual=1', {method:'GET'});
  ...
  function install(){
    ...
    if (!state.shipRefreshBound) {
      ...
      const refreshTodayV214 = ()=>{
        try{
          ['today_changes_light_v342','today_changes_light_v337','today_changes_light_v332','today_changes_light_v307','today_changes_light_v287','today_changes_light_v282','today_changes_light_v252','today_changes_light_v215','today_changes_light_v210','today_changes_light_v208','today_changes_light_v207','today_changes_light_v198','today_changes_v215','today_changes_v210','today_changes_v208','today_changes_v207','today_changes_v198'].forEach(k=>window.YX?.cache?.remove?.(k));
        }catch(_e){}
