...
app=read('app.py')
if 'V119-V520-FINAL-SHIP-CACHE-ALIGN-PACK30' not in app:
    fail.append('APP_VERSION is not V507')
if '119-v520_final_ship_cache_align_pack30' not in app:
...
if 'yuanxing-v520-static-css-icons' not in sw:
    fail.append('service-worker cache version is not v520')
...
if '119-v520_final_ship_cache_align_pack30' not in manifest:
    fail.append('manifest start/id version is not v507')

data=read('static/yx_data_store.js')
for token in ['installApiBridge','installFetchBridge','localResponseForApi','getTodayWithUnplaced','today-changes\\/(count|badge)','filterAvailableAgainstWarehouse','v520-final-ship-cache-align-pack30']:
...
for token in ['applyMutation','installApi','installFetch','reduceAfterShip','appendTodayLocal','v520-final-ship-cache-align-pack30']:
    if token not in mut:
...
for token in ['v520-final-ship-cache-align-pack30','yx_warehouse_cache_v520-final-ship-cache-align-pack30','yx_warehouse_available_cache_v520-final-ship-cache-align-pack30']:
