...
for token,msg in [
    ('V119-V514-POSTDEPLOY-EVIDENCE-COLLECTOR-PACK24','app.py not bumped to V509'),
    ('119-v514_postdeploy_evidence_collector_pack24','STATIC_VERSION not bumped to V509'),
    ('v514-postdeploy-evidence-collector-pack24','API_SCHEMA_VERSION not bumped to V509'),
...
for rel in ['scripts/deploy_smoke_verify.py','scripts/postdeploy_data_consistency_verify.py','scripts/smoke_test.py','scripts/postdeploy_operation_closed_loop_verify.py']:
    ...
    has(txt, 'V119-V514-POSTDEPLOY-EVIDENCE-COLLECTOR-PACK24', f'{rel} expected app version not V509')
    if rel not in ('scripts/smoke_test.py','scripts/postdeploy_operation_closed_loop_verify.py'):
        has(txt, '119-v514_postdeploy_evidence_collector_pack24', f'{rel} expected static version not V509')
...
has(sw, 'yuanxing-v514-static-css-icons', 'service-worker cache version not V509')
has(sw, "url.pathname.startsWith('/api/')", 'service worker must bypass API cache')

manifest=read('static/manifest.webmanifest')
has(manifest, '119-v514-postdeploy-evidence-collector-pack24', 'manifest version/start_url/id not V509')

pwa=read('static/pwa.js')
has(pwa, '119-v514_postdeploy_evidence_collector_pack24', 'pwa.js fallback version not V509')
...
has(read('scripts/postdeploy_operation_closed_loop_verify.py'), 'V514_POSTDEPLOY_EVIDENCE_COLLECTOR_PACK24', 'missing V510 operation closed-loop verify script marker')
