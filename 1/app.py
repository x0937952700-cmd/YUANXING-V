...
app = Flask(__name__)
APP_VERSION = 'V119-V343-RENDER-REQUIREMENTS-FIX'
STATIC_VERSION = "119-v343-render-requirements-fix"
# service-line retained: mainfile behavior consolidated into formal services.
...
def _warehouse_payload_from_cells(cells, include_source_qty=False):
    ...
    for cell in cells or []:
        try:
            ...
            pass
    payload = dict(success=True, zones=zones, cells=cells or [], cache_version='v342-status-unviewed-filter', cache_policy='v142-client-local-first-no-stale-overwrite')
...
def _warehouse_column_payload(zone, column_index, operation_id=None, **extra):
    ...
    payload = dict(
        ...
        warehouse_stability='v342-status-unviewed-filter'
    )
    payload.update(extra or {})
    return payload
...
def api_warehouse_consistency_check():
    ...
    try:
        ...
        payload['warehouse_stability'] = 'v342-status-unviewed-filter'
        return jsonify(payload)
    except Exception as e:
    ...
    for cell in cells:
        ...
        for it in items:
            ...
            if not tokens or all(t in hay.lower() for t in tokens):
                ...
                break
    return jsonify(success=True, items=matched, cache_version='v342-status-unviewed-filter')
