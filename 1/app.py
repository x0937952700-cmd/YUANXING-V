...
app = Flask(__name__)
APP_VERSION = 'V119-V307-FIVE-PACK-TARGETED-STATUS-DETAIL'
STATIC_VERSION = "119-v332-five-pack-shortcut-target-highlight"
# service-line retained: mainfile behavior consolidated into formal services.
...
def add_cache_headers(response):
    ...
    response.headers['X-Yuanxing-Mainfile'] = '119-mainfile-stable'
...
def yx_v190_json_api_exception_handler(e):
    """Keep API failures JSON-only so 出貨/今日異動 panels never render Flask HTML 500 pages."""
    if isinstance(e, HTTPException):
        if (request.path or '').startswith('/api/'):
            return jsonify(success=False, error=e.description or e.name or '請求失敗'), int(e.code or 500)
        return e
    try:
        log_error('unhandled_exception_v190', f"{request.method} {request.path}: {str(e)[:260]}")
    except Exception:
        pass
    if (request.path or '').startswith('/api/'):
        return jsonify(success=False, error='系統暫時忙碌，請重試', detail=str(e)[:180]), 500
    raise e



# ============================================================
# V119 batch2 formal operation guard: idempotent background saves
# ============================================================
    ...
    try:
        ...
        if row and row.get('status') == 'done':
            try:
                ...
                cached = {}
            return cached or {'success': True, 'duplicate': True, 'operation_id': operation_id}
        if row:
            cur.execute(sql("UPDATE operation_log SET status=?, request_json=?, updated_at=? WHERE operation_id=?"), ('running', json.dumps(payload or {}, ensure_ascii=False), now(), operation_id))
        else:
...
def api_login():
    try:
        ...
        try: log_error('runtime_init_login_v192', str(_yx_init_err))
        except Exception: pass
    try:
        data = request.get_json(silent=True) or {}
...
def _yx145_write_payload(module='', customer_name='', item=None, extra=None):
    payload = dict(success=True, fast_response=True, module=module or '', customer_name=(customer_name or '').strip(), item=item or {}, server_cache='v208-fast-write-db-confirmed-snapshot')
    try:
        if customer_name:
...
def _yx137_product_payload(rows, cache_label):
    ...
    payload = dict(success=True, items=sliced, rows=sliced, server_cache=cache_label, **meta)
    return payload
...
def api_orders():
    ...
    if request.method == "GET":
        try:
            cache_key = _fast_cache_key('orders', version='v287', user=current_username(), limit=request.args.get('limit') or '', offset=request.args.get('offset') or '', all=request.args.get('all') or '', qv=request.args.get('v') or request.args.get('v287') or request.args.get('v282') or request.args.get('v262') or request.args.get('v257') or request.args.get('v252') or request.args.get('v249') or request.args.get('v244') or request.args.get('v228') or request.args.get('v227') or request.args.get('v226') or request.args.get('v225') or request.args.get('v224') or request.args.get('v223') or request.args.get('v222') or request.args.get('v221') or request.args.get('v214') or request.args.get('v212') or request.args.get('v211') or request.args.get('v208') or request.args.get('v207') or request.args.get('v201') or request.args.get('v199') or request.args.get('v198') or request.args.get('v197') or request.args.get('v196') or request.args.get('v195') or request.args.get('v193') or request.args.get('v192') or '')
            use_fast_cache = (request.args.get('force') != '1' and (request.args.get('fast') == '1' or request.args.get('light') == '1'))
            cached = _fast_cache_get(cache_key, 900.0) if use_fast_cache else None
            if cached:
                return jsonify(cached)
    ...
    try:
        ...
        pass
    if _yx145_fast_write_requested(data):
        return jsonify(_yx145_write_payload('orders', customer_name, extra={'count': len(items), 'cache_bust': 'v287'}))
    payload = product_service_response_payload(customer_name)
...
def api_master_orders():
    ...
    if request.method == "GET":
        try:
            cache_key = _fast_cache_key('master_orders', version='v287', user=current_username(), limit=request.args.get('limit') or '', offset=request.args.get('offset') or '', all=request.args.get('all') or '', qv=request.args.get('v') or request.args.get('v287') or request.args.get('v282') or request.args.get('v262') or request.args.get('v257') or request.args.get('v252') or request.args.get('v249') or request.args.get('v244') or request.args.get('v228') or request.args.get('v227') or request.args.get('v226') or request.args.get('v225') or request.args.get('v224') or request.args.get('v223') or request.args.get('v222') or request.args.get('v221') or request.args.get('v214') or request.args.get('v212') or request.args.get('v211') or request.args.get('v208') or request.args.get('v207') or request.args.get('v201') or request.args.get('v199') or request.args.get('v198') or request.args.get('v197') or request.args.get('v196') or request.args.get('v195') or request.args.get('v193') or request.args.get('v192') or '')
            use_fast_cache = (request.args.get('force') != '1' and (request.args.get('fast') == '1' or request.args.get('light') == '1'))
            cached = _fast_cache_get(cache_key, 900.0) if use_fast_cache else None
            if cached:
                return jsonify(cached)
    ...
    try:
        ...
        pass
    if _yx145_fast_write_requested(data):
        return jsonify(_yx145_write_payload('master_order', customer_name, extra={'count': len(items), 'cache_bust': 'v287'}))
    payload = product_service_response_payload(customer_name)
...
def shipping_service_validate_preview_token(token, customer_name, items):
    ...
    raw = get_setting(shipping_service_snapshot_key(token), '') or ''
    if not raw:
        return False, '出貨預覽已過期，請重新預覽'
    try:
        saved = json.loads(raw)
    except Exception:
        return False, '出貨預覽資料損壞，請重新預覽'
    if (saved.get('customer_name') or '').strip() != (customer_name or '').strip():
...
def api_ship():
    try:
        ...
        def _ship_fail(msg, code=400, **extra):
            payload = {'success': False, 'error': msg, 'operation_id': operation_id, 'preserve_preview': True, 'retry_action': 'repreview', 'version': 'v287-five-pack-status-detail-card'}
            payload.update(extra or {})
        ...
        if not ok_snapshot:
            return _ship_fail(snapshot_error, error_code='preview_changed')
        ...
        if result.get("success"):
            ...
            for _m in ('ship', 'orders', 'master_order', 'warehouse', 'today_changes', 'customers'):
                audit_service_safe_side_effect('ship_notify_' + _m, notify_sync_event, kind='refresh', module=_m, message='出貨已更新', extra={'customer_name': customer_name, 'count': len(items), 'operation_id': operation_id, 'cache_bust': 'v287'})
        ...
        if isinstance(result, dict) and customer_name:
            ...
            try:
                ...
                result['sync_version'] = 'v287-five-pack-status-detail-card'
                ...
                result['cache_bust'] = 'v287-five-pack-status-detail-card'
        ...
        if isinstance(result, dict):
            ...
            if not result.get('success'):
                ...
                result.setdefault('version', 'v287-five-pack-status-detail-card')
                if '_yx_operation_finish' in globals():
...
def api_shipping_records():
    ...
    return jsonify(success=True, items=rows, records=rows, version='v287-five-pack-status-detail-card')
...
def api_customers():
    ...
    try:
        if request.method == "GET":
            cache_key = _fast_cache_key('customers', version='v287', user=current_username(), light=request.args.get('light') or '', ship=request.args.get('ship_single') or '', qv=request.args.get('v') or request.args.get('v287') or request.args.get('v282') or request.args.get('v262') or request.args.get('v257') or request.args.get('v252') or request.args.get('v249') or request.args.get('v244') or request.args.get('v228') or request.args.get('v227') or request.args.get('v226') or request.args.get('v225') or request.args.get('v224') or request.args.get('v223') or request.args.get('v222') or request.args.get('v221') or request.args.get('v214') or request.args.get('v212') or request.args.get('v211') or request.args.get('v208') or request.args.get('v207') or request.args.get('v201') or request.args.get('v199') or request.args.get('v198') or request.args.get('v197') or request.args.get('v196') or request.args.get('v195') or request.args.get('v193') or request.args.get('v192') or '')
            use_fast_cache = (request.args.get('force') != '1' and (request.args.get('fast') == '1' or request.args.get('light') == '1'))
            cached = _fast_cache_get(cache_key, 900.0) if use_fast_cache else None
            if cached:
                return jsonify(cached)
            payload = dict(success=True, items=get_customers(), server_cache='v196-customers-relation-count-cache')
...
def api_customers_move():
    try:
        ...
        audit_service_safe_side_effect('customer_move_notify', notify_sync_event, kind="refresh", module="customers", message=f"客戶已移動：{name} -> {region}", extra={"customer_name": name, "region": region})
        _clear_product_fast_cache()
        return jsonify(success=True, items=get_customers(), item=item)
    except Exception as e:
...
def api_customer_detail(name):
    if request.method == "PUT":
        try:
            ...
            notify_sync_event(kind="refresh", module="customers", message=f"客戶已改名：{name} -> {new_name}", extra={"customer_name": new_name, "old_customer_name": name, "new_customer_name": new_name, "sync_version": "v234"})
    ...
    if request.method == "DELETE":
        try:
            ...
            notify_sync_event(kind='refresh', module='customers', message=f"客戶已{'封存' if mode == 'archived' else '刪除'}：{name}", extra={'customer_name': name, 'mode': mode, 'sync_version': 'v234'})
...
def _warehouse_payload_from_cells(cells, include_source_qty=False):
    ...
    for cell in cells or []:
        try:
            ...
            pass
    payload = dict(success=True, zones=zones, cells=cells or [], cache_version='v332-shortcut-target-highlight', cache_policy='v142-client-local-first-no-stale-overwrite')
...
def _warehouse_column_payload(zone, column_index, operation_id=None, **extra):
    ...
    payload = dict(
        ...
        warehouse_stability='v332-shortcut-target-highlight'
    )
    payload.update(extra or {})
    return payload
...
def api_warehouse_consistency_check():
    ...
    payload = {
        ...
        'version': 'v287-five-pack-status-detail-card',
    ...
    try:
        ...
        payload['warehouse_stability'] = 'v332-shortcut-target-highlight'
        return jsonify(payload)
    except Exception as e:
        log_error('api_warehouse_consistency_check_v282', str(e))
        return jsonify(success=False, error='倉庫一致性檢查失敗：' + str(e)[:180], operation_id=operation_id, version='v287-five-pack-status-detail-card')

@app.route("/api/warehouse/source-qty-map", methods=["GET"])
@login_required_json
def api_warehouse_source_qty_map():
    try:
        cache_key = _fast_cache_key('warehouse_source_qty_map', user=current_username())
        cached = _fast_cache_get(cache_key, 120.0) if request.args.get('fast') == '1' else None
        if cached:
            return jsonify(cached)
        payload = dict(success=True, source_qty_map=_warehouse_source_qty_map_for_client(), cache_version='v142-source-qty-map-deferred')
    ...
    for cell in cells:
        ...
        for it in items:
            ...
            if not tokens or all(t in hay.lower() for t in tokens):
                ...
                matched.append({"cell": cell_meta, "item": item_meta, "cell_id": cell_meta.get('cell_id') or cell_meta.get('id') or '', "client_signature": cell_meta.get('client_signature') or '', "slot_number": cell_meta.get('slot_number'), "column_index": cell_meta.get('column_index'), "zone": cell_meta.get('zone')})
                break
    return jsonify(success=True, items=matched, cache_version='v332-shortcut-target-highlight')
...
def api_warehouse_available_items():
    ...
    try:
        ...
        if zone_filter not in ("A", "B"):
            zone_filter = ""
        if request.args.get('fast') == '1':
            cached = _fast_cache_get(_fast_cache_key('warehouse_available', zone=zone_filter or 'ALL', user=current_username()), 120.0)
            if cached:
                return jsonify(cached)
        ...
        for detail_key, details_all in source_details.items():
            ...
            # V205: placed_qty is derived from mixed old/new warehouse rows. Cap it to the
            # current source total so duplicated legacy cell rows cannot over-deduct the
            # unplaced dropdown or make the shipping selector look inconsistent.
            try:
        ...
        payload = dict(success=True, items=items, zone=zone_filter, zone_summary=zone_summary, cache_version='v287-five-pack-status-detail-card')
        if request.args.get('fast') == '1':
            _fast_cache_set(_fast_cache_key('warehouse_available', zone=zone_filter or 'ALL', user=current_username()), payload)
        return jsonify(payload)
    except Exception as e:
...
def api_customer_items():
    ...
    try:
        ...
        payload = dict(success=True, items=aggregated, server_cache='v214-source-warehouse-consistency-key')
        if use_customer_items_cache:
            _fast_cache_set(cache_key, payload)
        return jsonify(payload)
    except Exception as e:
...
def api_customer_items_batch_material():
    try:
        ...
        return jsonify(success=True, count=count, material=material, snapshots=product_service_snapshots(), customers=get_customers())
    except Exception as e:
...
def api_customer_items_batch_zone():
    ...
    try:
        ...
        audit_service_safe_side_effect('batch_zone_notify', notify_sync_event, kind="refresh", module="all", message=f"商品已批量移到 {zone} 區", extra={"zone": zone, "count": count})
        return jsonify(success=True, count=count, zone=zone, snapshots=product_service_snapshots(), customers=get_customers())
    except Exception as e:
...
def api_customer_items_batch_delete():
    ...
    try:
        ...
        return jsonify(success=True, count=deleted, snapshots=product_service_snapshots(), customers=get_customers())
    except Exception as e:
...
def api_customer_items_batch_update():
    ...
    try:
        ...
        return jsonify(success=True, count=updated, items=changed, snapshots=product_service_snapshots(), customers=get_customers())
    except Exception as e:
...
def api_warehouse_return_unplaced():
    ...
    try:
        ...
        sync_extra={'zone': zone, 'column_index': column_index, 'slot_number': slot_number, 'count': len(items), 'returned_items': items, 'customers': returned_customers, 'cache_bust': 'v287'}
...
def api_warehouse_batch_add_slots():
    ...
    try:
        ...
        if not result.get('success'):
            return error_response(result.get('error') or '批量新增格子失敗')
        _warehouse_cache_clear()
...
def api_warehouse_batch_remove_slots():
    ...
    try:
        ...
        if not result.get('success'):
            return error_response(result.get('error') or '批量刪除空格失敗')
...
def _today_unplaced_cached(force=False):
    ...
    cache_key = 'today_unplaced_cache_v287'
    if not force:
        try:
    ...
    try:
        ...
        try: log_error('today_unplaced_compute_v217', str(e))
        except Exception: pass
...
def api_today_changes_count():
    """Lightweight badge endpoint for home page. It does not compute unplaced items."""
    try:
        today = _today_key()
        conn = get_db(); cur = conn.cursor()
        ...
        if read_at and str(read_at).startswith(today):
            try:
                ...
                unread = 0
        conn.close()
        return jsonify(success=True, total=total, unread=unread, today=today, read_at=read_at)
    except Exception as e:
        log_error('today_changes_count', str(e))
        return jsonify(success=True, total=0, unread=0, today=_today_key())
...
def api_today_changes():
    ...
    cache_key = _fast_cache_key('today_changes', version='v287', user=current_username(), force='1' if force else '0')
    if not force:
        cached = _fast_cache_get(cache_key, 90.0)
        if cached:
            return jsonify(cached)
    payload = dict(success=True, version='v287', **_today_changes_payload(force_unplaced=force))
    if not force:
        _fast_cache_set(cache_key, payload)
    return jsonify(payload)
...
def api_health_smoke():
    ...
    try:
        ...
        required_routes = [
            '/health', '/api/health/extended', '/api/health/smoke', '/api/warehouse/cells',
            '/api/warehouse/move-cell', '/api/ship/preview', '/api/ship/confirm',
            '/api/today-changes/badge', '/api/sync/stream', '/api/backups', '/api/backup/verify'
        ]
        ...
        for rel in ['static/yx_core.js','static/yx_pages/product_page_core.js','static/css/base.css','static/service-worker.js','static/manifest.webmanifest','wsgi.py','render.yaml','Procfile']:
