...
app = Flask(__name__)
APP_VERSION = 'V119-V179-WAREHOUSE-STABILITY-MAINFIX'
STATIC_VERSION = "119-v182-warehouse-stability"
# service-line retained: mainfile behavior consolidated into formal services.
...
def _yx145_write_payload(module='', customer_name='', item=None, extra=None):
    payload = dict(success=True, fast_response=True, module=module or '', customer_name=(customer_name or '').strip(), item=item or {}, server_cache='v146-fast-write-no-full-snapshot')
    try:
        if customer_name:
...
def api_inventory():
    if request.method == "GET":
        try:
            cache_key = _fast_cache_key('inventory', user=current_username(), limit=request.args.get('limit') or '', offset=request.args.get('offset') or '', all=request.args.get('all') or '')
            cached = _fast_cache_get(cache_key, 900.0) if (request.args.get('fast') == '1' or request.args.get('light') == '1') else None
            if cached:
...
def api_orders():
    if request.method == "GET":
        try:
            cache_key = _fast_cache_key('orders', user=current_username(), limit=request.args.get('limit') or '', offset=request.args.get('offset') or '', all=request.args.get('all') or '')
            cached = _fast_cache_get(cache_key, 900.0) if (request.args.get('fast') == '1' or request.args.get('light') == '1') else None
            if cached:
                return jsonify(cached)
    ...
    _clear_product_fast_cache()
    if _yx145_fast_write_requested(data):
        return jsonify(_yx145_write_payload('orders', customer_name, extra={'count': len(items)}))
    payload = product_service_response_payload(customer_name)
...
def api_master_orders():
    if request.method == "GET":
        try:
            cache_key = _fast_cache_key('master_orders', user=current_username(), limit=request.args.get('limit') or '', offset=request.args.get('offset') or '', all=request.args.get('all') or '')
            cached = _fast_cache_get(cache_key, 900.0) if (request.args.get('fast') == '1' or request.args.get('light') == '1') else None
            if cached:
                return jsonify(cached)
    ...
    _clear_product_fast_cache()
    if _yx145_fast_write_requested(data):
        return jsonify(_yx145_write_payload('master_order', customer_name, extra={'count': len(items)}))
    payload = product_service_response_payload(customer_name)
...
def shipping_service_validate_preview_token(token, customer_name, items):
    ...
    try:
        ...
        return False, '出貨商品已變更，請重新預覽'
    # Re-preview immediately before confirm. This keeps the final ship transaction honest
    # without forcing a full page reload or another renderer.
    fresh = preview_ship_order(customer_name, items)
...
def api_ship():
    try:
        ...
        if not ok_snapshot:
            return error_response(snapshot_error)
        ...
        if result.get("success"):
            ...
            audit_service_safe_side_effect('ship_notify', notify_sync_event, kind='row_update', module='ship', message='出貨已更新', extra={'customer_name': customer_name, 'count': len(items), 'operation_id': operation_id})
...
def api_customers():
    try:
        if request.method == "GET":
            cache_key = _fast_cache_key('customers', user=current_username(), light=request.args.get('light') or '', ship=request.args.get('ship_single') or '')
            cached = _fast_cache_get(cache_key, 900.0) if (request.args.get('fast') == '1' or request.args.get('light') == '1') else None
            if cached:
                return jsonify(cached)
            payload = dict(success=True, items=get_customers(), server_cache='v142-fifth-pack-customers-cache')
            if (request.args.get('fast') == '1' or request.args.get('light') == '1'):
                _fast_cache_set(cache_key, payload)
            return jsonify(payload)
        data = request.get_json(silent=True) or {}
...
def _warehouse_cache_clear():
    try:
        ...
        pass
    # V163: any warehouse write also clears derived fast-cache buckets so
    # available dropdown and overstock hints do not keep stale data.
    try:
        _fast_cache_clear('warehouse_available|')
        _fast_cache_clear('warehouse_source_qty_map|')
    except Exception:
...
def _warehouse_payload_from_cells(cells, include_source_qty=False):
    ...
    for cell in cells or []:
        try:
            ...
            pass
    payload = dict(success=True, zones=zones, cells=cells or [], cache_version='v142-warehouse-first-paint-cache', cache_policy='v142-client-local-first-no-stale-overwrite')
...
def _warehouse_column_payload(zone, column_index, operation_id=None, **extra):
    ...
    # V158: every warehouse write returns DB readback metadata.
    # Frontend uses this to accept/ignore responses without doing a full warehouse reload.
    payload = dict(
        ...
        warehouse_stability='v182-protected-cache-db-merge'
    )
    payload.update(extra or {})
    return payload
...
def api_warehouse_available_items():
    ...
    try:
        ...
        payload = dict(success=True, items=items, zone=zone_filter, zone_summary=zone_summary, cache_version='v142-warehouse-available-fast-cache')
        if request.args.get('fast') == '1':
...
def api_customer_items():
    ...
    if request.args.get('fast') == '1':
        cache_key = _fast_cache_key('customer_items', name=name, uid=uid, variants=request.args.get('variants') or '', user=current_username())
    ...
    payload = dict(success=True, items=aggregated, server_cache='v142-customer-items-fast-cache')
    if request.args.get('fast') == '1':
        _fast_cache_set(_fast_cache_key('customer_items', name=name, uid=uid, variants=request.args.get('variants') or '', user=current_username()), payload)
    return jsonify(payload)
...
def api_today_changes():
    ...
    cache_key = _fast_cache_key('today_changes', user=current_username(), force='1' if force else '0')
    if not force:
        cached = _fast_cache_get(cache_key, 90.0)
        if cached:
            return jsonify(cached)
    payload = dict(success=True, **_today_changes_payload(force_unplaced=force))
    if not force:
        _fast_cache_set(cache_key, payload)
    return jsonify(payload)
