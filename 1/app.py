...
# ==== YX_READ_REPAIR_20260518: product/customer API compatibility helpers ====
def _yx_read_repair_rows_payload(items, source=''):
    """Return every legacy/front-end list key so old and new renderers read the same rows."""
    rows = list(items or [])
    customers = []
    seen = set()
    for r in rows:
        try:
            name = (r.get('customer_name') or r.get('customer') or r.get('name') or '').strip()
        except Exception:
            name = ''
...
def _yx_read_repair_all_product_rows():
    """Read inventory + orders + master_orders directly from DB functions without requiring a selected customer."""
    out = []
    readers = [
        ('inventory', '庫存', list_inventory),
        ('orders', '訂單', get_orders),
        ('master_orders', '總單', get_master_orders),
    ]
    for table, label, fn in readers:
        try:
            for r in (fn() or []):
                rr = dict(r)
                rr.setdefault('source_table', table)
                rr['source_table'] = rr.get('source_table') or table
                rr.setdefault('source', label)
                rr['source'] = rr.get('source') or label
                rr.setdefault('source_label', label)
                rr['source_label'] = rr.get('source_label') or label
                out.append(rr)
        except Exception as e:
            try:
                log_error('yx_read_repair_all_product_rows_' + table, str(e))
            except Exception:
                pass
    return out
...
def api_inventory():
    try:
        if request.method == "GET":
            ...
            payload = _yx_read_repair_rows_payload(items, source='inventory-raw-first-bs')
            payload.update({'inventory': items, 'raw_items': raw_items, 'summary_items': summary_items, 'raw_count': len(raw_items)})
...
def api_inventory_visible_rescue():
    ...
    try:
        ...
        try:
            ...
            summary_items = []
        items = raw_items or summary_items or []
        payload = _yx_read_repair_rows_payload(items, source='inventory-visible-raw-first-bs')
        payload.update({'raw_items': raw_items, 'summary_items': summary_items, 'raw_count': len(raw_items)})
        return jsonify(**payload)
    except Exception as e:
        log_error('inventory_visible_rescue', str(e))
        try:
            items = list_inventory()
            return jsonify(**_yx_read_repair_rows_payload(items, source='inventory-visible-raw'))
        except Exception as e2:
...
def api_orders():
    try:
        if request.method == "GET":
            items = get_orders() or []
            return jsonify(**_yx_read_repair_rows_payload(items, source='orders'))
        data = request.get_json(silent=True) or {}
...
def api_master_orders():
    try:
        if request.method == "GET":
            items = get_master_orders() or []
            return jsonify(**_yx_read_repair_rows_payload(items, source='master_orders'))
        data = request.get_json(silent=True) or {}
...
def api_customer_items():
    ...
    items = []
    # YX_READ_REPAIR_20260518：前端有些總表會直接打 /api/customer-items 但不帶客戶。
    # 舊邏輯直接回空陣列，造成「商品/客戶讀不到」。無指定客戶時改回傳庫存+訂單+總單全部商品。
    if not name and not uid:
        all_items = aggregate_customer_items(_yx_read_repair_all_product_rows())
        return jsonify(**_yx_read_repair_rows_payload(all_items, source='customer-items-all'))
    ...
    try:
        ...
        conn.close()
    agg = aggregate_customer_items(items)
    return jsonify(**_yx_read_repair_rows_payload(agg, source='customer-items'))
...
# ==== YX_READ_REPAIR_20260518: product list aliases for old/new front-end compatibility ====
@app.route('/api/products', methods=['GET'])
@app.route('/api/items', methods=['GET'])
@login_required_json
def yx_read_repair_api_products_items():
    try:
        source = (request.args.get('source') or request.args.get('module') or '').strip().lower()
        if source in ('inventory', '庫存'):
            items = list_inventory() or []
            return jsonify(**_yx_read_repair_rows_payload(items, source='inventory'))
        if source in ('orders', 'order', '訂單'):
            items = get_orders() or []
            return jsonify(**_yx_read_repair_rows_payload(items, source='orders'))
        if source in ('master_order', 'master_orders', 'master', '總單'):
            items = get_master_orders() or []
            return jsonify(**_yx_read_repair_rows_payload(items, source='master_orders'))
        items = aggregate_customer_items(_yx_read_repair_all_product_rows())
        return jsonify(**_yx_read_repair_rows_payload(items, source='all-products'))
    except Exception as e:
