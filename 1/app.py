...
_LAST_DB_READY_CHECK_AT = 0.0

def ensure_runtime_db_ready(force=False):
    """Retry DB init after a transient Render/PostgreSQL SSL failure.
    This keeps the app alive at boot but also lets inventory/orders/master/ship/warehouse
    recover automatically instead of showing empty data until the next redeploy.
    """
    global STARTUP_DB_ERROR, _LAST_DB_READY_CHECK_AT
    if not STARTUP_DB_ERROR and not force:
        return True
    now_ts = time.time()
    if not force and now_ts - _LAST_DB_READY_CHECK_AT < 8:
        return False
    _LAST_DB_READY_CHECK_AT = now_ts
    try:
        init_db()
        STARTUP_DB_ERROR = ''
        return True
        ...
        STARTUP_DB_ERROR = str(e)
        try:
            log_error('ensure_runtime_db_ready', STARTUP_DB_ERROR)
        except Exception:
            pass
        return False
...
def protect_pages():
    ...
    if not require_login() and path not in ("/",):
        ...
        if path.startswith("/api/"):
            return jsonify(success=False, error="請先登入"), 401
        return redirect(url_for("login_page"))
    # If PostgreSQL was temporarily unavailable during boot, retry lazily on real pages/API.
    # This fixes the situation where routes exist but inventory/orders/master/shipping/warehouse read 0.
    if STARTUP_DB_ERROR and (path.startswith('/api/') or path in ('/inventory','/orders','/master-order','/shipping','/warehouse','/today-changes','/diagnostics')):
        ensure_runtime_db_ready(False)
    return None
...
@app.route("/settings")
def settings_page():
    # 20260518 settings repair:
    # 設定頁必須能打開；即使 app_settings 表/欄位在舊資料庫尚未補齊，
    # 也不能讓整個設定頁 500。缺設定時採安全預設，並記錄錯誤供診斷查詢。
    ...
    try:
        ...
        try:
            log_error('settings_page_get_setting', settings_error)
        except Exception:
            pass
    return render_template(
        ...
        title="設定",
        is_admin=is_admin,
        native_ocr_mode=native_ocr_mode,
...
def api_inventory():
    try:
        if request.method == "GET":
            # 20260518 深度修復：資料頁讀取前先嘗試恢復 DB；若 DB 真的斷線，不再假裝 success=true + 空陣列。
            try:
                if STARTUP_DB_ERROR:
                    ensure_runtime_db_ready(True)
            except Exception:
                pass
            raw_items = []
            summary_items = []
            ...
            try:
                ...
                raw_error = str(e_raw)
                log_error('inventory_get_raw_first_failed', raw_error)
                raw_items = []
            try:
                summary_items = grouped_inventory() or []
            except Exception as e_sum:
                summary_error = str(e_sum)
                log_error('inventory_get_summary_side_failed', summary_error)
                summary_items = []
            if raw_error and summary_error and not raw_items and not summary_items:
                return jsonify(success=False, error='庫存清單載入失敗：資料庫連線尚未恢復，請到設定 > 系統診斷重新檢查 DB。', db_error=(STARTUP_DB_ERROR or raw_error or summary_error)[:500], raw_error=raw_error[:300], summary_error=summary_error[:300], items=[], rows=[], data=[]), 503
            items = raw_items or summary_items or []
            return jsonify(success=True, items=items, rows=items, data=items, inventory=items, raw_items=raw_items, summary_items=summary_items, count=len(items), raw_count=len(raw_items), source='inventory-raw-first-db-recovery-v3')
        data = request.get_json(silent=True) or {}
...
def api_inventory_visible_rescue():
    ...
    try:
        ...
        try:
            ...
            raw_error = str(e2)
            log_error('inventory_visible_raw_fallback', raw_error)
            raw_items = []
        try:
            summary_items = grouped_inventory() or []
        except Exception as e3:
            summary_error = str(e3)
            log_error('inventory_visible_summary_fallback', summary_error)
            summary_items = []
        if raw_error and summary_error and not raw_items and not summary_items:
            return jsonify(success=False, error='庫存清單載入失敗：DB 連線尚未恢復，未回傳假空資料。', db_error=(STARTUP_DB_ERROR or raw_error or summary_error)[:500], items=[], rows=[], data=[]), 503
        items = raw_items or summary_items or []
        return jsonify(success=True, items=items, rows=items, data=items, raw_items=raw_items, summary_items=summary_items, count=len(items), raw_count=len(raw_items), source='inventory-visible-raw-first-db-recovery-v3')
    except Exception as e:
    ...
    try:
        if request.method == "GET":
            ...
            try:
                ...
                log_error('orders_get_failed', str(e_get))
                return jsonify(success=False, error='訂單清單載入失敗：資料庫連線尚未恢復。', db_error=(STARTUP_DB_ERROR or str(e_get))[:500], items=[]), 503
        data = request.get_json(silent=True) or {}
    ...
    try:
        if request.method == "GET":
            ...
            try:
                ...
                log_error('master_orders_get_failed', str(e_get))
                return jsonify(success=False, error='總單清單載入失敗：資料庫連線尚未恢復。', db_error=(STARTUP_DB_ERROR or str(e_get))[:500], items=[]), 503
        data = request.get_json(silent=True) or {}
...
def _yx_diag_runtime_text(rel_path, limit=1000000):
    """Read source for runtime-risk checks, ignoring comments that only document forbidden words."""
    text = _yx_diag_read_text(rel_path, limit)
    cleaned = []
...
def _yx_runtime_uses_postgres():
    try:
        from db import USE_POSTGRES as _DB_USE_POSTGRES
        return bool(_DB_USE_POSTGRES)
    except Exception:
        return (os.getenv('DATABASE_URL','') or '').strip().lower().startswith(('postgres://','postgresql://'))
...
def _yx_diag_columns(table):
    cols = []
    try:
        # 深度修復：如果 Render PostgreSQL 啟動時曾 SSL 斷線，診斷欄位讀取前先強制重連一次，避免整份報告被舊的 STARTUP_DB_ERROR 誤判成 0 欄位。
        try:
            if STARTUP_DB_ERROR:
                ensure_runtime_db_ready(True)
        except Exception:
...
def _yx_diag_build_action_checks():
    ...
    wh = _yx_diag_runtime_text('static/yx_modules/warehouse_hardlock.js')
    products = _yx_diag_runtime_text('static/yx_pages/page_products_master.js')
    ship = _yx_diag_runtime_text('static/yx_modules/ship_single_lock.js')
    checks = []
...
def _yx_diag_api_surface_full_checks():
    ...
    groups = {
        ...
        '總單 API':['/api/master_order','/api/master-order','/api/master-orders','/api/customer-items/batch-update','/api/customer-items/batch-delete','/api/customer-items/batch-material','/api/customer-items/batch-zone'],
...
def api_diagnostics_operation_flow_report():
    ...
    add_flow('庫存 / 訂單 / 總單新增與批量操作', [
        ...
        chk('總單儲存 API 存在', '/api/master_order' in app_text and '/api/master-order' in app_text and '/api/master-orders' in app_text, '總單新增與讀取路由存在。', 'critical'),
...
def api_health_db_init_v520_full():
    # v6：手動/前端重試 DB 復原時，成功後同步清掉 STARTUP_DB_ERROR，避免頁面仍誤判 DB 未恢復。
...
@app.route('/api/data/availability', methods=['GET'])
@login_required_json
def api_data_availability_v6():
    # v6：給設定/診斷/資料頁快速判斷 DB 與核心資料是否可讀；只讀、不修改資料。
    try:
        ensure_runtime_db_ready(bool(STARTUP_DB_ERROR))
    except Exception:
        pass
    result = {'success': True, 'db_ready': not bool(STARTUP_DB_ERROR), 'startup_db_error': STARTUP_DB_ERROR, 'counts': {}, 'app_version': APP_VERSION, 'static_version': STATIC_VERSION}
    for table in ['inventory','orders','master_orders','warehouse_cells','shipping_records','today_changes']:
        try:
            conn = get_db(); cur = conn.cursor(); cur.execute(sql(f'SELECT COUNT(*) AS c FROM {table}')); row = cur.fetchone(); conn.close()
            result['counts'][table] = int((row[0] if not isinstance(row, dict) else row.get('c')) or 0)
        except Exception as e:
            result['success'] = False
            result['db_ready'] = False
            result.setdefault('errors', {})[table] = str(e)[:300]
...
def _yx_diag_detailed_api_checks():
    route_groups = {
        ...
        '總單':['/api/master_order','/api/master-order','/api/master-orders'],
...
def _yx_diag_detailed_frontend_checks():
    checks = []
    base = _yx_diag_read_text('templates/base.html', 1000000)
    products = _yx_diag_runtime_text('static/yx_pages/page_products_master.js', 1000000)
    customer = _yx_diag_runtime_text('static/yx_modules/customer_regions_hardlock.js', 1000000)
    wh = _yx_diag_runtime_text('static/yx_modules/warehouse_hardlock.js', 1000000)
    today = _yx_diag_runtime_text('static/yx_modules/today_changes_hardlock.js', 1000000)
    ship = _yx_diag_runtime_text('static/yx_modules/ship_single_lock.js', 1000000)
    ...
    for tokens,label in [(['pointer'],'pointer 拖拉'),(['contextmenu','右鍵'],'右鍵操作'),(['pointerdown','setTimeout','data-yx113-customer-act'],'長按操作'),(['archive','封存'],'封存'),(['delete','刪除'],'刪除')]:
...
def _yx_diag_master_requirement_checks():
    ...
    loaded_final_count = len(re.findall(r'<link[^>]+yx_final_mainfile_ui_20260516bs\.css', base))
