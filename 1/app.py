...
STATIC_VERSION = 'final-mainfile-contract-ui-warehouse-20260516ba'
APP_VERSION = '還完整主線_主檔契約巡檢_UI仔細二次修復_出貨跳轉庫存顯示倉庫整齊_20260516ba'
...
def api_inventory_visible_rescue():
    """20260516ba：庫存頁顯示救援端點。只讀取資料，不改 DB。"""
    try:
...
def yx_dom_layout_contract():
    return jsonify({
        ...
        'expected': {
            'final_ui_css': 'static/yx_modules/yx_final_mainfile_ui_20260516ba.css',
            ...
            'ui_file_uniqueness': 'only yx_final_mainfile_ui_20260516ba.css may exist/load',
...
def _yx_diag_master_requirement_checks():
    ...
    db_src = _yx_diag_read_text('db.py')
    base = _yx_diag_read_text('templates/base.html')
    css100 = _yx_diag_read_text('static/yx_modules/yx_final_mainfile_ui_20260516ba.css')
    ...
    checks.append(_yx_diag_check('按鈕文字保護主檔已載入', ('yx_final_mainfile_ui_20260516ba.css' in base and 'YXRestoreButtonLabels' in base and 'button' in css100 and 'warehouse-zone-columns' in css100), '全頁按鈕/標籤文字與倉庫三欄規則必須由最終主檔 CSS + base label guard 同時保護。', 'critical'))
    ...
    # 20260516ba legacy final UI file audit
    legacy_ui_files = [str(p) for p in pathlib.Path('static/yx_modules').glob('yx_final_mainfile_ui_20260516*.css') if '20260516ba' not in str(p)]
    checks.append(_yx_diag_check('最終 UI 檔唯一且無舊版殘留', (len(legacy_ui_files)==0 and 'yx_final_mainfile_ui_20260516ba.css' in base), '只允許載入本版最終 UI CSS，避免舊 CSS 蓋掉按鈕文字或倉庫三欄。', 'warn'))
    ...
    checks.append(_yx_diag_check('DOM 巡檢端點版本同步', ('yx_final_mainfile_ui_20260516ba.css' in _yx_diag_read_text('app.py') and 'dom-layout-contract' in _yx_diag_read_text('app.py')), 'DOM 巡檢需能回報本版 UI CSS 與三欄兩排契約。', 'warn'))
    checks.append(_yx_diag_check('精緻 UI 已補入且排除出貨', ('css/base.css' in base and 'yx_safe_520_visual_only.css' in base and 'yx_final_mainfile_ui_20260516ba.css' in base and 'primary-btn' in (v520css+css100) and ('warehouse-cell' in (v520css+css100) or 'vertical-slot' in (v520css+css100) or 'warehouse' in (v520css+css100))), '520 背景/按鈕/卡片 CSS 已載入非出貨頁，出貨頁排除新增精緻層。', 'warn'))
    checks.append(_yx_diag_check('倉庫只補缺格不清表', ('ensure_fixed_warehouse_grid' in db_src or 'ensure_warehouse_default_slots' in db_src) and '不清空 warehouse_cells' in db_src, '倉庫資料不能被重建洗掉。', 'critical'))
    checks.append(_yx_diag_check('診斷包含匯出報告', '/api/diagnostics/export' in app_src and '匯出診斷報告' in _yx_diag_read_text('static/yx_pages/diagnostics_page.js'), '診斷報告可匯出 JSON。', 'warn'))
    checks.append(_yx_diag_check('設定頁診斷入口', '/diagnostics' in _yx_diag_read_text('templates/settings.html'), '入口放設定頁，不放首頁干擾操作。', 'warn'))
    return checks
...
def ui_runtime_contract_20260516ba():
    return jsonify({
        'success': True,
        'app_version': APP_VERSION,
        'static_version': STATIC_VERSION,
