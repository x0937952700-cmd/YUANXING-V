...
def _deduct_from_warehouse_cells_after_ship(cur, used_rows, source_table, source_customer, fallback_product_text='', material='', ship_qty=0):
    ...
    This runs inside the same DB transaction as ship_order.  It only edits existing
    warehouse cell items; it does not create cells or rebuild the warehouse map.
    Matching prefers source_table + source_id, then safely falls back to
    size/customer/material so old saved cells without source_id still decrement.
    """
    ...
    def item_match(it, allow_fallback=False):
        ...
        if source_table != 'inventory' and want_customer and item_customer and item_customer != want_customer:
            return False
            ...
            for it in items:
                ...
                if match_by_source_id:
                    if not sid or sid not in source_ids:
                        new_items.append(it); continue
                    want_take = min(source_ids.get(sid, 0), remain_total)
                    if want_take <= 0 or not item_match(it, allow_fallback=False):
                        new_items.append(it); continue
                else:
                    # Fallback only for old warehouse rows saved without source_id.
                    if sid and source_ids:
                        new_items.append(it); continue
                ...
                try:
                    ...
                    old_qty = 0
                # V204: warehouse deduct may be announced to multiple front-end modules, but
                # database deduction must be absolute and capped here. Never let a stale
                # duplicate source row deduct more than this item currently has.
                take = min(max(0, old_qty), max(0, want_take), max(0, remain_total))
                if take <= 0:
                    new_items.append(it); continue
                remain_total -= take
                if sid and sid in source_ids:
                    source_ids[sid] = max(0, source_ids.get(sid, 0) - take)
    ...
    apply_pass(True)
    apply_pass(False)
    # V223: shipping also deducts warehouse cells inside this same transaction.
    # Keep the normalized warehouse_cell_items table aligned with items_json so
    # location lookup, unplaced dropdown, and A/B counts do not disagree after refresh.
    for _z, _c in sorted(touched_columns):
        try:
            if _z in ('A', 'B') and int(_c or 0) > 0:
                _yx_120_sync_cell_items_for_column(cur, _z, _c)
        except Exception as _sync_err:
            try: log_error('ship_warehouse_cell_items_sync_v223', _sync_err)
            except Exception: pass
    return changes
