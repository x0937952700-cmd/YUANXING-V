...
# 20260518 Supabase rebuild mode
# Priority: SUPABASE_DATABASE_URL > DATABASE_URL.  Do not read old Render DB if the
# Supabase URL is provided.  For production, set REQUIRE_POSTGRES=1 so a missing
# DATABASE_URL fails loudly instead of silently creating an empty SQLite database.
_RAW_DATABASE_URL = (
    os.getenv("SUPABASE_DATABASE_URL")
    or os.getenv("DATABASE_URL")
    or ("" if os.getenv("REQUIRE_POSTGRES", "0") == "1" else "sqlite:///warehouse.db")
)
DATABASE_URL = (_RAW_DATABASE_URL or "").strip()
if not DATABASE_URL:
    raise RuntimeError("DATABASE_URL / SUPABASE_DATABASE_URL is required when REQUIRE_POSTGRES=1")

def _normalize_database_url(url: str) -> str:
    """PostgreSQL URL safety for Supabase/Render/Neon. Always require SSL unless explicitly set."""
    u = (url or '').strip().replace('\n', '').replace('\r', '')
    if u.lower().startswith(('postgres://', 'postgresql://')) and 'sslmode=' not in u.lower():
        joiner = '&' if '?' in u else '?'
...
def reset_pg_pool():
    """Close and recreate the PostgreSQL pool after Render/SSL stale connections."""
    global _PG_POOL
    pool_obj = _PG_POOL
    _PG_POOL = None
...
def _pg_connect_kwargs():
    # Render PostgreSQL can close idle SSL sockets. Keepalives + TCP user timeout
    # prevent dead pooled connections from being reused and making pages show 0 data.
    return dict(
        connect_timeout=int(os.getenv('PG_CONNECT_TIMEOUT', '12') or '12'),
        keepalives=1,
        keepalives_idle=int(os.getenv('PG_KEEPALIVES_IDLE', '30') or '30'),
        keepalives_interval=int(os.getenv('PG_KEEPALIVES_INTERVAL', '10') or '10'),
        keepalives_count=int(os.getenv('PG_KEEPALIVES_COUNT', '5') or '5'),
        application_name=os.getenv('PG_APPLICATION_NAME', 'yuanxing-warehouse'),
    )
...
def _get_pg_pool():
    ...
    if _PG_POOL is None:
        ...
        maxconn = int(os.getenv('PG_POOL_MAX', '6') or '6')
        # Pass keepalive options through psycopg2 instead of relying only on URL.
        _PG_POOL = _pg_pool.SimpleConnectionPool(minconn, maxconn, dsn=DATABASE_URL, **_pg_connect_kwargs())
    return _PG_POOL
            ...
            try:
                ...
                if getattr(conn, 'closed', 1):
                    try:
                        ...
                        pass
                    conn = None
                    raise RuntimeError('pooled postgres connection already closed')
                # Validate before giving it to callers; this prevents live pages from
                # silently reading 0 rows after Render closes an idle SSL connection.
                try:
                ...
                if conn is not None and pg_pool is not None:
                    try:
                        ...
                        pass
                if _pg_error_is_stale(e):
                    reset_pg_pool()
                try:
                    time.sleep(0.25 * (_i + 1))
                except Exception:
                    pass
        # Last chance: direct connection without pool, still with SSL/keepalive guards.
        try:
...
def ensure_fixed_warehouse_grid(conn=None, cur=None):
    ...
    整合規則：只 INSERT 缺少的空格；不清空 warehouse_cells、不重排有商品格、
    不把欄位洗成 1 格，也不覆蓋使用者已新增/插入的格子。
    """
    own_conn = False
    ...
    try:
        ...
        if total == 0:
            ...
            # 只補缺少的 1~20 空格；已存在、有商品或使用者新增的 21+ 格完全不動。
            for zone in ('A', 'B'):
                for col in range(1, 7):
                    for num in range(1, 21):
                        cur.execute(sql("""
...
# YX_PERFECT_V4_SCHEMA_PATCH: 訂單/總單/庫存/出貨/倉庫欄位完整補齊，不刪資料。
def init_db():
    conn = get_db()
    cur = conn.cursor()
...
f"""CREATE TABLE IF NOT EXISTS audit_trails (
    ...
    _schema_columns = {
        ...
        'inventory': [('customer_uid','TEXT'),('product_text','TEXT'),('product_code','TEXT'),('material','TEXT'),('month_tag','TEXT'),('qty','INTEGER DEFAULT 0'),('area','TEXT'),('location','TEXT'),('source','TEXT'),('note','TEXT'),('customer_name','TEXT'),('operator','TEXT'),('source_text','TEXT'),('created_at','TEXT'),('updated_at','TEXT')],
        ...
        'master_orders': [('customer_uid','TEXT'),('customer_name','TEXT'),('product_text','TEXT'),('product_code','TEXT'),('material','TEXT'),('month_tag','TEXT'),('qty','INTEGER DEFAULT 0'),('area','TEXT'),('location','TEXT'),('source','TEXT'),('note','TEXT'),('operator','TEXT'),('created_at','TEXT'),('updated_at','TEXT')],
        'shipping_records': [('customer_uid','TEXT'),('customer_name','TEXT'),('product_text','TEXT'),('product_code','TEXT'),('material','TEXT'),('qty','INTEGER DEFAULT 0'),('source_table','TEXT'),('before_qty','INTEGER DEFAULT 0'),('after_qty','INTEGER DEFAULT 0'),('operator','TEXT'),('shipped_at','TEXT'),('note','TEXT'),('created_at','TEXT')],
        'warehouse_cells': [('zone','TEXT'),('band','INTEGER DEFAULT 0'),('column_index','INTEGER'),('row_name',"TEXT DEFAULT ''"),('slot','INTEGER DEFAULT 0'),('slot_no','INTEGER DEFAULT 0'),('slot_type','TEXT'),('slot_number','INTEGER'),('customer_name','TEXT'),('product_text','TEXT'),('material','TEXT'),('qty','INTEGER DEFAULT 0'),('placement_label','TEXT'),('items_json','TEXT'),('note','TEXT'),('created_at','TEXT'),('updated_at','TEXT')],
        ...
        'customers': [('name','TEXT'),('customer_uid','TEXT'),('region','TEXT'),('is_archived','INTEGER DEFAULT 0'),('created_at','TEXT'),('updated_at','TEXT')],
    ...
    # YX_WAREHOUSE_DIAG_COLUMNS_PATCH: keep diagnostics-required warehouse columns without deleting data.
    for _col, _def in (
        ('band','INTEGER DEFAULT 0'), ('row_name', "TEXT DEFAULT ''"), ('slot','INTEGER DEFAULT 0'),
        ('slot_no','INTEGER DEFAULT 0'), ('customer_name','TEXT'), ('product_text','TEXT'),
        ('material','TEXT'), ('qty','INTEGER DEFAULT 0'), ('placement_label','TEXT'), ('created_at','TEXT')
    ):
        _ensure_column('warehouse_cells', _col, _def)
    try:
        cur.execute(sql("UPDATE warehouse_cells SET slot_no = COALESCE(NULLIF(slot_no,0), slot_number, slot, 0)"))
        cur.execute(sql("UPDATE warehouse_cells SET slot = COALESCE(NULLIF(slot,0), slot_number, slot_no, 0)"))
        cur.execute(sql("UPDATE warehouse_cells SET row_name = COALESCE(NULLIF(row_name,''), '')"))
        cur.execute(sql("UPDATE warehouse_cells SET created_at = COALESCE(NULLIF(created_at,''), updated_at, ?) WHERE created_at IS NULL OR created_at = ''"), (now(),))
    except Exception as e:
        log_error('warehouse_diag_columns_patch', str(e))
    ...
    # V4：診斷要求欄位補齊後，將空白 created_at / updated_at 補成安全時間；只補空值，不覆蓋舊資料。
    try:
        for _t in ['inventory','orders','master_orders','shipping_records','warehouse_cells','today_changes','logs','errors','audit_trails','customers','customer_profiles']:
            _cols = _table_columns(cur, _t)
            if 'created_at' in _cols:
                cur.execute(sql(f"UPDATE {_t} SET created_at = COALESCE(NULLIF(created_at,''), ?) WHERE created_at IS NULL OR created_at = ''"), (now(),))
            if 'updated_at' in _cols:
                cur.execute(sql(f"UPDATE {_t} SET updated_at = COALESCE(NULLIF(updated_at,''), created_at, ?) WHERE updated_at IS NULL OR updated_at = ''"), (now(),))
    except Exception as e:
        log_error('perfect_v4_timestamp_backfill', str(e))
    ...
    # 520 整合安全規則：不可 DELETE all 後重建 warehouse_cells。
    # 只把空 slot_type 標準化為 direct，並交給 ensure_fixed_warehouse_grid 補缺少空格。
    try:
        cur.execute(sql("UPDATE warehouse_cells SET slot_type = 'direct' WHERE slot_type IS NULL OR slot_type = ''"))
    except Exception as e:
        log_error('warehouse_safe_slot_type_direct', str(e))
    ...
    # YX_FINAL_REQUIRED_COLUMNS_PATCH: final non-destructive schema alignment for full diagnostics.
    _final_required_cols = {
        'inventory': [('month_tag','TEXT'),('area','TEXT'),('source','TEXT'),('note','TEXT')],
        'orders': [('month_tag','TEXT'),('area','TEXT'),('source','TEXT'),('note','TEXT'),('location','TEXT')],
        'master_orders': [('month_tag','TEXT'),('area','TEXT'),('source','TEXT'),('note','TEXT'),('location','TEXT')],
        'shipping_records': [('source_table','TEXT'),('before_qty','INTEGER DEFAULT 0'),('after_qty','INTEGER DEFAULT 0'),('created_at','TEXT')],
        'warehouse_cells': [('band','INTEGER DEFAULT 0'),('row_name',"TEXT DEFAULT ''"),('slot','INTEGER DEFAULT 0'),('slot_no','INTEGER DEFAULT 0'),('customer_name','TEXT'),('product_text','TEXT'),('material','TEXT'),('qty','INTEGER DEFAULT 0'),('placement_label','TEXT'),('created_at','TEXT')],
        'today_changes': [('action','TEXT'),('table_name','TEXT'),('customer_name','TEXT'),('product_text','TEXT'),('detail_json','TEXT'),('operator','TEXT'),('created_at','TEXT'),('unread','INTEGER DEFAULT 1')],
    }
    for _table, _cols in _final_required_cols.items():
    ...
    try:
        ...
        log_error('final_required_columns_patch', str(e))
