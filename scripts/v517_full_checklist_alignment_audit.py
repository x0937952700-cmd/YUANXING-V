...
# Versions
for tok in ['V119-V517-FULL-CHECKLIST-ALIGNMENT-PACK27','119-v517_full_checklist_alignment_pack27','v517-full-checklist-alignment-pack27']:
...
# Diagnostics/report
for token in ['/api/diagnostics/action-audit','/api/diagnostics/master-requirements','/api/health/postdeploy-evidence-report','v517_static_audit_resolved']:
    ok(token in app+read('static/yx_pages/diagnostics_page.js'), f'diagnostics marker missing {token}')
# Checklist shipped
ok((root/'diagnostics_v517_full_alignment_checklist.txt').exists(), 'v517 checklist not shipped')
if fail:
    print('V517 FULL CHECKLIST ALIGNMENT AUDIT FAILED')
    for x in fail: print('-',x)
    sys.exit(1)
print('V517 FULL CHECKLIST ALIGNMENT AUDIT OK')
