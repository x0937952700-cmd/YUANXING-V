...
(function(){
  ...
  root.sync = root.sync || {};
  root.version = '119-v155-asset-cache-alignment';
  window.YX = root;
})();
...
(function(){
    ...
    try {
      ...
      for (const item of q.slice()) {
        ...
        try {
          ...
          if (isPermanentFailure(e)) { remove(item.id); }
          else {
  ...
  function enqueue(url, payload, opt){
    ...
    try { window.dispatchEvent(new CustomEvent('yx:bg-save-queued', {detail:{pending:read().length, item}})); } catch(_e) {}
    return item;
  }
  ...
  window.YXBackgroundSave = {enqueue, request, requestSoft, drain, pending:()=>read().length, key:KEY, version:'119-v155-queued-lanes'};
...
(function(){ try { window.YX = window.YX || {}; window.YX.cacheVersion = 'v155-degrade-guard'; } catch(_e){} })();
