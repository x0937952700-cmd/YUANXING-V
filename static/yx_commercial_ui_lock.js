...
async function loadRegionsFinal(m){if(!['orders','master_order','ship'].includes(m)) return; let d=await api('/api/regions/'+m); const map={'北區':'region-north','中區':'region-center','南區':'region-south'}; Object.keys(map).forEach(r=>{let el=$(map[r]); if(el){let arr=(d.details&&d.details[r])||[]; el.innerHTML=arr.length?arr.map(customerButton).join(''):'<span class="muted">無</span>';}})}
window.selectCustomer=async function(name){try{window.__yxSelectedCustomer=name; let c=$('customer-name'); if(c)c.value=name; let m=page(); ensureCustomerTable(m,name); let d=await api('/api/customer-items?module='+encodeURIComponent(m)+'&customer='+encodeURIComponent(name)); window.__yxRows=d.items||[]; renderProductTable(window.__yxRows,m); fillShipSelect(d.items||[]); if(m==='ship'){let ta=$('ocr-text'); if(ta && !ta.value.trim()) ta.placeholder='可從下拉選單點商品加入，或整批加入後修改要出的數量';} }catch(e){toast(e.message)}};

function optionLabel(x){let pp=parseProduct(x.product); return `${pp.size}${pp.expr?'='+pp.expr:''}｜${qtyOf(x)}件｜${x.material||'未填材質'}｜${x.source||''}`}
function fillShipSelect(items){window.__shipItems=items||[]; let sel=$('ship-customer-item-select'); if(sel) sel.innerHTML='<option value="">請選擇商品</option>'+window.__shipItems.map((x,i)=>`<option value="${i}">${esc(optionLabel(x))}</option>`).join('');}
...
window.confirmSubmit=async function(){try{let m=page(); let text=$('ocr-text')?.value||''; let customer=$('customer-name')?.value||window.__yxSelectedCustomer||''; let btn=$('submit-btn'); if(btn)btn.disabled=true; let d=await api('/api/submit/'+m,{method:'POST',body:JSON.stringify({text,customer})}); showSubmitResult(d); toast(d.message||'已送出'); if(m==='inventory') loadProductTable(); if(['orders','master_order','ship'].includes(m)){loadRegionsFinal(m); if(customer) selectCustomer(customer);} }catch(e){toast(e.message)}finally{let btn=$('submit-btn'); if(btn)btn.disabled=false}}
