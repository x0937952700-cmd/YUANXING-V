...
(function(){
  ...
  async function retryFailedWarehouseSaves(opts={}){
    ...
    if(ok){ try{ clearWarehouseCaches(); cacheWarehouseNow(); notifyWarehouseChanged({action:'cell-save-retry',version:'v307',count:ok}); await loadAvailable().catch(()=>{}); updateAllSlots(); }catch(_e){} }
    try{ window.dispatchEvent(new CustomEvent('yx:operation-status',{detail:{source:'warehouse',status:fail?'pending':'success',reason:'warehouse-cell-save-retry',message:fail?`倉庫保存已重送 ${ok} 筆，仍有 ${fail} 筆待重試`:`倉庫保存已重送 ${ok} 筆`,count:ok,failed:fail,version:'v307',detail_text:arr.slice(0,3).map(r=>r.label||r.id).filter(Boolean).join('、')}})); }catch(_e){}
  ...
  async function retryFailedWarehouseStructureOps(opts={}){
    ...
    if(ok){
      try{
        ...
        notifyWarehouseChanged({action:'structure-op-retry',version:'v307',count:ok});
      }catch(_e){}
    }
    try{ window.dispatchEvent(new CustomEvent('yx:operation-status',{detail:{source:'warehouse',status:fail?'pending':'success',reason:'warehouse-structure-retry',message:fail?`倉庫結構操作已重送 ${ok} 筆，仍有 ${fail} 筆待重試`:`倉庫結構操作已重送 ${ok} 筆`,count:ok,failed:fail,version:'v307',detail_text:arr.slice(0,3).map(r=>r.label||r.id).filter(Boolean).join('、')}})); }catch(_e){}
  ...
  function bgPost(url, payload, onSuccess, label, opts={}){
    ...
    return Promise.resolve(runner).then(result=>{
      ...
      try{window.dispatchEvent(new CustomEvent('yx:operation-soft-failed',{detail:{source:'warehouse',reason:'warehouse-background-failed',error:result?.error||'請稍後確認',version:'v307',retry_saved:isStructureRetry,url,payload,label,cell_label:[clean(payload?.zone||payload?.from?.zone||''), payload?.column_index||payload?.from?.column_index||'', payload?.slot_number||payload?.from?.slot_number||''].filter(Boolean).join('-')}}));}catch(_e){}
  ...
  const CACHE_VERSION = 'v332-shortcut-target-highlight';
  ...
  function buildConsistencyPayload(detail){
    ...
    return {id:[op,z,c,s].join('|'), operation_id:op, zone:z, column_index:c, slot_number:s, action:clean(detail?.action||detail?.reason||'warehouse-final-check'), client_cell_signature:(z&&c&&s)?localCellCompareSignature(z,c,s):'', client_available_signature:localAvailableSignature(), version:'v307'};
  }
  ...
  function mutateAvailableByItems(items, sign, fallbackZone){
    ...
    state.availableByZone = state.availableByZone || {A:[],B:[]};
    if(!Array.isArray(state.availableByZone.A)) state.availableByZone.A=[];
    if(!Array.isArray(state.availableByZone.B)) state.availableByZone.B=[];
    const targetListsForItem = (item)=>{
      const bucket=availableZoneBucket(item, fallbackZone);
      return [allList, bucket==='B'?state.availableByZone.B:state.availableByZone.A];
    };
    (items||[]).forEach(raw=>{
      const item=withWarehouseZone([raw], fallbackZone)[0] || raw;
      const g=availableGroupKey(item);
      targetListsForItem(item).forEach(list=>{
        ...
        if(sign < 0){
          ...
          if(q<=0) return;
          const mergeIndex=list.findIndex(it=>availableGroupKey(it)===g && productSupportText(it)===productSupportText(item));
          if(mergeIndex>=0){
            const it=list[mergeIndex];
  ...
  function notifyWarehouseChanged(detail){
    ...
    const enriched = {...detail, source:'warehouse', reason:detail.action||'warehouse-changed', version:'v307', customer_names:customerNames};
  ...
  function saveCellPayload(z,c,s,items,note){ return {operation_id:yxOperationId('warehouse-cell-save'),zone:clean(z).toUpperCase(),column_index:Number(c),slot_type:'direct',slot_number:Number(s),items:sanitizeCellItemsForSave(items||[]),note:note||'',client_stability:'v332-shortcut-target-highlight'}; }
  ...
  async function saveWarehouseCell(){
    ...
    try{
      ...
      try{ persistWarehouseDraft(); }catch(_e){}
      try{window.dispatchEvent(new CustomEvent('yx:operation-soft-failed',{detail:{source:'warehouse',reason:'cell-save-failed',error:e.message||'儲存格位失敗',version:'v307',zone:state.current?.zone,column_index:state.current?.col,slot_number:state.current?.slot,payload:saveCellPayload(state.current?.zone,state.current?.col,state.current?.slot,state.current?.items||[],($('warehouse-note')?.value||''))}}));}catch(_e){}
  ...
  async function moveCellContents(from,to){
    ...
    try{
      ...
      queuedWarehouseMovePost({
        ...
        client_stability:'v332-shortcut-target-highlight',
  ...
  async function batchInsertWarehouseCells(z,c,s){
    ...
    queuedWarehousePost('/api/warehouse/batch-add-slots',{operation_id:yxOperationId('warehouse-batch-add-slots'),client_stability:'v332-shortcut-target-highlight',zone:z,column_index:c,insert_after:s,count,slot_type:'direct'}, (d, token)=>{
  ...
  async function insertWarehouseCell(z,c,s){
    ...
    const token=beginColumnOp(z,c);
    state.lastGoodColumns.set(token.key, columnBefore);
    queuedWarehousePost('/api/warehouse/add-slot',{operation_id:yxOperationId('warehouse-add-slot'),client_stability:'v332-shortcut-target-highlight',zone:z,column_index:c,insert_after:s,slot_type:'direct'}, (d, token)=>{
  ...
  async function deleteWarehouseCell(z,c,s){
    ...
    const token=beginColumnOp(z,c);
    state.lastGoodColumns.set(token.key, columnBefore);
    queuedWarehousePost('/api/warehouse/remove-slot',{operation_id:yxOperationId('warehouse-remove-slot'),client_stability:'v332-shortcut-target-highlight',zone:z,column_index:c,slot_number:s,slot_type:'direct'}, (d, token)=>{
  ...
  async function returnWarehouseCell(z,c,s){
    ...
    const token=beginColumnOp(z,c);
    state.lastGoodColumns.set(token.key, columnBefore);
    queuedWarehousePost('/api/warehouse/return-unplaced',{operation_id:yxOperationId('warehouse-return-unplaced'),client_stability:'v332-shortcut-target-highlight',zone:z,column_index:c,slot_number:s}, async (d, token)=>{
      applyWarehouseResponse(d,z,c, token);
  ...
  function bindGlobal(){
    ...
    document.addEventListener('click',async ev=>{
      ...
      if(ev.target?.id==='yx121-save-cell'){ ev.preventDefault(); try{ await saveWarehouseCell(); }catch(e){ try{window.dispatchEvent(new CustomEvent('yx:operation-soft-failed',{detail:{source:'warehouse',reason:'cell-save-failed',error:e.message||'儲存格位失敗',version:'v307',zone:state.current?.zone,column_index:state.current?.col,slot_number:state.current?.slot,payload:saveCellPayload(state.current?.zone,state.current?.col,state.current?.slot,state.current?.items||[],($('warehouse-note')?.value||''))}}));}catch(_e){}
  ...
  function install(){ if(!isWarehouse()) return; document.documentElement.dataset.yxWarehouseSingleHtmlDataJs='true'; document.documentElement.dataset.yxWarehouseLongpressDbSync='v332-shortcut-target-highlight'; bindGlobal(); bindSlots(); setTimeout(()=>{ retryPendingWarehouseConsistencyChecks({toast:false}).catch(()=>{}); }, 900); if(!state.productDataChangedBound){ state.productDataChangedBound=true; const refreshFromExternal=(ev)=>{ const d=ev&&ev.detail?ev.detail:ev; const eid=[ev&&ev.type||'', d&&d.operation_id||d&&d.request_key||d&&d.event_id||'', d&&d.reason||'', d&&d.customer_name||''].join('::'); state.externalRefreshSeen=state.externalRefreshSeen||new Map(); const now=Date.now(); const last=Number(state.externalRefreshSeen.get(eid)||0); if(eid && last && now-last<900) return; state.externalRefreshSeen.set(eid,now); let applied=false; try{ applied=!!applyWarehouseDeductFromShip(d); if(applied) updateAllSlots(); }catch(_e){} try{ clearWarehouseCaches(); }catch(_e){} try{ state.availableCache = {}; }catch(_e){} try{ state.availableSeq++; }catch(_e){} try{ if(applied){ setTimeout(()=>{ try{ renderWarehouse(true); }catch(_e){} }, 680); } else renderWarehouse(true); }catch(_e){} }; window.addEventListener('yx:product-data-changed',refreshFromExternal,false); window.addEventListener('yx:ship-completed',refreshFromExternal,false); window.addEventListener('yx:order-master-changed',refreshFromExternal,false); } setWarehouseZone(localStorage.getItem('warehouseActiveZone')||'A',false); renderWarehouse(false); }
  ...
  window.YXFinalWarehouse={version:'v332-shortcut-target-highlight',render:renderWarehouse, openWarehouseModal, saveWarehouseCell, jumpProductToWarehouse, applyTargetedRetryRefresh};
