/* 沅興木業 FULL MASTER V3 PER PAGE - page_ship_master_v2 */
(function(){ window.__YX_FULL_MASTER_V2_PAGE__='page_ship_master_v2'; })();
...
(function(){
  ...
  function syncHiddenSelect(){const select=$('ship-customer-item-select');if(!select)return;select.hidden=false;select.removeAttribute('hidden');select.setAttribute('aria-hidden','false');if(!state.customer){select.innerHTML='<option value="">請先點選北 / 中 / 南客戶</option>';return;}if(!state.items.length){select.innerHTML='<option value="">此客戶目前沒有可出貨商品</option>';return;}select.innerHTML='<option value="">點這裡選擇商品</option>'+state.items.map((it,i)=>`<option value="${i}">${esc(productLabel(it))}</option>`).join('');}
  ...
  function renderItems(){const box=$('ship-customer-item-list');syncHiddenSelect();if(box)box.innerHTML='';if(!state.customer){setCount('請先點選北 / 中 / 南客戶');return;}if(state.loadingName===state.customer){setCount(`${state.customer}：商品載入中…`);return;}if(!state.items.length){setCount(`${state.customer}：0 筆 / 0 件`);return;}const total=state.items.reduce((sum,it)=>sum+qtyFromText(it.product_text,it.qty),0);setCount(`${state.customer}：${state.items.length} 筆 / ${total} 件`);}
  ...
  function install(){document.documentElement.dataset.yxShipSingle='locked-one-line-html-v7';bind();loadCustomers();window.addEventListener('yx:customers-loaded',e=>{state.customers=Array.isArray(e.detail?.items)?e.detail.items:state.customers;},false);window.addEventListener('yx:customer-selected',e=>{const name=clean(e.detail?.name||'');if(name)loadItems(name,{force:true}).catch(err=>toast(err.message,'error'));},false);renderSelected();const name=clean($('customer-name')?.value||'');if(name)loadItems(name).catch(()=>{});else renderItems();window.confirmSubmit=confirmSubmit;}
