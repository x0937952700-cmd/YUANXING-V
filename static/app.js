...
async function undoLastAction(){
  ...
  try {
    ...
    if (state.module) {
      if (state.module === 'inventory') { await loadInventory(); await loadCustomerBlocks(); }
...
async function renderWarehouse(){
  try {
    const savedZone = localStorage.getItem('warehouseActiveZone') || state.warehouse.activeZone || 'A';
    ...
    renderWarehouseZones();
    setWarehouseZone(savedZone, false);
    try {
...
function setWarehouseZone(zone, doScroll=true){
  ...
  const zoneA = $('zone-A');
  const zoneB = $('zone-B');
  if ($('warehouse-selection-pill')) $('warehouse-selection-pill').textContent = `目前區域：${mode === 'ALL' ? '全部' : mode + ' 區'}`;
  ...
  if (doScroll) jumpToZone(mode === 'ALL' ? 'A' : mode);
}
