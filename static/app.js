...
(function(){
  ...
  let loading=null;async function renderWarehouse(force=false){if(loading)return loading;loading=(async()=>{try{const d=await api('/api/warehouse?yx105=1&ts='+Date.now(),{method:'GET'});wh().cells=Array.isArray(d.cells)?d.cells:[];wh().zones=d.zones||{A:{},B:{}};renderGrid();try{if(typeof window.installUnplacedPill99==='function')window.installUnplacedPill99();}catch(_e){}}catch(e){toast(e.message||'倉庫圖載入失敗','error');}finally{loading=null;}})();return loading;}

  function modalCard(){const m=$('warehouse-modal');return m?.querySelector?.('.modal-card')||m;}
  function cleanupModal(){const m=$('warehouse-modal');if(!m)return;m.classList.remove('yx101-final-cell-editor','yx102-final-cell-editor','yx103-final-cell-editor');m.classList.add('yx105-final-cell-editor');m.querySelectorAll('#yx99-warehouse-detail-panel,#yx101-warehouse-detail-panel,#yx102-warehouse-detail-panel,#yx103-warehouse-detail-panel,#yx80-warehouse-batch-panel,#yx81-warehouse-batch-panel,#yx82-warehouse-batch-panel,#yx83-warehouse-batch-panel,#yx89-warehouse-batch-panel,#yx91-warehouse-batch-panel,#yx97-warehouse-batch-panel,#yx98-warehouse-batch-panel,#yx99-warehouse-batch-panel,#yx103-warehouse-batch-panel,#warehouse-cell-items,#warehouse-detail-panel,.yx101-warehouse-detail-panel,.yx102-warehouse-detail-panel,.yx103-warehouse-detail-panel,.yx99-warehouse-detail-panel').forEach(el=>{if(el.id==='warehouse-cell-items'||el.id==='warehouse-detail-panel'){el.innerHTML='';el.style.display='none';el.classList.add('hidden','yx105-hidden-legacy');}else el.remove();});}
  function ensureDetail(){const card=modalCard();if(!card)return null;let el=$('yx105-warehouse-detail-panel');if(!el){el=document.createElement('div');el.id='yx105-warehouse-detail-panel';el.className='yx105-warehouse-detail-panel';}const meta=$('warehouse-modal-meta');if(meta?.parentNode)meta.insertAdjacentElement('afterend',el);else card.insertBefore(el,card.firstChild);return el;}
  function ensureBatch(){const card=modalCard();if(!card)return null;let el=$('yx105-warehouse-batch-panel');if(!el){el=document.createElement('div');el.id='yx105-warehouse-batch-panel';el.className='yx105-warehouse-batch-panel';}const note=$('warehouse-note');const noteLabel=note?.previousElementSibling?.matches?.('label.field-label')?note.previousElementSibling:null;(noteLabel?.parentNode||card).insertBefore(el,noteLabel||note||null);return el;}
  function currentCell(){const c=window.state?.currentCell||{};return{zone:clean(c.zone).toUpperCase(),col:Number(c.column_index||c.column||c.col||0),slot:Number(c.slot_number||c.slot||c.num||0)};}
  function renderDetails(){const detail=ensureDetail();if(!detail)return;const c=currentCell();const items=Array.isArray(window.state?.currentCellItems)?window.state.currentCellItems:cellItems(c.zone,c.col,c.slot);detail.innerHTML=`<div class="yx105-detail-head"><div class="section-title">格位詳細資料</div>${items.length?'<button type="button" class="ghost-btn tiny-btn" id="yx105-return-unplaced-btn">返回未入倉</button>':''}</div>`+(items.length?items.map((it,idx)=>`<div class="deduct-card yx105-detail-card" draggable="true"><div class="yx105-detail-title"><span class="yx105-placement-badge">${esc(placement(idx,it))}</span><strong>${esc(customerName(it.customer_name||'未指定客戶'))}</strong></div><div class="yx105-detail-main">${esc(productText(it))}</div><div class="yx105-detail-line">尺寸：${esc(productText(it))}</div><div class="yx105-detail-line">材質：${esc(materialText(it))}</div><div class="yx105-detail-line qty">數量：${qtyOf(it)} 件</div></div>`).join(''):'<div class="empty-state-card compact-empty">此格目前沒有商品</div>');const btn=$('yx105-return-unplaced-btn');if(btn)btn.onclick=()=>returnCellToUnplaced(c.zone,c.col,c.slot);}
  async function refreshAvailable(){try{const d=await api('/api/warehouse/available-items?yx105=1&ts='+Date.now(),{method:'GET'});wh().availableItems=Array.isArray(d.items)?d.items:[];}catch(_e){wh().availableItems=Array.isArray(wh().availableItems)?wh().availableItems:[];}return wh().availableItems;}
  function defaultBatchRows(){const cur=Array.isArray(window.state?.currentCellItems)?window.state.currentCellItems:[];return cur.map((it,idx)=>({value:`cell:${idx}`,qty:String(qtyOf(it)||0)}));}
  function snapshotBatch(){return Array.from(document.querySelectorAll('#yx105-warehouse-batch-panel .yx105-batch-row')).map(row=>({value:row.querySelector('select')?.value||'',qty:row.querySelector('input[type="number"]')?.value||'0'}));}
  function mergeOptions(available){const cur=Array.isArray(window.state?.currentCellItems)?window.state.currentCellItems:[];const map=new Map();cur.forEach((it,idx)=>map.set(`cell:${idx}`,{...it,__key:`cell:${idx}`,__current:true,__idx:idx}));(available||[]).forEach((it,idx)=>map.set(`avail:${idx}`,{...it,__key:`avail:${idx}`,__current:false,__idx:idx,source:'unplaced'}));return map;}
  async function renderBatch(keep){const batch=ensureBatch();if(!batch)return;const base=defaultBatchRows();const prev=keep||(base.length?base:snapshotBatch());const rows=Math.max(3,Number(batch.dataset.rows||0),prev.length,base.length);const q=clean($('warehouse-item-search')?.value||'').toLowerCase();const available=await refreshAvailable();const optMap=mergeOptions(available);const options=Array.from(optMap.values()).filter(it=>it.__current||!q||`${it.customer_name||''} ${materialText(it)} ${productText(it)} ${it.source_summary||''}`.toLowerCase().includes(q));const optionHTML=options.map(it=>`<option value="${esc(it.__key)}">${esc(optionText(it,it.__current))}</option>`).join('');const rowHtml=i=>{const label=i===0?'後排':i===1?'中間':i===2?'前排':`第${i+1}筆`;return`<div class="yx105-batch-row" data-batch-idx="${i}"><label>${label}</label><select class="text-input yx105-batch-select"><option value="">不加入</option>${optionHTML}</select><input class="text-input yx105-batch-qty" type="number" min="0" value="0"></div>`;};batch.innerHTML=`<label class="field-label">批量加入商品</label><div class="small-note">格內原本的後排／中間／前排會自動帶入；要新增商品請按「增加批量」，會從第4筆開始。</div><div id="yx105-batch-rows">${Array.from({length:rows},(_,i)=>rowHtml(i)).join('')}</div><div class="btn-row compact-row"><button type="button" class="ghost-btn small-btn" id="yx105-add-batch-row">增加批量</button><button type="button" class="primary-btn small-btn" id="yx105-add-batch-items">批量加入格位</button></div>`;batch.querySelectorAll('.yx105-batch-row').forEach((row,i)=>{const st=prev[i]||base[i]||{};const sel=row.querySelector('select');const qty=row.querySelector('input[type="number"]');if(st.value&&optMap.has(st.value)){sel.value=st.value;qty.value=st.qty&&st.qty!=='0'?st.qty:String(qtyOf(optMap.get(st.value)));}sel.addEventListener('change',()=>{const it=optMap.get(sel.value);qty.value=it?String(qtyOf(it)):'0';});});$('yx105-add-batch-row').onclick=()=>{batch.dataset.rows=String(rows+1);renderBatch(snapshotBatch().concat([{value:'',qty:'0'}]));};$('yx105-add-batch-items').onclick=batchSave;}
  ...
  async function saveCell(){const c=currentCell();if(!c.zone||!c.col||!c.slot)throw new Error('格位資料不完整，請重新點選格子');const d=await api('/api/warehouse/cell',{method:'POST',body:JSON.stringify({zone:c.zone,column_index:c.col,slot_type:'direct',slot_number:c.slot,items:Array.isArray(window.state?.currentCellItems)?window.state.currentCellItems:[],note:$('warehouse-note')?.value||''})});if(Array.isArray(d.cells))wh().cells=d.cells;if(d.zones)wh().zones=d.zones;return d;}
  async function batchSave(){const btn=$('yx105-add-batch-items');const available=wh().availableItems||[];const optMap=mergeOptions(available);const rows=Array.from(document.querySelectorAll('#yx105-warehouse-batch-panel .yx105-batch-row'));const next=[];rows.forEach(row=>{const idx=Number(row.dataset.batchIdx||0);const key=row.querySelector('select')?.value||'';const src=optMap.get(key);if(!src)return;let qty=parseInt(row.querySelector('input[type="number"]')?.value||'0',10);if(!Number.isFinite(qty)||qty<=0)return;const label=idx===0?'後排':idx===1?'中間':idx===2?'前排':`第${idx+1}筆`;if(!src.__current)qty=Math.min(qty,qtyOf(src));next.push({...src,product_text:productText(src),product_size:productText(src),material:materialText(src),product_code:materialText(src),qty,customer_name:src.customer_name||'',placement_label:label,layer_label:label,source:src.__current?(src.source||'placed'):'unplaced'});});if(!next.length)return toast('請至少保留或選擇一筆商品','warn');try{if(btn){btn.disabled=true;btn.textContent='儲存中…';}window.state.currentCellItems=next.map(({__key,__current,__idx,...it})=>it);await saveCell();const c=currentCell();await renderWarehouse(true);await openModal(c.zone,c.col,c.slot,{skipFetch:true});toast(`已更新格位，共 ${next.length} 筆`,'ok');}catch(e){toast(e.message||'批量加入失敗','error');}finally{if(btn){btn.disabled=false;btn.textContent='批量加入格位';}}}
  ...
  function install(){document.documentElement.dataset.yxFix105=VERSION;installEvents();if(isToday()){loadTodayAuto();}if(isWarehouse()){window.renderWarehouse=renderWarehouse;window.renderWarehouseZones=renderGrid;setTimeout(()=>renderWarehouse(true),30);[150,450,1000].forEach(ms=>setTimeout(()=>document.querySelectorAll('.yx105-slot').forEach(bindSlot),ms));}}
