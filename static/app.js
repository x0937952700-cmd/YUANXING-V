...
async function confirmSubmit(){
  ...
  try {
    ...
    if (customer_name && ['inventory','orders','master_order'].includes(module)) {
    ...
    <div class="success-card">
      ...
      ${breakdown.map(item => `
        <div class="deduct-card">
          ...
          <div class="small-note">總單扣 ${item.master_deduct || 0}｜訂單扣 ${item.order_deduct || 0}｜庫存扣 ${item.inventory_deduct || 0}</div>
          <div class="small-note">來源：${item.used_inventory_fallback ? '總單 / 訂單不足，已改扣庫存' : '依正常流程扣減'}</div>
          <div class="ship-breakdown-list">${(item.locations || []).map(loc => `<span class="ship-location-chip">${escapeHTML(loc.zone)}-${loc.column_index}-${String(loc.visual_slot || loc.slot_number || 0).padStart(2,'0')}｜扣 ${loc.ship_qty || loc.qty || 0}</span>`).join('') || '<span class="small-note">倉庫圖中尚未找到此商品位置</span>'}</div>
        </div>
      `).join('')}
    </div>`;
}
...
function showResult(msg, isError=false){
  ...
  if (!box) return;
  box.classList.remove('hidden');
  box.innerHTML = `<div class="${isError ? 'error-card' : 'success-card'}">${escapeHTML(msg || '')}</div>`;
  if (msg) toast(String(msg).replace(/^錯誤：/, ''), isError ? 'error' : 'ok');
}
...
async function loadCustomerBlocks(){
  try {
    ...
    (data.items || []).filter(c => !q || (c.name || '').toLowerCase().includes(q)).forEach(c => {
      ...
      card.addEventListener('click', () => { if (card.dataset.longPressTriggered === '1') { card.dataset.longPressTriggered = '0'; return; } if (['inventory','orders','master_order','ship'].includes(state.module)) selectCustomerForModule(c.name); else openCustomerModal(c.name); });
      if (state.module === 'customers') {
        let holdTimer = null;
        const startHold = () => {
          clearTimeout(holdTimer);
          holdTimer = setTimeout(async () => {
            card.dataset.longPressTriggered = '1';
            const ok = await askConfirm(`確定刪除客戶「${c.name}」？`, '刪除客戶', '刪除', '取消');
            if (!ok) return;
            try {
              await requestJSON(`/api/customers/${encodeURIComponent(c.name)}`, { method:'DELETE' });
              card.remove();
              const panel = $('selected-customer-items');
              if (panel && panel.querySelector('.section-title')?.textContent?.trim() === c.name.trim()) {
                panel.classList.add('hidden');
                panel.innerHTML = '';
              }
              toast('客戶已刪除', 'ok');
            } catch (e) { toast(e.message || '客戶刪除失敗', 'error'); }
          }, 700);
        };
...
async function selectCustomerForModule(name){
  ...
  try {
    ...
    if (panel) {
      ...
      const stats = buildCommonCustomerStats(data.items || []);
      const groupedShipItems = Array.from((data.items || []).reduce((map, it) => {
        const key = (it.product_text || '').trim();
        if (!key) return map;
        const row = formatCustomerProductRow(key);
        const obj = map.get(key) || { product_text:key, size: row.size || key, totalQty:0 };
        obj.totalQty += Number(it.qty || 0);
        map.set(key, obj);
        return map;
      }, new Map()).values());
      const shipPicker = state.module === 'ship' ? `
        <div class="ship-picker-card">
          <div class="section-title">選取要出貨的商品</div>
          <div class="ship-picker-row">
            <select id="ship-item-select" class="text-input">
              <option value="">請選擇商品</option>
              ${groupedShipItems.map(it => `<option value="${escapeHTML(it.product_text)}">${escapeHTML(it.product_text)}｜可出 ${it.totalQty || 0}</option>`).join('')}
            </select>
            <button class="ghost-btn" id="ship-item-add-btn">加入商品資料</button>
          </div>
          <div id="ship-item-avail" class="small-note">選商品後會顯示可扣來源與倉庫位置</div>
        </div>` : '';
      ...
      if (batchToggle) batchToggle.onchange = () => {
        if (batchToggle.checked) toast('已啟用批量添加材質，請直接使用商品資料內容中的材質規則。', 'ok');
      };
      if (state.module === 'ship') {
        const shipSelect = $('ship-item-select');
        const shipInfo = $('ship-item-avail');
        const updateAvail = async () => {
          const val = shipSelect?.value || '';
          if (!val) { if (shipInfo) shipInfo.textContent = '選商品後會顯示可扣來源與倉庫位置'; return; }
          try {
            const preview = await requestJSON('/api/ship-preview', { method:'POST', body: JSON.stringify({ customer_name: name, items:[{ product_text: val, product_code: val, qty: 1 }] }) });
            const row = (preview.items || [])[0];
            if (row && shipInfo) {
              const loc = (row.locations || []).map(loc => `${loc.zone}-${loc.column_index}-${String(loc.visual_slot || loc.slot_number || 0).padStart(2,'0')}`).join('、') || '未錄入倉庫圖';
              shipInfo.textContent = `總單可扣 ${row.master_available || 0}｜訂單可扣 ${row.order_available || 0}｜庫存可扣 ${row.inventory_available || 0}｜位置：${loc}`;
            }
          } catch (e) {
            if (shipInfo) shipInfo.textContent = '可扣數量讀取失敗';
          }
        };
        shipSelect?.addEventListener('change', updateAvail);
        updateAvail();
        $('ship-item-add-btn')?.addEventListener('click', () => {
          const val = shipSelect?.value || '';
          if (!val) return toast('請先選擇商品', 'warn');
          appendShipProductToTextarea(val);
          toast('已加入商品資料框', 'ok');
        });
      }
    }
  ...
  if (state.module === 'ship') {
    ...
    if (resultBox) {
      if (!rows.length) {
        ...
        resultBox.innerHTML = `<div class="success-card"><div class="section-title">反查商品位置</div>${rows.map(r => `<div class="deduct-card"><div><strong>${escapeHTML(r.product_text)}</strong></div><div class="small-note">${escapeHTML(r.cell.zone)} 區第 ${r.cell.column_index} 欄 第 ${String(r.cell.slot_number).padStart(2,'0')} 格</div><div class="small-note">客戶：${escapeHTML(r.item.customer_name || '未指定客戶')}</div></div>`).join('')}</div>`;
      }
    }
    return;
  }
