...
/* ===== warehouse fixed skeleton override (full base app.js preserved) ===== */
window.__warehouseFixedOverride = true;
...
function buildCellKey(zone, column, num){
  return [zone, Number(column), 'direct', Number(num)];
}

function getCellItems(zone, column, num){
  const cells = (state.warehouse && Array.isArray(state.warehouse.cells)) ? state.warehouse.cells : [];
  const found = cells.find(c => String(c.zone)===String(zone) && Number(c.column_index)===Number(column) && Number(c.slot_number)===Number(num));
  if (!found) return [];
  try {
    if (Array.isArray(found.items_json)) return found.items_json;
    return JSON.parse(found.items_json || '[]');
  } catch (_e) {
    return [];
  }
}
...
function renderWarehouseBaseGrid(){
  ['A','B'].forEach(zone => {
    ...
    /* only generate if template didn't already output cards */
    if (wrap.querySelector('.vertical-column-card')) return;
    wrap.innerHTML = '';
...
function bindWarehouseSlotInteractions(){
  document.querySelectorAll('.vertical-slot').forEach(slot => {
    if (slot.dataset.boundWarehouse === '1') return;
    slot.dataset.boundWarehouse = '1';
    ...
    slot.addEventListener('click', () => {
      const items = getCellItems(zone, c, n);
      try { if (typeof showWarehouseDetail === 'function') showWarehouseDetail(zone, c, n, items); } catch(_e){}
      try { if (typeof openWarehouseModal === 'function') openWarehouseModal(zone, c, n); } catch(_e){}
    });
...
function applyWarehouseDataToGrid(){
  ['A','B'].forEach(zone => {
    ...
    wrap.querySelectorAll('.vertical-slot').forEach(slot => {
      ...
      try {
        highlighted = !!(state.searchHighlightKeys && state.searchHighlightKeys.has(directKey));
      } catch(_e){}
...
async function renderWarehouse(){
  ...
  setWarehouseZone((state.warehouse && state.warehouse.activeZone) || 'A', false);
}
...
async function searchWarehouse(){
  ...
  try {
    ...
    data.items.forEach(r => {
      ...
      div.addEventListener('click', () => {
        setWarehouseZone(cell.zone);
        setTimeout(() => {
          highlightWarehouseCell(cell.zone, cell.column_index, cell.slot_number);
...
function highlightWarehouseCell(zone, column, num){
  const slot = document.querySelector(`.vertical-slot[data-zone="${zone}"][data-column="${column}"][data-num="${num}"]`);
  if (!slot) return;
...
/* ===== end warehouse fixed skeleton override ===== */
