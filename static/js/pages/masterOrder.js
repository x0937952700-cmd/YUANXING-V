...
function renderCustomers(customers) { const active = state.selectedCustomer?.uid || ''; return ['北區','中區','南區'].map(r => `<div class="region-col"><div class="region-title">${r}</div>${customers.filter(c=>c.region===r).map(c=>customerCard(c,active)).join('') || '<div class="empty">沒有總單客戶</div>'}</div>`).join(''); }
